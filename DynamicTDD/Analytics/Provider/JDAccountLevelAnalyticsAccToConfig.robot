...
JD-TC-AccountLevelAnalyticsAccToConfig-1

    [Documentation]   take walk in checkins for a provider and check account level analytics according to config for
    ...   TOTAL_FOR_TOKEN, TOTAL_ON_TOKEN, SP_USER_LOGIN_WEB_COUNT,  NEW_CONSUMER_TOTAL.
    ...
    Append To File  ${EXECDIR}/data/TDD_Logs/providernumbers.txt  ${SUITE NAME} - ${TEST NAME} - ${PUSERPH0}${\n}
    
    ${DAY1}=  db.get_date_by_timezone  ${tz}
    ${user_count}=  Evaluate  ${user_count}+1
    Set Suite Variable  ${user_count}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${user_count}  ${resp.content}
    ...
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['metricId']}  ${tokenAnalyticsMetrics['TOTAL_FOR_TOKEN']}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['value']}     ${token_len}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['amount']}    ${def_amt}
   
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][2]['metricId']}  ${tokenAnalyticsMetrics['TOTAL_ON_TOKEN']}
    ...
    ${resp}=  Get Account Level Analytics Acc To config   ${DAY1}   dateTo=${DAY1}   frequency=${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Get Account Level Analytics Acc To config   ${DAY1}   dateTo=${DAY1}   frequency=${analyticsFrequency[4]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[4]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['metricId']}  ${tokenAnalyticsMetrics['TOTAL_FOR_TOKEN']}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['value']}     ${token_len}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['amount']}    ${def_amt}
    # Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['dateFor']}   ${DAY1}
...
JD-TC-AccountLevelAnalyticsAccToConfig-2
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${online_token_ids}  ${cwid${a}}
    ...
    Log Many   ${waitlist_ids}  ${online_token_ids}
    ...
    ${online_token_len}=  Evaluate  len($online_token_ids)
    Set Test Variable   ${online_token_len}
