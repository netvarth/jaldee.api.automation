...
${start}       11
${def_amt}     0.0
&{androidcons_headers}       Content-Type=application/json  User-Agent=android  BOOKING_REQ_FROM=CONSUMER_APP 
&{android_sp_headers}        Content-Type=application/json  User-Agent=android  BOOKING_REQ_FROM=SP_APP  
...
JD-TC-AndroidLevelAnalytics-1
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=  App Consumer Login  ${androidcons_headers}  ${CUSERNAME${a}}  ${PASSWORD}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        ${resp}=  App Consumer Logout   ${androidcons_headers}
        Log  ${resp.content}
        Should Be Equal As Strings    ${resp.status_code}    200
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=  App Get consumer Waitlist By Id  ${androidcons_headers}  ${cwid${a}}  ${pid}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AndroidLevelAnalytics-2
    ...
    ...  ${resp.json()['apptSchedule']['timeSlots'][0]['eTime']}  5    5  ${lid}  ${resp.json()['timeDuration']}  ${bool[1]}  ${s_id}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=  App Get Appointment Slots By Schedule and Date    ${androidcons_headers}   ${sch_id2}    ${DAY1}   ${pid}    
        Log   ${resp.json()}
        Should Be Equal As Strings    ${resp.status_code}    200
        Verify Response  ${resp}  scheduleId=${sch_id2}
        ${no_of_slots}=  Get Length  ${resp.json()['availableSlots']}
        ...
        ${resp}=   App Take Appointment For Provider   ${androidcons_headers}  ${pid}  ${s_id}  ${sch_id2}  ${DAY1}  ${cnote}   ${apptfor}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        ${resp}=   App Get consumer Appointment By Id   ${androidcons_headers}  ${pid}  ${apptid1}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AndroidLevelAnalytics-3
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=   App Create Order For Pickup   ${androidcons_headers}  ${cookie}   ${pid}    ${self}    ${CatalogId4}   ${bool[1]}  ${sTime1}    ${eTime1}   ${DAY2}    ${CUSERNAME${a}}    ${email}  ${countryCodes[1]}  ${EMPTY_List}   ${item_id1}    ${item_quantity2} 
        Log   ${resp.json()}
        Should Be Equal As Strings    ${resp.status_code}    200
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=   App Get Order By Id  ${androidcons_headers}  ${pid}  ${orderid${a}}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200

        ${resp}=   App Get Order By Id  ${androidcons_headers}  ${pid}  ${orderid1${a}}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['dateFor']}   ${FUTDAY1}

    ${resp}=  Get Account Level Analytics  metricId=${orderAnalyticsMetrics['WEB_ORDER']}  dateFrom=${FUTDAY1}  dateTo=${DAY}  frequency=${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}       ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues']}    []

    ${resp}=  Get Account Level Analytics  metricId=${orderAnalyticsMetrics['ONLINE_ORDER']}  dateFrom=${FUTDAY1}  dateTo=${DAY}  frequency=${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['value']}     ${android_order_len}
...
JD-TC-AndroidLevelAnalytics-4
    ...
    ${resp}=   App ProviderLogin  ${android_sp_headers}  ${PUSERPH0}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=  App GetCustomer  ${android_sp_headers}  phoneNo-eq=${CUSERNAME${a}}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  App GetCustomer  ${android_sp_headers}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=   App ProviderLogout  ${android_sp_headers}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=   Get Account Level Analytics  ${tokenAnalyticsMetrics['ANDROID_TOKENS']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
    ...
    ${resp}=   Get Account Level Analytics  ${tokenAnalyticsMetrics['WEB_TOKENS']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}       ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues']}    []

    ${resp}=   Get Account Level Analytics  ${tokenAnalyticsMetrics['WALK_IN_TOKEN']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
...
JD-TC-AndroidLevelAnalytics-5
    ...
    ${resp}=   Get Account Level Analytics  ${tokenAnalyticsMetrics['PHONE_TOKEN']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
...
JD-TC-AndroidLevelAnalytics-6
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${resp}=  App Get Appointment Slots By Date Schedule  ${android_sp_headers}  ${sch_id2}  ${DAY1}  ${s_id}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        Verify Response  ${resp}  scheduleId=${sch_id2}
        ${no_of_slots}=  Get Length  ${resp.json()['availableSlots']}
        ...
        ${cnote}=   FakerLibrary.word
        ${resp}=  App Take Appointment For Consumer  ${android_sp_headers}  ${cid${a}}  ${s_id}  ${sch_id2}  ${DAY2}  ${cnote}  ${apptfor}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=   Get Account Level Analytics  ${appointmentAnalyticsMetrics['ANDROID_APPMTS']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
    ...
    ${resp}=   Get Account Level Analytics  ${appointmentAnalyticsMetrics['WEB_APPMTS']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}       ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues']}    []

    ${resp}=   Get Account Level Analytics  ${appointmentAnalyticsMetrics['WALK_IN_APPMT']}  ${DAY2}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
...
JD-TC-AndroidLevelAnalytics-7
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${cnote}=   FakerLibrary.word   
        ${resp}=  App Take Appointment with Appointment Mode  ${android_sp_headers}  ${appointmentMode[1]}  ${cid${a}}  ${s_id}  ${sch_id2}  ${DAY3}  ${cnote}  ${apptfor}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=   Get Account Level Analytics  ${appointmentAnalyticsMetrics['ANDROID_APPMTS']}  ${DAY3}  ${DAY3}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
    ...
    ${resp}=   Get Account Level Analytics  ${appointmentAnalyticsMetrics['WEB_APPMTS']}  ${DAY3}  ${DAY3}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}       ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues']}    []

    ${resp}=   Get Account Level Analytics  ${appointmentAnalyticsMetrics['PHONE_APPMT']}  ${DAY3}  ${DAY3}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
