...
JD-TC-ServiceWiseAnalytics-3
    ...
    ${Future_service_count_len}=   Evaluate  len($service_count) + len($service_count1) + ${Future_count1}
...
JD-TC-ServiceWiseAnalytics-4
    [Documentation]   take prepayment checkin for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN

    ${resp}=   ProviderLogin  ${PUSERPH0}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${SERVICE3}=    Set Variable  ${ser_names[3]}
    ${min_pre}=   Random Int   min=10   max=50
    ${servicecharge}=   Random Int  min=100  max=200
    ${s_id3}=  Create Sample Service with Prepayment   ${SERVICE3}  ${min_pre}  ${servicecharge}  maxBookingsAllowed=10
    ...
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}  ${s_id}  ${s_id2}   ${s_id3}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${service_count2}=  Create List
    Set Suite Variable   ${service_count2}
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${desc}=   FakerLibrary.word
        ${resp}=  Add To Waitlist  ${cid${a}}  ${s_id3}  ${q_id1}  ${DAY1}  ${desc}  ${bool[1]}  ${cid${a}} 
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        Append To List   ${service_count2}  ${wid${a}}

    END

    Log List   ${prepay_walkin_waitlist_ids}
    Log List   ${service_count2}

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2)
    Set Suite Variable   ${service_count_len}
    sleep  01s
    # sleep  05m
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['metricId']}  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['value']}   ${TOTAL_ON_TOKEN_Service_Count_Len}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['amount']}   ${def_amt}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['dateFor']}   ${DAY1}

JD-TC-ServiceWiseAnalytics-5
    [Documentation]   take checkin for a virtual service for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${resp}=  Create virtual Service  ${SERVICE4}   ${description}   2   ${status[0]}   ${btype}    ${bool[1]}    ${notifytype[2]}  ${EMPTY}  ${Total1}  ${bool[0]}   ${bool[0]}   ${vstype}   ${virtualCallingModes}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${resp.json()['queueStartTime']}  ${resp.json()['queueEndTime']}
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}  ${s_id}  ${s_id2}  ${s_id3}  ${v_s1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${vs_service_count}=  Create List
    Set Suite Variable   ${vs_service_count}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${vs_service_count}  ${wid${a}}


    END

    Log List   ${vs_walkin_waitlist_ids}
    Log List   ${vs_service_count}

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count)
    Set Suite Variable   ${service_count_len}
    sleep  01s
    # sleep  05m
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
...
JD-TC-ServiceWiseAnalytics-6
    [Documentation]   take online checkins for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${online_service_count}=  Create List
    Set Suite Variable   ${online_service_count}
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${online_service_count}  ${cwid${a}}
    ...
    Log List   ${online_service_count}

    ${resp}=   Provider Login  ${PUSERPH0}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count)
    Set Suite Variable   ${service_count_len}
    sleep  01s
    # sleep  05m
    ...
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_FOR_TOKEN']}  ${DAY}  ${DAY}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['metricId']}  ${ServiceWiseMetric['TOTAL_FOR_TOKEN']}
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY}  ${DAY}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['metricId']}  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}
...
JD-TC-ServiceWiseAnalytics-7
    [Documentation]   take online checkins for a prepayment service for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${SERVICE4}=    Set Variable  ${ser_names[4]}
    ${min_pre}=   Random Int   min=10   max=50
    ${servicecharge}=   Random Int  min=100  max=200
    ${s_id4}=  Create Sample Service with Prepayment   ${SERVICE4}  ${min_pre}  ${servicecharge}  maxBookingsAllowed=10
    ...
    ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${resp.json()['queueStartTime']}  ${resp.json()['queueEndTime']}
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}    ${s_id}  ${s_id2}  ${s_id3}  ${s_id4}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${online_prepay_service_count}=  Create List
    Set Suite Variable   ${online_prepay_service_count}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${DAY}=  get_date
        ${cnote}=   FakerLibrary.word
        ${resp}=  Add To Waitlist Consumers  ${pid}  ${q_id1}  ${DAY}  ${s_id4}  ${cnote}  ${bool[0]}  ${self} 
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ${wid}=  Get Dictionary Values  ${resp.json()}
        Set Suite Variable  ${cwid${a}}  ${wid[0]}
        ...
        Append To List   ${online_prepay_service_count}  ${cwid${a}}
    ...
    Log List   ${online_prepay_service_count}
    ...
    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count)
    Set Suite Variable   ${service_count_len}
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY}  ${DAY}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
...
JD-TC-ServiceWiseAnalytics-8
    [Documentation]   take online checkin for a virtual service for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${Total1}=   Random Int   min=100   max=500
    ${Total1}=  Convert To Number  ${Total1}  1
    ${SERVICE5}=    Set Variable  ${ser_names[5]}
    ${description}=    FakerLibrary.word
    Set Test Variable  ${vstype}  ${vservicetype[1]}
    ${resp}=  Create virtual Service  ${SERVICE5}   ${description}   2   ${status[0]}   ${btype}    ${bool[1]}    ${notifytype[2]}  ${EMPTY}  ${Total1}  ${bool[0]}   ${bool[0]}   ${vstype}   ${virtualCallingModes}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${resp.json()['queueStartTime']}  ${resp.json()['queueEndTime']}
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}  ${s_id}  ${s_id2}  ${s_id3}  ${s_id4}  ${v_s2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${online_vs_service_count}=  Create List
    Set Suite Variable   ${online_vs_service_count}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${cnote}=   FakerLibrary.word
        ${resp}=  Consumer Add To WL With Virtual Service  ${pid}  ${q_id1}  ${DAY1}  ${v_s2}  ${cnote}  ${bool[0]}  ${vScallingmode}   0
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200 
        ${wid}=  Get Dictionary Values  ${resp.json()}
        Set Suite Variable  ${cwid${a}}  ${wid[0]}
        ...
        Append To List   ${online_vs_service_count}  ${cwid${a}}
    ...
    Log List   ${online_vs_service_count}

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count)
    Set Suite Variable   ${service_count_len}
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
...
JD-TC-ServiceWiseAnalytics-9
    [Documentation]   take phone-in checkins for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${pro_phonein_service_count}=  Create List
    Set Suite Variable   ${pro_phonein_service_count}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${desc}=   FakerLibrary.word
        ${resp}=  Add To Waitlist with mode  ${waitlistMode[2]}  ${cid${a}}  ${s_id2}  ${q_id1}  ${DAY1}  ${desc}  ${bool[1]}  ${cid${a}} 
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        Append To List   ${pro_phonein_service_count}  ${wid${a}}

    END

    Log List   ${pro_phonein_service_count}

    # ${Ser_phonein_token_len}=  Evaluate  len($pro_phonein_service_count)
    # Set Suite Variable   ${Ser_phonein_token_len}

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count)
    Set Suite Variable   ${service_count_len}
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
...
JD-TC-ServiceWiseAnalytics-10
    [Documentation]   take phone-in checkins for a prepayment service for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${SERVICE5}=    Set Variable  ${ser_names[6]}
    ${min_pre}=   Random Int   min=10   max=50
    ${servicecharge}=   Random Int  min=100  max=200
    ...
    ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${resp.json()['queueStartTime']}  ${resp.json()['queueEndTime']}
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}  ${s_id}  ${s_id2}  ${s_id3}  ${s_id4}   ${s_id5} 
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${pro_phonein_prepay_service_count}=  Create List
    Set Suite Variable   ${pro_phonein_prepay_service_count}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${desc}=   FakerLibrary.word
        ${resp}=  Add To Waitlist with mode  ${waitlistMode[2]}  ${cid${a}}  ${s_id5}  ${q_id1}  ${DAY1}  ${desc}  ${bool[1]}  ${cid${a}} 
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        Append To List   ${pro_phonein_prepay_service_count}  ${wid${a}}

    END

    Log List   ${pro_phonein_prepay_service_count}

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count) + len($pro_phonein_prepay_service_count)
    Set Suite Variable   ${service_count_len}
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count) + len($pro_phonein_prepay_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
...
JD-TC-ServiceWiseAnalytics-11
    [Documentation]   take phone-in checkin for a virtual service for a provider and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${Total1}=   Random Int   min=100   max=500
    ${Total1}=  Convert To Number  ${Total1}  1
    ${SERVICE7}=    Set Variable  ${ser_names[7]}
    ${description}=    FakerLibrary.word
    Set Test Variable  ${vstype}  ${vservicetype[1]}
    ${resp}=  Create virtual Service  ${SERVICE7}   ${description}   2   ${status[0]}   ${btype}    ${bool[1]}    ${notifytype[2]}  ${EMPTY}  ${Total1}  ${bool[0]}   ${bool[0]}   ${vstype}   ${virtualCallingModes}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${resp.json()['queueStartTime']}  ${resp.json()['queueEndTime']}
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}  ${s_id}  ${s_id2}  ${s_id3}  ${s_id4}   ${s_id5}  ${v_s3}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${pro_vs_phonein_service_count}=  Create List
    Set Suite Variable   ${pro_vs_phonein_service_count}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${desc}=   FakerLibrary.word
        ${virtualService}=  Create Dictionary   ${CallingModes[1]}=${countryCodes[0]}${PUSERPH0} 
        ${resp}=  Provider Add To WL With Virtual Service  ${cid${a}}  ${v_s3}  ${q_id1}  ${DAY1}  ${desc}  ${bool[1]}  ${waitlistMode[2]}  ${virtualService}   ${cid${a}}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        Append To List   ${pro_vs_phonein_service_count}  ${wid${a}}

    END

    Log List   ${pro_vs_phonein_service_count}

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count) + len($pro_phonein_prepay_service_count) + len($pro_vs_phonein_service_count)
    Set Suite Variable   ${service_count_len}
    ...
    ${TOTAL_ON_TOKEN_Service_Count_Len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count) + len($pro_phonein_prepay_service_count) + len($pro_vs_phonein_service_count) + len($Future_count1)
    ${resp}=  Get Account Level Analytics  ${ServiceWiseMetric['TOTAL_ON_TOKEN']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}   ${analyticsFrequency[0]}
...
JD-TC-ServiceWiseAnalytics-12
    [Documentation]   Reschedule a checkin in checked-in status and check service wise analytics for TOTAL_FOR_TOKEN and TOTAL_ON_TOKEN
    ...
    ${SERVICE8}=    Set Variable  ${ser_names[8]}
    ${s_id8}=  Create Sample Service  ${SERVICE8}  maxBookingsAllowed=10
    ...
    # ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    # ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${time_now}  ${eTime1}
    # ...  ${resp.json()['parallelServing']}   500  ${lid}  ${s_id}  ${s_id8}
    # Log  ${resp.content}
    # Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Update Queue  ${q_id1}  ${resp.json()['name']}  ${resp.json()['queueSchedule']['recurringType']}  ${resp.json()['queueSchedule']['repeatIntervals']}
    ...  ${resp.json()['queueSchedule']['startDate']}  ${EMPTY}  ${EMPTY}  ${resp.json()['queueStartTime']}  ${resp.json()['queueEndTime']}
    ...  ${resp.json()['parallelServing']}   ${resp.json()['capacity']}  ${lid}  ${s_id}  ${s_id2}  ${s_id3}  ${s_id4}   ${s_id5}  ${s_id8}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${DAY}=  get_date
        ${cnote}=   FakerLibrary.word
        ${resp}=  Add To Waitlist Consumers  ${pid}  ${q_id1}  ${DAY}  ${s_id8}  ${cnote}  ${bool[0]}  ${self} 
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ${wid}=  Get Dictionary Values  ${resp.json()}
        Set Suite Variable  ${cwid${a}}  ${wid[0]}

        Append To List   ${online_waitlist_Service_count}  ${cwid${a}}
    ...
    Log List   ${online_waitlist_Service_count}

    ${resp}=   Provider Login  ${PUSERPH0}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${service_count_len}=   Evaluate  len($service_count) + len($service_count1) + len($service_count2) + len($vs_service_count) + len($online_service_count) + len($online_prepay_service_count) + len($online_vs_service_count) + len($pro_phonein_service_count) + len($pro_phonein_prepay_service_count) + len($pro_vs_phonein_service_count) + len($online_waitlist_Service_count)
    Set Suite Variable   ${service_count_len}
    ...
    ${rescheduled_serice}=  Create List
    ${rescheduled_online_serice}=  Create List
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${rescheduled_serice}  ${cwid${a}}
        Append To List   ${rescheduled_online_serice}  ${cwid${a}}
    ...
    Log List   ${rescheduled_online_serice}
    Set Suite Variable   ${rescheduled_online_serice}

    Log List   ${rescheduled_serice}
    Set Suite Variable   ${rescheduled_serice}
