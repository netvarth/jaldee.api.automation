...
JD-TC-Take Appointment-UH13
    ...
    ${resp}=    Add FamilyMember For ProviderConsumer       ${family_fname1}  ${family_lname1}  ${dob}  ${gender}  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable  ${cidfor}   ${resp.json()}
...
JD-TC-Take Appointment-14
   ...
   #............provider consumer creation..........
    ...
    Set Test Variable  ${consumerEmail}  ${CUSERNAME10}${f_Name}.${test_mail}

    
    ${resp}=  AddCustomer  ${CUSERNAME10}    firstName=${f_Name}   lastName=${l_Name}  countryCode=${countryCodes[1]}   email=${consumerEmail}  
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Take Appointment-15
    ...
    # clear_appt_schedule   ${HLPUSERNAME20}
    ${DAY1}=  db.get_date_by_timezone  ${tz}
    ${DAY2}=  db.add_timezone_date  ${tz}  10        
    ${list}=  Create List  1  2  3  4  5  6  7
    ${sTime1}=  add_timezone_time  ${tz}  0  15  
   ...
   #............provider consumer creation..........
    ...
    ${resp}=    Add FamilyMember For ProviderConsumer   ${mem_fname1}  ${mem_lname1}  ${dob}  ${gender}  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable   ${fmem1}  ${resp.json()}
    ...
    ${resp}=    Add FamilyMember For ProviderConsumer   ${mem_fname2}  ${mem_lname2}  ${dob}  ${gender}   
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200 
...
JD-TC-Take Appointment-UH7
    ...
    ${s_id}=  Create Sample Service  ${SERVICE4}
...
JD-TC-Take Appointment-UH8
    ...
    ${s_id}=  Create Sample Service  ${SERVICE5}
...
JD-TC-Take Appointment-18
    ...
    ${resp}=  AddCustomer  ${CUSERNAME25}    firstName=${fname1}   lastName=${lname1}  countryCode=${countryCodes[1]}   email=${consumerEmail}  
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Make payment Consumer Mock  ${pidHL}  ${min_pre}  ${purpose[0]}  ${apptid2}  ${s_id}  ${bool[0]}   ${bool[1]}   ${cid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Take Appointment-19
    ...
    ${resp}=  AddCustomer  ${CUSERNAME1}    firstName=${f_Name}   lastName=${l_Name}  countryCode=${countryCodes[1]}  
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=    Send Otp For Login    ${CUSERNAME1}    ${pid}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token   ${CUSERNAME1}    ${pid}  ${token7} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-Take Appointment-21
    ...
    ${resp}=  AddCustomer  ${CUSERNAME2}    firstName=${f_Name}   lastName=${l_Name}  countryCode=${countryCodes[1]}  
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=    ProviderConsumer Login with token   ${CUSERNAME2}    ${pid}  ${token7} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-Take Appointment-UH15
    ...
    ${resp}=    Get All Schedule Slots By Date Location and Service  ${pid02}  ${DAY3}  ${p1_l1}  ${p1_s2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${no_of_slots}=  Get Length  ${resp.json()[0]['availableSlots']}
    @{slots}=  Create List
...
JD-TC-Take Appointment-UH16
    ...
    [Documentation]   Consumer tries to take an appointment when online presence disabled.
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  ${resp.json()}  ${APPT_NOT_TAKEN}

    ${resp}=  Encrypted Provider Login  ${PUSERNAME60}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=  Set jaldeeIntegration Settings    ${boolean[1]}  ${EMPTY}  ${boolean[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=  Get jaldeeIntegration Settings
    Log   ${resp.json()}
...
JD-TC-Take Appointment-UH9
    ...
    ${invalid}=  Evaluate  ${pid3}+8000000000
    ${cid4}=  get_id  ${CUSERNAME4}
