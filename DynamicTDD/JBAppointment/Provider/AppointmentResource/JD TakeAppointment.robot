...
JD-TC-Take Appointment-37
    ...
    ${branch}=   db.get_place
    ${resp}=   Update Account Payment Settings   ${bool[0]}  ${bool[0]}  ${bool[1]}  ${PUSERNAME345}   ${pan_num}  ${bank_ac}  ${bank_name}  ${ifsc_code}  ${name}  ${name}  ${branch}  ${businessFilingStatus[1]}  ${accountType[1]}   
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=   Update Account Payment Settings   ${bool[1]}  ${bool[0]}  ${bool[1]}  ${PUSERNAME345}   ${pan_num}  ${bank_ac}  ${bank_name}  ${ifsc_code}  ${name}  ${name}  ${branch}  ${businessFilingStatus[1]}  ${accountType[1]}   
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-Take Appointment-UH1
    ...
    ${PUSERNAME_A}=  Evaluate  ${PUSERNAME}+450001449
...
JD-TC-Take Appointment-UH20
    ...
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer   ${cid}  ${s_id}  ${sch_id}  ${DAY1}  ${cnote}  ${apptfor}  appointmentMode=${appointmentMode[2]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
...
JD-TC-Take Appointment-UH28
    ...
    ${resp}=    Verify Otp For Login   ${CUSERPH_SECOND}   ${OtpPurpose['Authentication']}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable  ${token}  ${resp.json()['token']}
...
JD-TC-Take Appointment-40
    ...
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer  ${cid}  ${s_id}  ${sch_id}  ${DAY1}  ${cnote}  ${apptfor}  phoneNumber=${CUSERPH8}   countryCode=91
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Take Appointment-41
    ...
    ${CUSERPH8}=  Evaluate  ${CUSERNAME8}+${PO_Number}
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer  ${cid}  ${s_id}  ${sch_id}  ${DAY1}   ${cnote}  ${apptfor}  phoneNumber=${CUSERPH8}  countryCode=${country_code}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
