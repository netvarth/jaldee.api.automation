...
JD-TC-GetAppointmentToday-1
    ...
    # clear_location_n_service  ${PUSERNAME238}
    clear_customer   ${PUSERNAME238}
...
JD-TC-GetAppointmentToday-4
    ...
    # clear_service   ${PUSERNAME238}
    # clear_appt_schedule   ${PUSERNAME238}
    ...
    ${SERVICE1}=    generate_unique_service_name  ${service_names}
    Append To List  ${service_names}  ${SERVICE1}
    ${s_id}=  Create Sample Service  ${SERVICE1}   isPrePayment=${bool[1]}   minPrePaymentAmount=${prepay_amt}  maxBookingsAllowed=10
    # Log   ${resp.content}
    # Should Be Equal As Strings  ${resp.status_code}  200  
    # Set Test Variable  ${s_id}  ${resp.json()}
...
JD-TC-GetAppointmentToday-24
    ...
    ${cnote}=   FakerLibrary.name
    ${resp}=   Customer Take Appointment   ${account_id}  ${s_id1}  ${sch_id}  ${DAY1}  ${cnote}   ${apptfor}    location=${lid}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${apptid2}=  Get From Dictionary  ${resp.json()}  ${fname}
