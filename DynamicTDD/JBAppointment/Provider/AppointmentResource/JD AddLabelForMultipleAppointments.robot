...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    Set Suite Variable  ${email_id1}  ${CUSERNAME19}.${test_mail}


    ${resp}=    Send Otp For Login    ${CUSERNAME19}    ${accountId}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer SignUp    ${fname1}  ${lname1}  ${email_id1}    ${CUSERNAME19}     ${accountId}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200    
...
JD-TC-AddMultipleAppointmentLabel-6
    ...
    ${resp}=  AddCustomer  ${CUSERNAME19}   firstName=${fname1}   lastName=${lname1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AddMultipleAppointmentLabel-9
    ...
    ${slot1}=  Set Variable  ${random slots[0]}
    ${slot2}=  Set Variable  ${random slots[1]}
    ...
    ${resp}=   Take Appointment For Provider   ${accountId}  ${s_id}  ${sch_id}  ${DAY1}  ${cnote}   ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AddMultipleAppointmentLabel-10
    ...
    ${apptfor}=   db.apptfor  ${self}  ${slot2}  ${fname1}

    ${cnote}=   FakerLibrary.name
    ${resp}=   Take Appointment For Provider   ${accountId}  ${s_id1}  ${sch_id}  ${DAY1}  ${cnote}   ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AddMultipleAppointmentLabel-UH7
    ...
    Set Test Variable  ${accountId1}  ${resp.json()['id']}
    ...
    ELSE
        ...
        Set Suite Variable  ${tz2}  ${resp.json()[0]['bSchedule']['timespec'][0]['timezone']}
    END
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    ${resp}=    ProviderConsumer SignUp    ${fname2}  ${lname2}  ${email_id2}    ${primaryMobileNo2}     ${accountId1}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200    
   
    ${resp}=    ProviderConsumer Login with token   ${primaryMobileNo2}    ${accountId1}  ${token2} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    Set Suite Variable  ${email_id3}  ${CUSERNAME20}.${test_mail}


    ${resp}=    Send Otp For Login    ${CUSERNAME20}    ${accountId1}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer SignUp    ${fname3}  ${lname3}  ${email_id3}    ${CUSERNAME20}     ${accountId1}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200    
   
    ${resp}=    ProviderConsumer Login with token   ${CUSERNAME20}    ${accountId1}  ${token3} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=  Create Sample Schedule   ${lid2}   ${s_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${sch_id}  ${resp.json()}
    ...
    ${apptfor1}=  Create Dictionary  id=${cid3}   apptTime=${slot1}
    ${apptfor}=   Create List  ${apptfor1}
    
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer  ${cid3}  ${s_id}  ${sch_id}  ${DAY1}  ${cnote}  ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
          
    ${apptid1}=  Get From Dictionary  ${resp.json()}  ${fname2}
    ...
    ${apptfor1}=  Create Dictionary  id=${cid4}   apptTime=${slot2}
    ${apptfor}=   Create List  ${apptfor1}
    
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer  ${cid4}  ${s_id}  ${sch_id}  ${DAY1}  ${cnote}  ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    
    ${apptid2}=  Get From Dictionary  ${resp.json()}  ${fname3}
