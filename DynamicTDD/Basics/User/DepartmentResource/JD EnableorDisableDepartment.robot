...
${SERVICE5}	   Groom MakeupW_E
${SERVICE6}	   Groom MakeupW_F
${SERVICE7}	   Groom MakeupW_G
${SERVICE8}	   Groom MakeupW_H
...
JD-TC-Enable or Disable Department-3
    ...
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${did02}  ${resp.json()}
    ${total_amount}=    Random Int  min=100  max=500
    ${min_prepayment}=  Random Int   min=1   max=50
    ${resp}=  Create Service Department  ${SERVICE3}  ${desc}   ${ser_duratn}    ${bType}  ${bool[1]}  ${notifytype[2]}  ${min_prepayment}  ${total_amount}  ${bool[1]}  ${bool[0]}  ${did02}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${sid3}  ${resp.json()}
    ${resp}=  Create Service Department  ${SERVICE4}  ${desc}   ${ser_duratn}   ${bType}  ${bool[1]}  ${notifytype[2]}  ${min_prepayment}  ${total_amount}  ${bool[1]}  ${bool[0]}  ${did02}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${sid4}  ${resp.json()}
    ${resp}=  Enable service   ${sid4} 
    #${resp}=  Disable Department  ${did02} 
    #Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=  Add Services To Department  ${did01}  ${sid4}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=  Get Department ById  ${did01}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}  departmentName=${dep_name1}  departmentId=${did01}  departmentCode=${dep_code1}  departmentDescription=${dep_desc}  departmentStatus=${status[0]}
