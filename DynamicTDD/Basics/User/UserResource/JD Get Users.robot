...
JD-TC-GetUsers-1
     ...
     ${PH_Number}=  Evaluate  ${PUSERNAME}+550117
     ${highest_package}=  get_highest_license_pkg


     ${resp}=  Account SignUp  ${firstname_A}  ${lastname_A}  ${None}  ${domains}  ${sub_domains}  ${PH_Number}    ${highest_package[0]}
     Log  ${resp.json()}
     Should Be Equal As Strings    ${resp.status_code}    202
     ${resp}=  Account Activation  ${PH_Number}  0
     Log   ${resp.json()}
     Should Be Equal As Strings    ${resp.status_code}    200
     ${resp}=  Account Set Credential  ${PH_Number}  ${PASSWORD}  0  ${PH_Number}
     Should Be Equal As Strings    ${resp.status_code}    200
     ${resp}=  Encrypted Provider Login  ${PH_Number}  ${PASSWORD}
     Log  ${resp.json()}
     Should Be Equal As Strings    ${resp.status_code}    200


     Append To File  ${EXECDIR}/TDD/numbers.txt  ${PH_Number}${\n}
     Set Suite Variable  ${PH_Number}
     ${id}=  get_id  ${PH_Number}
     ...
     ${PUSERNAME_U1}=    Evaluate    f'{${PH_Number}:0>7d}'
     Log  ${PUSERNAME_U1}
     ...
     ${PH_Number}    Random Number 	       digits=5 
     ${PUSERNAME_U2}=    Evaluate    f'{${PH_Number}:0>7d}'
     Log  ${PUSERNAME_U2}
     ...
     FOR  ${i}  IN RANGE   ${len}
        ...
        ...    AND  Should Be Equal As Strings  ${resp.json()[${i}]['userType']}                        ${userType[0]}     
        ...    AND  Should Be Equal As Strings  ${resp.json()[${i}]['status']}                          ACTIVE    
        ...    AND  Should Be Equal As Strings  ${resp.json()[${i}]['mobileNo']}                        ${PH_Number}
        ...    AND  Should Be Equal As Strings  ${resp.json()[${i}]['deptId']}                          ${dep_id}    
