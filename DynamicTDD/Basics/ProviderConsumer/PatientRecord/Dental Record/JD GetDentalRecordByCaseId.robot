...
JD-TC-Update Dental Record-1
    ...
    ${resp}=    Get DentalRecord ByCaseId   ${caseUId}    
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    Should Be Equal As Strings    ${resp.json()[0]['provider']['firstName']}     ${pdrfname} 
    Should Be Equal As Strings    ${resp.json()[0]['provider']['lastName']}     ${pdrlname} 

    Should Be Equal As Strings    ${resp.json()[0]['investigation'][0]}     ${note1} 
    Should Be Equal As Strings    ${resp.json()[0]['toothSurfaces'][0]}     ${toothSurfaces[0]} 
