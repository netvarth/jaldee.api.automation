...
JD-TC-UserPerformanceReport-1
    ...
    Set Test Variable  ${sub_domain_id}  ${resp.content['serviceSubSector']['id']}
    ...
    IF  ${resp.content['enableJaldeeFinance']}==${bool[0]}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.content['enableJaldeeFinance']}  ${bool[1]}
    ...
    IF   '${resp.content}' == '${emptylist}'
        ...
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        Set Test Variable  ${tz}  ${resp.content['bSchedule']['timespec'][0]['timezone']}

    ELSE
        Set Test Variable  ${locId}  ${resp.content[0]['id']}
        Set Test Variable  ${tz}  ${resp.content[0]['timezone']}
    END
    ...
    ELSE
        Set Test Variable  ${dep_id}  ${resp.content['departments'][0]['departmentId']}
    END
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${BUSER_U1}  ${resp.content['mobileNo']}
    Set Test Variable  ${userf_name}  ${resp.content['firstName']}
    Set Test Variable  ${userl_name}  ${resp.content['lastName']}
    ...
    Log   ${resp.content}  
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${ser_dur}      ${resp.content['serviceDuration']}
    Set Test Variable   ${ser_amount}   ${resp.content['totalAmount']}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${sch_id}  ${resp.content}
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Suite Variable  ${token}  ${resp.content['token']}
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.content['providerConsumer']}
    ...
    Should Not Contain   ${resp.content['subServiceData']}                               ${subser_id1}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${token_id1}   ${resp.content}
  
    ${resp}=  Get Report Status By Token Id  ${token_id1}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${tot_amt}   ${resp.content['reportContent']['dataHeader']['Grand Total']}
    ...
    Should Be Equal As Strings  ${resp.content['status']}                                        ${Report_Status[0]}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['5']}                 ${SERVICE1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['6']}                 1
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['7']}                 ${ser_amount}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['5']}                 ${subser_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['7']}                 ${total}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        2
    Should Be Equal As Strings  ${tot_amt}                                                      ${total_amt}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportName']}                   User Performance Report
    Should Be Equal As Strings  ${resp.content['reportType']}                                    ${reportType[6]}
    Should Be Equal As Strings  ${resp.content['reportResponseType']}                            ${ReportResponseType[0]}
    Should Be Equal As Strings  ${resp.content['reportTokenID']}                                 ${token_id1}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportHeader']['Time Period']}  ${Report_Date_filter[4]}  

JD-TC-UserPerformanceReport-2

    [Documentation]   Create a service with auto invoice generation on and with price, then take appointment(walk in). 
    ...
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable    ${invoice_id1}    ${resp.content[0]['invoiceUid']}
    Should Not Contain   ${resp.content[0]['serviceList']}                               ${subser_id1}
    ...
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.content[0]['serviceList'][0]['serviceId']}                      ${s_id}   
    Should Be Equal As Strings  ${resp.content[0]['serviceList'][1]['serviceId']}                      ${subser_id1}
...
JD-TC-UserPerformanceReport-3
    ...
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${no_of_slots}=  Get Length  ${resp.content['availableSlots']}
    @{slots}=  Create List
    FOR   ${i}  IN RANGE   0   ${no_of_slots}
        IF  ${resp.content['availableSlots'][${i}]['noOfAvailbleSlots']} > 0   
            Append To List   ${slots}  ${resp.content['availableSlots'][${i}]['time']}
        END
    END
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['5']}                 ${SERVICE1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['6']}                 1
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['7']}                 ${ser_amount}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['1']}                 ${ser_date}
    ...
    Should Be Equal As Strings  ${resp.content['reportResponseType']}                            ${ReportResponseType[0]}
    Should Be Equal As Strings  ${resp.content['reportTokenID']}                                 ${token_id1}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportHeader']['Time Period']}  ${Report_Date_Category[4]}   ignore_case=True
...
JD-TC-UserPerformanceReport-4
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable  ${fid}  ${resp.content}
    ...
    # Log   ${resp.content}
    # Should Be Equal As Strings    ${resp.status_code}    200
    # Set Test Variable     ${fid}    ${resp.content[0]['id']}
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['3']}                 ${fname} ${lname}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['5']}                 ${SERVICE1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['6']}                 1
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['7']}                 ${ser_amount}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['3']}                 ${fname} ${lname}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['5']}                 ${subser_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['7']}                 ${total}
...
JD-TC-UserPerformanceReport-5
    ...
    Log   ${resp.content}  
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${subser_id3}  ${resp.content}
    ...
    Should Be Equal As Strings  ${resp.content['subServiceData'][0]['serviceId']}        ${s_id}
    Should Be Equal As Strings  ${resp.content['subServiceData'][1]['serviceId']}        ${subser_id1}
    Should Be Equal As Strings  ${resp.content['subServiceData'][2]['serviceId']}        ${subser_id2}
    Should Be Equal As Strings  ${resp.content['subServiceData'][3]['serviceId']}        ${subser_id3}
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['7']}                 ${total}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['5']}                 ${subser_name1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['7']}                 ${total1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['5']}                 ${subser_name2}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['7']}                 ${total2}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        4
    Should Be Equal As Strings  ${tot_amt}                                                      ${total_amt}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportName']}                   User Performance Report
    Should Be Equal As Strings  ${resp.content['reportType']}                                    ${reportType[6]}
    Should Be Equal As Strings  ${resp.content['reportResponseType']}                            ${ReportResponseType[0]}
...
JD-TC-UserPerformanceReport-6
    ...
    Should Be Equal As Strings  ${resp.content['subServiceData'][2]['serviceId']}        ${subser_id2}
    Should Be Equal As Strings  ${resp.content['subServiceData'][3]['serviceId']}        ${subser_id3}
    Set Test Variable   ${seq_id1}      ${resp.content['subServiceData'][1]['sequenceId']}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${token_id2}   ${resp.content}

    ${resp}=  Get Report Status By Token Id  ${token_id2}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${tot_amt1}   ${resp.content['reportContent']['dataHeader']['Grand Total']}
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['5']}                 ${subser_name1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][1]['7']}                 ${total1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['5']}                 ${subser_name2}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['7']}                 ${total2}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['5']}                 ${subser_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][3]['7']}                 ${total11}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        4
    Should Be Equal As Strings  ${tot_amt1}                                                      ${total_amt1}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportName']}                   User Performance Report
    Should Be Equal As Strings  ${resp.content['reportType']}                                    ${reportType[6]}
    Should Be Equal As Strings  ${resp.content['reportResponseType']}                            ${ReportResponseType[0]}
    Should Be Equal As Strings  ${resp.content['reportTokenID']}                                 ${token_id2}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportHeader']['Time Period']}  ${Report_Date_Category[4]}   ignore_case=True
...
JD-TC-UserPerformanceReport-7
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][2]['7']}                 ${total2}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        3
    Should Be Equal As Strings  ${tot_amt1}                                                      ${total_amt1}
    Should Be Equal As Strings  ${resp.content['reportContent']['reportName']}                   User Performance Report
    Should Be Equal As Strings  ${resp.content['reportType']}                                    ${reportType[6]}
    Should Be Equal As Strings  ${resp.content['reportResponseType']}                            ${ReportResponseType[0]}
...
JD-TC-UserPerformanceReport-8
    ...
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${encId2}=  Set Variable   ${resp.content}
    ...
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${encId1}=  Set Variable   ${resp.content}
    ...
    ${len}=  Get Length  ${resp.content['reportContent']['data']}

    FOR  ${i}  IN RANGE   ${len}

        IF  '${resp.content['reportContent']['data'][${i}]['2']}' == '${encId1}'  
            Should Be Equal As Strings  ${resp.content['status']}                                    ${Report_Status[0]}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['1']}          ${ser_date}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['3']}          ${custf_name} ${custl_name}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['4']}          ${userf_name} ${userl_name}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['5']}          ${subser_name}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['6']}          ${subser_qnty}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['7']}          ${total}

        ELSE IF     '${resp.content['reportContent']['data'][${i}]['2']}' == '${encId2}'            
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['1']}          ${ser_date}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['3']}          ${custf_name} ${custl_name}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['4']}          ${userf_name} ${userl_name}
            Should Be Equal As Strings  ${resp.content['reportContent']['data'][${i}]['5']}          ${subser_name}
    ...
    Should Be Equal As Strings  ${resp.content['status']}                                        ${Report_Status[0]}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['2']}                 ${encId1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['5']}                 ${SERVICE1}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['6']}                 1
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['7']}                 ${ser_amount}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        1
...
JD-TC-UserPerformanceReport-9
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['4']}                 ${userf_name} ${userl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['5']}                 ${subser_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['7']}                 ${total}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['1']}                 ${ser_date}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        1
...
JD-TC-UserPerformanceReport-10
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${BUSER_U2}  ${resp.content['mobileNo']}
    Set Test Variable  ${userf_name2}  ${resp.content['firstName']}
    Set Test Variable  ${userl_name2}  ${resp.content['lastName']}
    ...
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['2']}                 ${encId}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['3']}                 ${custf_name} ${custl_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['4']}                 ${userf_name} ${userl_name}, ${userf_name2} ${userl_name2}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['5']}                 ${subser_name}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['6']}                 ${subser_qnty}
    Should Be Equal As Strings  ${resp.content['reportContent']['data'][0]['7']}                 ${total}
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        1
...
JD-TC-UserPerformanceReport-11
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${token_id3}   ${resp.content}
  
    ${resp}=  Get Report Status By Token Id  ${token_id3}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.content['reportContent']['count']}                        0
...
JD-TC-UserPerformanceReport-UH1
    ...
    ...   ${bool[0]}   dateTime=${bool[1]}  serviceBookingType=${serviceBookingType[1]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${s_id}  ${resp.content}
    ...
    Should Be Equal As Strings    ${resp.content}    ${NO_INVOICE_GENERATED}

    ${resp}=   Get next invoice Id   ${locId}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${invoiceId}   ${resp.content}

    ${name}=   FakerLibrary.word
    ${resp}=  Create Category   ${name}  ${categoryType[3]} 
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable   ${category_id1}   ${resp.content}
