...
*** Test Cases ***
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-1
    [Documentation]  Get Teleservice meeting request for waitlist in WhatsApp (WALK-IN CHECKIN)
    ...
    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+10501
    ...
    ${VirtualcallingMode2}=   Create Dictionary   callingMode=${CallingModes[1]}   countryCode=${countryCodes[0]}  value=${PUSERPH0}   status=ACTIVE    instructions=${instructions2} 
    ...
    ${resp}=  Create virtual Service  ${SERVICE1}   ${desc1}   5   ${status[0]}   ${btype}    ${bool[1]}    ${notifytype[2]}  ${EMPTY}  ${Total1}  ${bool[0]}   ${bool[0]}   ${vstype}   ${virtualCallingModes1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    Should Be Equal As Strings  ${resp.status_code}  200
    Log  ${resp.json()}
    Verify Response  ${resp}  name=${SERVICE1}  description=${desc1}  serviceDuration=5   notification=${bool[1]}   notificationType=${notifytype[2]}   totalAmount=${Total1}  status=${status[0]}  bType=${btype}  isPrePayment=${bool[0]}  serviceType=virtualService   virtualServiceType=${vstype}
    ...
    ${resp}=  Create virtual Service  ${SERVICE2}   ${desc2}   5   ${status[0]}   ${btype}    ${bool[1]}    ${notifytype[2]}  ${EMPTY}  ${Total2}  ${bool[0]}   ${bool[0]}   ${vstype2}   ${virtualCallingModes2}
    ...
    Should Be Equal As Strings  ${resp.status_code}  200
    Log  ${resp.json()}
    Verify Response  ${resp}  name=${SERVICE2}  description=${desc2}  serviceDuration=5   notification=${bool[1]}   notificationType=${notifytype[2]}   totalAmount=${Total2}  status=${status[0]}  bType=${btype}  isPrePayment=${bool[0]}  serviceType=virtualService   virtualServiceType=${vstype2}
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${pc_id}  ${resp.json()}
    ...
    ${resp}=  Provider Add To WL With Virtual Service  ${pcid}  ${p1_s1}  ${queueId}  ${DAY}  ${desc}  ${bool[1]}  ${waitlistMode[2]}  ${virtualService}   ${pcid}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Should Be Equal As Strings  ${resp.json()['waitlistingFor'][0]['id']}         ${pcid}
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-2
    [Documentation]  Get Teleservice meeting request for waitlist in Zoom (WALK-IN CHECKIN)
    ...
    ${resp}=  Provider Add To WL With Virtual Service  ${pcid}  ${p1_s2}  ${queueId}  ${DAY}  ${desc1}  ${bool[1]}  ${waitlistMode[2]}  ${virtualService}   ${pcid}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${wid}=  Get Dictionary Values  ${resp.json()}
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-UH1
    [Documentation]  Get Teleservice meeting request for waitlist in Zoom,WhatsApp,phone and Googlemeet (WALK-IN CHECKIN)
    ...
    Set Suite Variable  ${desc}
    ${virtualService}=  Create Dictionary   ${CallingModes[1]}=${WHATSAPP_id2}
    ${resp}=  Provider Add To WL With Virtual Service  ${pcid}  ${p1_s1}  ${queueId}  ${DAY}  ${desc1}  ${bool[1]}  ${waitlistMode[2]}  ${virtualService}   ${pcid}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${wid}=  Get Dictionary Values  ${resp.json()}
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-3
    [Documentation]  Get Teleservice meeting request for waitlist in WhatsApp (ONLINE CHECKIN)
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-4
    [Documentation]  Get Teleservice meeting request for waitlist in Zoom (ONLINE CHECKIN)
    ...
    Should Be Equal As Strings  ${resp.json()['service']['id']}  ${p1_s2}
    Should Be Equal As Strings  ${resp.json()['jaldeeConsumer']['id']}  ${cid}
    Should Be Equal As Strings  ${resp.json()['waitlistingFor'][0]['id']}  ${pcid2}
    Should Be Equal As Strings  ${resp.json()['queue']['id']}  ${queueId}
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-UH2
    [Documentation]  Get Teleservice meeting request for waitlist  in Zoom and WhatsApp (ONLINE CHECKIN)
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-5

    [Documentation]   Get waitlist teleservice Zoom meeting request Which  is already created
    ${resp}=   Consumer Login  ${CUSERNAME0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${cid}=  get_id  ${CUSERNAME0}    
...
JD-TC-GetTeleserviceWaitlist-(Billable Subdomain)-6

    [Documentation]   Get waitlist teleservice Whatsapp meeting request Which  is already created
    ${resp}=   Consumer Login  ${CUSERNAME0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${cid}=  get_id  ${CUSERNAME0}    
...
JD-TC-GetTeleserviceWaitlist-UH3
    [Documentation]  Get waitlist teleservice meeting request without login
    ${resp}=  Get Waitlist Meeting Request   ${wid6}   ${CallingModes[0]} 
...
JD-TC-GetTeleserviceWaitlist-UH4
...
JD-TC-GetTeleserviceWaitlist-UH5
    ...
    ${resp}=  Get Waitlist Meeting Request   ${INVALID_Wid}   ${CallingModes[0]} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}   422
...
#     ${resp}=  Get Waitlist Meeting Request   ${wid6}   ${CallingModes[5]} 
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  422
#     Should Be Equal As Strings  "${resp.json()}"   "${VIRTUAL_CALLING_MODES_INVALID}"
...
JD-TC-GetTeleserviceWaitlist-UH6
    [Documentation]    Get waitlist teleservice meeting request  for a cancelled waitlist 
    ${resp}=  Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${DAY}=  db.get_date_by_timezone  ${tz}
...
JD-TC-GetTeleserviceWaitlist-(Non billable Subdomain)-7
    ...
    ${VirtualcallingMode2}=   Create Dictionary   callingMode=${CallingModes[1]}   countryCode=${countryCodes[0]}  value=${PUSERPH2}   status=ACTIVE    instructions=${instructions2} 
    ${vcm1}=  Create List  ${VirtualcallingMode1}   ${VirtualcallingMode2}
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${pc_id0}  ${resp.json()}
...
JD-TC-GetTeleserviceWaitlist-(Non billable Subdomain)-UH7
    [Documentation]  Create Teleservice meeting request for waitlist  in Zoom,WhatsApp,phone and Googlemeet (Non billable Subdomain)
