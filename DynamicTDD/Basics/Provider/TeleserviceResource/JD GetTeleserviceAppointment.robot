...
*** Test Cases ***
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-1
    [Documentation]  Get Teleservice meeting request for Appointment in WhatsApp (ONLINE CHECKIN)
    ...
    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+989898
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-2
    [Documentation]  Get Teleservice meeting request for Appointment in Zoom (ONLINE CHECKIN)
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-UH1
    [Documentation]  Get Teleservice meeting request for Appointment  in Zoom and WhatsApp (ONLINE CHECKIN)
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-3
    [Documentation]  Get Teleservice meeting request for Appointment in Zoom (WALK-IN CHECKIN)
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    # Set Suite Variable  ${pcid6}  ${resp.json()[0]['id']}
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-4
    [Documentation]  Get Teleservice meeting request for Appointment in WhatsApp (WALK-IN CHECKIN)
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-UH2
    [Documentation]  Get Teleservice meeting request for Appointment in Zoom and WhatsApp (WALK-IN CHECKIN)
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-5

    [Documentation]   Get Appointment teleservice Zoom meeting request Which  is already created
    ${resp}=  Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetTeleserviceAppointment-(Billable Subdomain)-6

    [Documentation]   Get Appointment teleservice Whatsapp meeting request Which  is already created
...
JD-TC-GetTeleserviceAppointment-UH3
    [Documentation]  Get Appointment teleservice meeting request without login
    ${resp}=  Get Appointment Meeting Request   ${apptid8}   ${CallingModes[1]} 
    Log  ${resp.json()}
...
JD-TC-GetTeleserviceAppointment-UH4
    [Documentation]  Consumer try to Get Appointment teleservice meeting request 
...
JD-TC-GetTeleserviceAppointment-UH5
    [Documentation]    Get Appointment teleservice meeting request  with invalid  Appointment id 
    ${resp}=  Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    Set Test Variable   ${INVALID_Wid}   0000
    ${resp}=  Get Appointment Meeting Request   ${INVALID_Wid}   ${CallingModes[1]}  
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
...
# JD-TC-GetTeleserviceAppointment-UH4
#     [Documentation]    Get Appointment teleservice meeting request  with invalid  Calling mode 
#     ${resp}=  Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200

#     ${resp}=  Get Appointment Meeting Request   ${apptid8}   ${CallingModes[5]} 
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  422
#     Should Be Equal As Strings  "${resp.json()}"   "${VIRTUAL_CALLING_MODES_INVALID}"


JD-TC-GetTeleserviceAppointment-UH6
    [Documentation]    Get Appointment teleservice meeting request  for a cancelled Appointment 
    ${resp}=  Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetTeleserviceAppointment-(Non billable Subdomain)-7
    [Documentation]  Get Teleservice meeting request for Appointment  in Zoom (Non billable Subdomain)
    ${resp}=  Consumer Login  ${CUSERNAME0}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    ...
    ${PUSERPH2}=  Evaluate  ${PUSERNAME}+90909
    ...
    Should Be Equal As Strings  ${resp.status_code}  200
    Log  ${resp.json()}
    Verify Response  ${resp}  name=${SERVICE2}  description=${description2}  serviceDuration=5   notification=${bool[1]}   notificationType=${notifytype[2]}   totalAmount=0.0  status=${status[0]}  bType=${btype}  isPrePayment=${bool[0]}  serviceType=virtualService   virtualServiceType=${vstype}
    ...
    ${DAY1}=  db.get_date_by_timezone  ${tz}
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Virtual Service Appointment For Consumer  ${pcid0}  ${p2_s1}  ${sch_id2}  ${DAY1}  ${cnote}  ${CallingModes[0]}  ${ZOOM_id2}  ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${msg}=   FakerLibrary.word
    Append To File  ${EXECDIR}/TDD/TDD_Logs/msgslog.txt  ${SUITE NAME} - ${TEST NAME} - ${msg}${\n}
    ${resp}=    Provider Cancel Appointment  ${apptid9}  ${reason}  ${msg}  ${DAY1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetTeleserviceAppointment-(Non billable Subdomain)-UH7
    [Documentation]  Get Teleservice meeting request for Appointment  in Zoom and WhatsApp (Non billable Subdomain)
    ...
    Should Be Equal As Strings  ${resp.json()['consumer']['userProfile']['firstName']}   ${f_Name0}
    Should Be Equal As Strings  ${resp.json()['consumer']['userProfile']['lastName']}   ${l_Name0}
    Should Be Equal As Strings  ${resp.json()['service']['id']}   ${p2_s0}
    Should Be Equal As Strings  ${resp.json()['schedule']['id']}   ${sch_id2}
    Should Be Equal As Strings  ${resp.json()['appmtFor'][0]['firstName']}   ${EMPTY}
    Should Be Equal As Strings  ${resp.json()['appmtFor'][0]['lastName']}   ${EMPTY}
    Should Be Equal As Strings  ${resp.json()['appmtFor'][0]['apptTime']}   ${slot1}
