...
JD-TC-Delete Work in Treatment Plan-1

    [Documentation]    Delete Work in Treatment Plan using work id.
    ...
    Set Suite Variable       ${work_id}         ${resp.json()['works'][0]['id']}  

    ${resp}=    Delete Treatment Plan Work By id    ${work_id}    
    Log   ${resp.json()}
    Should Be Equal As Strings              ${resp.status_code}   200
    ...
    Should Be Equal As Strings    ${resp.json()['caseDto']['createdDate']}     ${DAY1}
    Should Be Equal As Strings    ${resp.json()['treatment']}     ${treatment}
    Should Be Equal As Strings    ${resp.json()['works']}     []
