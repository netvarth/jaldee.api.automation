...
JD-TC-Get_Pie_Chart-1

    [Documentation]   Get Pie Chart
    ...
    ${resp}=    Create Lead Consumer  ${firstName_n}  ${lastName_n}  phone=${CUSERNAME23}  countryCode=${countryCodes[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=    Get Pie Chart  ${ivr_category[2]}  ${DAY1}  ${DAY1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}                  200
    Should Be Equal As Strings  ${resp.json()['data'][0]['value']}  ${ChannelName1}