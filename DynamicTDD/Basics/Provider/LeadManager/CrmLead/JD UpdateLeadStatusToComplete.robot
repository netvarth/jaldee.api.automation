...
JD-TC-Update_Lead_Status_To_Complete-1

    [Documentation]   Update Lead Status To Complete
...
JD-TC-Update_Lead_Status_To_Complete-2

    [Documentation]   Update Lead Status To Complete - completed status to complete
...
JD-TC-Update_Lead_Status_To_Complete-3

    [Documentation]   Update Lead Status To Complete - rejected status to complete

    ${resp}=  Encrypted Provider Login  ${PUSERNAME100}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${firstName_n1}=   FakerLibrary.firstName
    ${lastName_n1}=    FakerLibrary.lastName
    
    ${resp}=    Create Lead Consumer  ${firstName_n1}  ${lastName_n1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=    Create Crm Lead  ${clid}  ${firstName_n1}  ${con_id1}  ${lastName_n1}  ${pid}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200
    ...
    ${resp}=    Crm Lead Status Change To Rejected  ${crm_lead_id1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200
    ...
    ${resp}=    Crm Lead Status Change To Complete  ${crm_lead_id1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200

JD-TC-Update_Lead_Status_To_Complete-3

    [Documentation]   Update Lead Status To Complete - active status to complete
    ...
    ${resp}=    Crm Lead Status Change To Active  ${crm_lead_id1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200
...
JD-TC-Update_Lead_Status_To_Complete-4

    [Documentation]   Update Lead Status To Complete - without login

    ${resp}=    Crm Lead Status Change To Complete  ${crm_lead_id}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200

JD-TC-Update_Lead_Status_To_Complete-5

    [Documentation]   Update Lead Status To Complete - lead id is invalid
    ...
    ${resp}=    Crm Lead Status Change To Complete  ${inv}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200
