...
JD-TC-Get_Stage_Count_By_Filter-1

    [Documentation]   Get Stage Count By Filter
    ...
    ${resp}=    Create Lead Consumer  ${firstName_n}  ${lastName_n}  phone=${CUSERNAME13}  countryCode=${countryCodes[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=    Get Stage Count By Filter
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200