...
JD-TC-Get_All_Member_QNR-1

    [Documentation]  Get All Member QNR
    ...
    ${accountId}=    get_acc_id       ${PUSERNAME46}
   ...
   FOR  ${i}  IN RANGE   1  ${j}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()[0]['questionnaireId']}    ${qid}

JD-TC-Get_All_Member_QNR-UH1

    [Documentation]  Get All Member QNR where member id is empty

    ${resp}=    ProviderConsumer Login with token   ${number1}    ${accountId}  ${token} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200 

    ${resp}=    get all Member questionnaire    ${accountId}    ${empty}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404

JD-TC-Get_All_Member_QNR-UH2

    [Documentation]  Get All Member QNR where member id is invalid

    ${resp}=    ProviderConsumer Login with token   ${number1}    ${accountId}  ${token} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200 

    ${fakenum}=  Generate random string    4    0123456789

    ${resp}=    get all Member questionnaire    ${accountId}    ${fakenum}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
    Should Be Equal As Strings  ${resp.json()}    ${MEMBER_NOT_EXISTS}

JD-TC-Get_All_Member_QNR-UH3

    [Documentation]  Get All Member QNR with provider login
...
JD-TC-Get_All_Member_QNR-UH4

    [Documentation]  Get All Member QNR with anthoer consumer login
...
JD-TC-Get_All_Member_QNR-UH5

    [Documentation]  Get All Member QNR without login
