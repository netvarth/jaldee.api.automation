...
JD-TC-Resubmit_Member_QNR-1

    [Documentation]  Resubmit Member QNR
    ...
    ${accountId}=    get_acc_id       ${PUSERNAME61}
    ...
    ${resp}=    Resubmit Member Questionnaire    ${accountId}    ${memberid1}    ${data}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Resubmit_Member_QNR-UH1

    [Documentation]  Resubmit Member QNR where memberid is invalid
    ...
    ${resp}=    Resubmit Member Questionnaire    ${accountId}    ${fake}    ${data}
    Log  ${resp.content}
