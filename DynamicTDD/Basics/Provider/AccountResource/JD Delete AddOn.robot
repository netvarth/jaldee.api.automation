...
# --------------------------  Multi User - 50 Count ---------------------
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable    ${addon_id1}      ${resp.json()[6]['addons'][0]['addonId']}
    Set Suite Variable    ${addon_name1}      ${resp.json()[6]['addons'][0]['addonName']}
    Log   ${addon_id1}
...
JD-TC-Delete addon -2
    [Documentation]   Add another Addon and check its status(not_used or Not).
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable    ${addon_id}      ${resp.json()[6]['addons'][1]['addonId']}
    Set Suite Variable    ${addon_name}      ${resp.json()[6]['addons'][1]['addonName']}
    Log   ${addon_id}
