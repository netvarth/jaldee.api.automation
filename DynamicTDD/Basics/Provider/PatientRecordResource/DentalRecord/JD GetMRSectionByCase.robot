...
JD-TC-Get MR Section By Case-1

    [Documentation]    Create Sections with valid details then get that by case.
...
JD-TC-Get MR Section By Case-UH1

    [Documentation]     get MR Section by case using another provider login.
...
JD-TC-Get MR Section By Case-UH2

    [Documentation]     get MR Section by case using without login.
...
JD-TC-Get MR Section By Case-UH3

    [Documentation]     get MR Section by case where id is invalid.
    ...
    ${resp}    Get MR Sections By Case     ${fake_id}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings   ${resp.json()}   ${INVALID_CASE_ID}

JD-TC-Get MR Section By Case-UH4

    [Documentation]     get MR Section by case using provider consumer login
