...
JD-TC-GetDonations-1
        ...
        ${resp}=  Provider Login  ${PUSERNAME54}  ${PASSWORD}
        Log  ${resp.json()}
        ...
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
        Verify Response  ${resp}    uid=${don_id1}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[0]}  donationAmount=${don_amt1}
        ...
        ${resp}=  Get Payment By UUId  ${don_id1}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
        Verify Response  ${resp}    uid=${don_id2}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[0]}  donationAmount=${don_amt2}
        Should Be Equal As Strings  ${resp.json()['consumer']['id']}  ${con_id}
        ...
        ${resp}=  Get Payment By UUId  ${don_id2}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200
        Set Test Variable  ${pay_id}  ${resp.json()[0]['id']}
...
JD-TC-GetDonations-2
        ...
        ${resp}=  Provider Login  ${PUSERNAME54}  ${PASSWORD}
        Should Be Equal As Strings    ${resp.status_code}    200
        ${resp}=  Get Donations  date-eq=${CUR_DAY}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
        Verify Response List  ${resp}  1   uid=${don_id1}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt1}
        Verify Response List  ${resp}  0   uid=${don_id2}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt2}
...
JD-TC-GetDonations-3
        ...
        ${resp}=  Provider Login  ${PUSERNAME54}  ${PASSWORD}
        Should Be Equal As Strings    ${resp.status_code}    200
        ${resp}=  Get Donations  service-eq=${sid2}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
        Verify Response List  ${resp}  0    uid=${don_id2}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt2}
        Should Be Equal As Strings  ${resp.json()[0]['consumer']['id']}  ${con_id}
...
JD-TC-GetDonations-4
        ...
        ${resp}=  Provider Login  ${PUSERNAME54}  ${PASSWORD}
        Should Be Equal As Strings    ${resp.status_code}    200
        ${resp}=  Get Donations  location-eq=${loc_id1}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
        Verify Response List  ${resp}  1   uid=${don_id1}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt1}
        Verify Response List  ${resp}  0   uid=${don_id2}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt2}
...
JD-TC-GetDonations-5
        ...
        ${resp}=  Provider Login  ${PUSERNAME54}  ${PASSWORD}
        Should Be Equal As Strings    ${resp.status_code}    200
        ${resp}=  Get Donations  donationAmount-eq=${don_amt1}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-GetDonations-6
        ...
        ${resp}=  Provider Login  ${PUSERNAME54}  ${PASSWORD}
        Should Be Equal As Strings    ${resp.status_code}    200
        ${resp}=  Get Donations  billPaymentStatus-eq=${paymentStatus[2]}
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200 
        Verify Response List  ${resp}  1   uid=${don_id1}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt1}
        Verify Response List  ${resp}  0   uid=${don_id2}   date=${CUR_DAY}  billPaymentStatus=${paymentStatus[2]}  donationAmount=${don_amt2}
