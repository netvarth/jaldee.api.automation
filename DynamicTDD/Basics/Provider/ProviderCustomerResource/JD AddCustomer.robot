...
JD-TC-AddCustomer-18
     ...
     [Documentation]  Add a new valid customer with age, telegram number, whats app number ( dob is given so birth year and month will calculate by it )
     ...
     Log  ${resp.content}
     Should Be Equal As Strings  ${resp.status_code}  200
     ${firstname18}=  FakerLibrary.first_name
     ${lastname18}=  FakerLibrary.last_name
     ${cust18}=  Evaluate  ${PUSERNAME23}+73782
     ${cust182}=  Evaluate  ${PUSERNAME23}+73445
     ${dob18}=  FakerLibrary.Date
     ${gender18}=  Random Element    ${Genderlist}
     ${address18}=  FakerLibrary.address
     ${year}=   Random Int   min=1   max=150
     ${month}=  Random Int   min=1   max=11
     Set Suite Variable  ${email18}  ${firstname18}${cust18}${C_Email}.${test_mail}
     ${telegramNum}=     Create Dictionary  countryCode=${countryCodes[0]}  number=${cust18}
     ${whatsAppNum}=     Create Dictionary  countryCode=${countryCodes[0]}  number=${cust182}
     ${age}=   Create Dictionary   year=${year}   month=${month}

     ${resp}=  AddCustomer    ${cust18}   countryCode=${countryCodes[0]}  firstName=${firstname18}   lastName=${lastname18}  address=${address18}   gender=${gender18}  dob=${dob18}  secondaryCountryCode=${countryCodes[0]}  secondaryPhoneNo=${cust182}   email=${email18}   telegramNum=${telegramNum}   whatsAppNum=${whatsAppNum}   age=${age}
     Log  ${resp.content}
     Should Be Equal As Strings  ${resp.status_code}  200

     ${yearofdob}  ${monthofdob}=  db.calculate_age_years_months     ${dob18}
     Log  ${yearofdob}
     Log  ${monthofdob}
     

     ${resp}=  GetCustomer    phoneNo-eq=${cust18}
     Log  ${resp.content}
     Should Be Equal As Strings    ${resp.status_code}  200
     Should Be Equal As Strings    ${resp.json()[0]['firstName']}                   ${firstname18}
     Should Be Equal As Strings    ${resp.json()[0]['lastName']}                    ${lastname18}
     Should Be Equal As Strings    ${resp.json()[0]['address']}                     ${address18}
     Should Be Equal As Strings    ${resp.json()[0]['email']}                       ${email18}
     Should Be Equal As Strings    ${resp.json()[0]['gender']}                      ${gender18}
     Should Be Equal As Strings    ${resp.json()[0]['dob']}                         ${dob18}
     Should Be Equal As Strings    ${resp.json()[0]['phoneNo']}                     ${cust18}
     Should Be Equal As Strings    ${resp.json()[0]['countryCode']}                 ${countryCodes[0]} 
     Should Be Equal As Strings    ${resp.json()[0]['internationalConsumer']}       ${bool[0]}
     Should Be Equal As Strings    ${resp.json()[0]['whatsAppNum']['countryCode']}  ${countryCodes[0]}
     Should Be Equal As Strings    ${resp.json()[0]['whatsAppNum']['number']}       ${cust182}
     Should Be Equal As Strings    ${resp.json()[0]['telegramNum']['countryCode']}  ${countryCodes[0]}
     Should Be Equal As Strings    ${resp.json()[0]['telegramNum']['number']}       ${cust18}
     Should Be Equal As Strings    ${resp.json()[0]['age']['year']}                 ${yearofdob}
     Should Be Equal As Strings    ${resp.json()[0]['age']['month']}                ${monthofdob}


JD-TC-AddCustomer-19

     [Documentation]  Add a new valid customer where age year is above 150
     ...
     ${firstname18}=  FakerLibrary.first_name
     ${lastname18}=  FakerLibrary.last_name
     ${cust18}=  Evaluate  ${PUSERNAME23}+73685
     ${cust182}=  Evaluate  ${PUSERNAME23}+73787
     ${dob18}=  FakerLibrary.Date
     ${gender18}=  Random Element    ${Genderlist}
     ${address18}=  FakerLibrary.address
     ${year}=   Random Int   min=300   max=900
     ${month}=  Random Int   min=1   max=12
     Set Suite Variable  ${email18}  ${firstname18}${cust18}${C_Email}.${test_mail}
     ${age}=   Create Dictionary   year=${year}   month=${month}

     ${resp}=  AddCustomer    ${cust18}   countryCode=${countryCodes[0]}  firstName=${firstname18}   lastName=${lastname18}  address=${address18}   gender=${gender18}  dob=${dob18}  secondaryCountryCode=${countryCodes[0]}  secondaryPhoneNo=${cust182}   email=${email18}   age=${age}
     Log  ${resp.content}
     Should Be Equal As Strings  ${resp.status_code}  422
     Should Be Equal As Strings  ${resp.json()}    ${AGE_YEAR_INVALID}

JD-TC-AddCustomer-20

     [Documentation]  Add a new valid customer where age month is above 12
     ...
     ${firstname18}=  FakerLibrary.first_name
     ${lastname18}=  FakerLibrary.last_name
     ${cust18}=  Evaluate  ${PUSERNAME23}+73678
     ${cust182}=  Evaluate  ${PUSERNAME23}+73248
     ${dob18}=  FakerLibrary.Date
     ${gender18}=  Random Element    ${Genderlist}
     ${address18}=  FakerLibrary.address
     ${year}=   Random Int   min=1  max=150
     ${month}=  Random Int   min=13   max=50
     Set Suite Variable  ${email18}  ${firstname18}${cust18}${C_Email}.${test_mail}
     ${age}=   Create Dictionary   year=${year}   month=${month}
     ...
     Log  ${resp.content}
     Should Be Equal As Strings  ${resp.status_code}  422
     Should Be Equal As Strings  ${resp.json()}    ${AGE_MONTH_INVALID}


JD-TC-AddCustomer-21

     [Documentation]  Add a new valid customer where telegram numner is 3 digit
     ...
     ${firstname18}=  FakerLibrary.first_name
     ${lastname18}=  FakerLibrary.last_name
     ${cust18}=  Evaluate  ${PUSERNAME23}+73458
     ${cust182}=  Evaluate  ${PUSERNAME23}+73867
     ${dob18}=  FakerLibrary.Date
     ${gender18}=  Random Element    ${Genderlist}
     ${address18}=  FakerLibrary.address
     ${year}=   Random Int   min=1  max=150
     ...
     ${telegramNum}=     Create Dictionary  countryCode=${countryCodes[0]}  number=${inv}
     ${resp}=  AddCustomer    ${cust18}   countryCode=${countryCodes[0]}  firstName=${firstname18}   lastName=${lastname18}  address=${address18}   gender=${gender18}  dob=${dob18}  secondaryCountryCode=${countryCodes[0]}  secondaryPhoneNo=${cust182}   email=${email18}   age=${age}  telegramNum=${telegramNum}
     Log  ${resp.content}
     Should Be Equal As Strings  ${resp.status_code}  422
...
JD-TC-AddCustomer-22
     ...
     [Documentation]  Add a new valid customer where whats app numner is 3 digit
     ...
     ${firstname18}=  FakerLibrary.first_name
     ${lastname18}=  FakerLibrary.last_name
     ${cust18}=  Evaluate  ${PUSERNAME23}+73245
     ${cust182}=  Evaluate  ${PUSERNAME23}+73478
     ${dob18}=  FakerLibrary.Date
     ${gender18}=  Random Element    ${Genderlist}
     ${address18}=  FakerLibrary.address
     ${year}=   Random Int   min=1  max=150
     ...
     ${age}=   Create Dictionary   year=${year}   month=${month}
     ${inv}=   Random Int  min=111  max=999
     ${whatsAppNum}=     Create Dictionary  countryCode=${countryCodes[0]}  number=${inv}
     ${resp}=  AddCustomer    ${cust18}   countryCode=${countryCodes[0]}  firstName=${firstname18}   lastName=${lastname18}  address=${address18}   gender=${gender18}  dob=${dob18}  secondaryCountryCode=${countryCodes[0]}  secondaryPhoneNo=${cust182}   email=${email18}   age=${age}  whatsAppNum=${whatsAppNum}
     Log  ${resp.content}
     Should Be Equal As Strings  ${resp.status_code}  422
