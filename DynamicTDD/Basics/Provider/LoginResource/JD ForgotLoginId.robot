...
JD-TC-Forgot_LoginId-6
    ...
    ${resp}=  Create User  ${firstname_u}  ${lastname_u}  ${countryCodes[0]}  ${ph}  ${userType[0]}  
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}     422
...
JD-TC-Forgot_LoginId-7
    ...
    ${resp}=  Encrypted Provider Login  ${loginId555}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-Forgot_LoginId-8
    ...
    ${resp}=  Encrypted Provider Login  ${loginId_str}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
