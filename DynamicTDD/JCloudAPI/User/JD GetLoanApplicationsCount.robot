...
JD-TC-GetLoanApplicationsCount-1

    [Documentation]   create a loan application by sales officer
    ...   branch1 and dealer1(credit approved status)
    ...   and check Get Loan Applications Count count with user token 
    
    # ${HLMUSERNAME20}=  Account with Multiple Users in NBFC
    # Log  ${HLMUSERNAME20}
    # Set Suite Variable  ${HLMUSERNAME20}
...
# .... Create Branch2....
    ...
    ${resp}=    Create BranchMaster    ${branchCode1}    ${branchName1}    ${locId}    ${status[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
#  .....Get Loan Applications Count Public APi call....
    ...
    ${resp}=  Get Loan Applications Count  ${user_token}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1
    
JD-TC-GetLoanApplicationsCount-2

    [Documentation]   create 2 loan applications by sales officer for different partners
    ...   branch1 and dealer2(credit approved)
    ...   and check Get Loan Applications Count with user token 
...
JD-TC-GetLoanApplicationsCount-3
...
JD-TC-GetLoanApplicationsCount-4
...
JD-TC-GetLoanApplicationsCount-5

    [Documentation]   check Get Loan Applications Count with user token and id filter

    ${resp}=  Get Loan Applications Count  ${user_token}   id-eq=${loanid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1
   
    ${resp}=  Get Loan Applications Count  ${user_token}   id-eq=${loanid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1
  
    ${resp}=  Get Loan Applications Count  ${user_token}   id-eq=${loanid2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

    ${resp}=  Get Loan Applications Count  ${user_token}   id-eq=${loanid3}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-6

    [Documentation]   check Get Loan Applications Count with user token and uid filter

    ${resp}=  Get Loan Applications Count  ${user_token}   uid-eq=${loanUid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

    ${resp}=  Get Loan Applications Count  ${user_token}   uid-eq=${loanUid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

    ${resp}=  Get Loan Applications Count  ${user_token}   uid-eq=${loanUid2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-7

    [Documentation]   check Get Loan Applications Count with user token and referenceNo filter

    ${resp}=  Get Loan Applications Count  ${user_token}   referenceNo-eq=${ref_no}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

    ${resp}=  Get Loan Applications Count  ${user_token}   referenceNo-eq=${ref_no1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

    ${resp}=  Get Loan Applications Count  ${user_token}   referenceNo-eq=${ref_no2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-8

    [Documentation]   check Get Loan Applications Count with user token and customer id filter

    ${resp}=  Get Loan Applications Count  ${user_token}   customer-eq=${custid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}        2

JD-TC-GetLoanApplicationsCount-9

    [Documentation]   check Get Loan Applications Count with user token and customer firstname filter

    ${resp}=  Get Loan Applications Count  ${user_token}   customerFirstName-eq=${Custfname1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-10

    [Documentation]   check Get Loan Applications Count with user token and customer lastname filter

    ${resp}=  Get Loan Applications Count  ${user_token}   customerLastName-eq=${Custlname2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-11

    [Documentation]   check Get Loan Applications Count with user token and location filter

    ${resp}=  Get Loan Applications Count  ${user_token}   location-eq=${locId}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         3

    ${resp}=  Get Loan Applications Count  ${user_token}   location-eq=${locid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-12

    [Documentation]   check Get Loan Applications Count with user token and location name filter

    ${resp}=  Get Loan Applications Count  ${user_token}   locationName-eq=${locname}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         3

    ${resp}=  Get Loan Applications Count  ${user_token}   locationName-eq=${locname1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-13

    [Documentation]   check Get Loan Applications Count with user token and category id filter

    ${resp}=  Get Loan Applications Count  ${user_token}   category-eq=${categoryid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         3

    ${resp}=  Get Loan Applications Count  ${user_token}   category-eq=${categoryid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-14

    [Documentation]   check Get Loan Applications Count with user token and categoryName filter

    ${resp}=  Get Loan Applications Count  ${user_token}   categoryName-eq=${categoryname}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}        3

    ${resp}=  Get Loan Applications Count  ${user_token}   categoryName-eq=${categoryname1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}       1

JD-TC-GetLoanApplicationsCount-15

    [Documentation]   check Get Loan Applications Count with user token and type id filter

    ${resp}=  Get Loan Applications Count  ${user_token}   type-eq=${typeid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}        3

    ${resp}=  Get Loan Applications Count  ${user_token}   type-eq=${typeid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-16

    [Documentation]   check Get Loan Applications Count with user token and type name filter

    ${resp}=  Get Loan Applications Count  ${user_token}   typeName-eq=${typename}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}        3

    ${resp}=  Get Loan Applications Count  ${user_token}   typeName-eq=${typename1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

JD-TC-GetLoanApplicationsCount-17

    [Documentation]   check Get Loan Applications Count with user token and rejected filter
    ...
    ${resp}=  Get Loan Applications Count  ${user_token}   isRejected-eq=${bool[1]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}         1

    ${resp}=  Get Loan Applications Count  ${user_token}   isRejected-eq=${bool[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()}        3