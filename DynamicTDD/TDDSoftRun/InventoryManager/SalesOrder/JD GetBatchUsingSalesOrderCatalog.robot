...
JD-TC-Get Batches using Salesordercatalog-1
    ...
    ${resp}=  Get Batches using Salesordercatalog    ${SO_itemEncIds} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Should Be Equal As Strings  ${resp.json()[0]['inventoryItemBatch']['encId']}   ${batch_encid1}
    Should Be Equal As Strings  ${resp.json()[0]['inventoryItemBatch']['name']}   ${batch_name}
    Should Be Equal As Strings  ${resp.json()[0]['inventoryItemBatch']['expiryDate']}   ${expiryDate}
    Should Be Equal As Strings  ${resp.json()[0]['inventoryItemBatch']['availableQty']}   ${availableQty}
    Should Be Equal As Strings  ${resp.json()[0]['inventoryItemBatch']['trueAvailableQty']}   ${availableQty}
    Should Be Equal As Strings  ${resp.json()[0]['inventoryItemBatch']['futureAvailableQty']}   ${availableQty}
    Should Be Equal As Strings  ${resp.json()[0]['encId']}   ${batchEncid}
    Should Be Equal As Strings  ${resp.json()[0]['name']}   ${batch_name}
