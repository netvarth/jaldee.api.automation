...
JD-TC-PurchaseAttachment-1
    ...
    ${netR}=         Evaluate    ${taxableAmount} + ${taxAmount}
    ${netRate}=         Evaluate    round(${netR}, 2)
    ...
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
    Set Suite Variable    ${purchase_driveid}    ${resp.json()[0]['driveId']}

    ${attachment1}=     Create Dictionary   owner=${pid}  ownerType=${ownerType[0]}  fileName=${jpgfile}  fileSize=${fileSize}  caption=${caption}  fileType=${fileType}  action=${LoanAction[0]}  file=${jpgfile}  order=${order}  driveId=${purchase_driveid}

    ${resp}=    Add Purchase Attachment  ${purchaseId}  ${attachment1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
...
JD-TC-PurchaseAttachment-2
    ...
    ${attachment1}=     Create Dictionary   owner=${pid}  ownerType=${ownerType[0]}  fileName=${jpgfile}  fileSize=${fileSize}  caption=${caption3}  fileType=${fileType}  action=${LoanAction[3]}  file=${jpgfile}  order=${order}  driveId=${purchase_driveid}
...
JD-TC-PurchaseAttachment-3
    ...
    ${attachment1}=     Create Dictionary   owner=${pid}  ownerType=${ownerType[0]}  fileName=${jpgfile}  fileSize=${fileSize}  caption=${caption}  fileType=${fileType}  action=${LoanAction[1]}  file=${jpgfile}  order=${order}  driveId=${purchase_driveid}
