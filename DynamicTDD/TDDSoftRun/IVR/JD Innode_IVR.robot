...
JD-TC-Innode_IVR-UH2
    ...
    [Documentation]   Innode IVR without provider login
...
JD-TC-Innode_IVR-UH3
    ...
    ${resp}=    innode IVR    ${acc_id}     ${incall_id}    ${cons_verfy_node_value}    ${current_time}    ${clid}    ${empty}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${IVR_SETTING_NOt_FOUND}"
...
JD-TC-Innode_IVR-UH6
    ...
    ${resp}=    Incall IVR    ${fake_id}     ${incall_id}    ${incall_uid}    ${reference_id}    ${company_id}  ${clid_row}   ${clid}    ${empty}    ${ivr_inputValue[1]}    ${ivr_inputValue[1]}    ${empty}    ${empty}    ${created_date}    ${call_time}    ${empty}    ${NONE}    ${empty}
    Log  ${resp.json()}
    ...
    ${resp}=    innode IVR    ${fake_id}     ${incall_id}    ${cons_verfy_node_value}    ${current_time}    ${clid}    ${empty}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  404
    Should Be Equal As Strings  "${resp.json()}"   "${ACCOUNT_NOT_EXIST}"
