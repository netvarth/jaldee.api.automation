...
JD-TC-Update_IVR_Call_Status-1
    ...
    ${resp}=    ivr_user_details    ${acc_id}  ${countryCodes[1]}  ${myoperator_id}  ${PUSERNAME178}  ${countryCodes[1]}${PUSERNAME178}  ${user_id}  ${user_name}
...
#........  user answered    ........
    ...
    ${clid_user}=    Convert To String    ${countryCodes[1]}${PUSERNAME178}
...
#.......  call finished      .........                                                                                                                                                                                                                                                                                                   
    ...
    ${resp}=    Update IVR Call Status    ${ivr_uid}    ${ivr_status[3]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['callStatus']}    ${ivr_status[3]}

JD-TC-Update_IVR_Call_Status-2
    ...
    ${resp}=    Update IVR Call Status    ${ivr_uid}    ${ivr_status[4]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Update_IVR_Call_Status-UH1
    ...
    ${resp}=    Update IVR Call Status    ${inv}    ${ivr_status[2]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}    ${bool[0]}

JD-TC-Update_IVR_Call_Status-UH2
    ...
    ${invstatus}    FakerLibrary.firstName

    ${resp}=    Update IVR Call Status    ${ivr_uid}    ${invstatus}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  500

JD-TC-Update_IVR_Call_Status-UH3

    [Documentation]   Update IVR Call Status without login
