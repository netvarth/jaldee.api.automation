...
JD-TC-Get sp item type Filter-1

    [Documentation]  Get sp item type filter using status
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    ${resp}=    Get sp item type Filter     ${accountId}   status-eq=${toggle[0]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()[0]['typeCode']}                                              ${typeCode}
    Should Be Equal As Strings    ${resp.json()[0]['typeName']}                                            ${TypeName}
    Should Be Equal As Strings    ${resp.json()[0]['status']}                                                      ${toggle[0]}

JD-TC-Get sp item type Filter-UH1

    [Documentation]  Get sp item type filter using status as Element Should Be Disabled

    ${resp}=    ProviderConsumer Login with token   ${primaryMobileNo}    ${accountId}  ${token} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Get sp item type Filter     ${accountId}   status-eq=${toggle[1]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()}   []

JD-TC-Get sp item type Filter-UH2

    [Documentation]  Get sp item type  Filter  using invalid account id

    ${resp}=    ProviderConsumer Login with token   ${primaryMobileNo}    ${accountId}  ${token} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Get sp item type Filter     ${primaryMobileNo}   status-eq=${toggle[1]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()}   []

JD-TC-Get sp item type Filter-3

    [Documentation]  create 3 item with item type and filter
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${PUSERNAME103}
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()[0]['typeCode']}                                              ${typeCode2}
    Should Be Equal As Strings    ${resp.json()[0]['typeName']}                                            ${Name}
    Should Be Equal As Strings    ${resp.json()[0]['status']}                                                      ${toggle[0]}
    Should Be Equal As Strings    ${resp.json()[1]['typeCode']}                                              ${typeCode1}
    Should Be Equal As Strings    ${resp.json()[1]['typeName']}                                            ${categoryName}
    Should Be Equal As Strings    ${resp.json()[1]['status']}                                                      ${toggle[0]}
    Should Be Equal As Strings    ${resp.json()[2]['typeCode']}                                              ${typeCode}
    Should Be Equal As Strings    ${resp.json()[2]['typeName']}                                            ${TypeName}
    Should Be Equal As Strings    ${resp.json()[2]['status']}                                                      ${toggle[0]}


JD-TC-Get sp item type Filter-4

    [Documentation]  In the sales order catalog where inventory manager is on ,then get sp item type filter
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${PUSERNAME102}
    Set Test Variable    ${accountId} 
