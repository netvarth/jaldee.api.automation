...
JD-TC-Get Item List By CartUid-1
...
# -------------------------------- Create store type -----------------------------------
    ...
    ${PhoneNumber}=  Evaluate  ${PUSERNAME}+100187478
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    ${primaryMobileNo}    Generate random string    10    132456789
    ...
    ${resp}=    Get Item List By Cart Uid   ${cartUid} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    FOR  ${i}  IN RANGE   ${len}
        ...
        IF  '${resp.json()[${i}]['uid']}' == '${cartItemUid1}'  
            Should Be Equal As Strings    ${resp.json()[${i}]['providerConsumer']['id']}                                              ${cid}
            ...
            Should Be Equal As Strings    ${resp.json()[${i}]['deliveryType']}                                                        ${deliveryType[0]}
            ...
            Should Be Equal As Strings    ${resp.json()[${i}]['quantity']}                                                            ${quantity}      
            Should Be Equal As Strings    ${resp.json()[${i}]['price']}                                                               ${price}  
