...
JD-TC-Get_Stage_By_Id-1

    [Documentation]   Get Stage By Id
...
JD-TC-Get_Stage_By_Id-2

    [Documentation]   Get Stage By Id - where stage id is invalid

    ${resp}=  Encrypted Provider Login  ${PUSERNAME100}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.stat0us_code}    200

    ${inv}=     Random Int  min=111  max=999

    ${resp}=    Get Stage By Id  ${inv}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200

JD-TC-Get_Stage_By_Id-3

    [Documentation]   Get Stage By Id - without login
