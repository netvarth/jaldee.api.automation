...
JD-TC-GetConsumerApptSchedulesBySchedule_id-1
    [Documentation]   When prodider create appointment schedules 
    
    ${resp}=  Encrypted Provider Login  ${PUSERNAME156}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accId}=  get_acc_id  ${PUSERNAME156}
    Set Suite Variable  ${PUSERNAME156_id}   ${accId}
...
JD-TC-GetConsumerApptSchedulesBySchedule_id-2
...
JD-TC-GetConsumerApptSchedulesBySchedule_id-3
...
JD-TC-GetConsumerApptSchedulesBySchedule_id-4
    ...
    ${resp}=  Get Appointment Schedule ById Consumer  ${sch_id1}  ${PUSERNAME156_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetConsumerApptSchedulesBySchedule_id-UH1

    [Documentation]   Get Appointment without consumer login

    ${resp}=  Get Appointment Schedule ById Consumer  ${sch_id2}  ${PUSERNAME156_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  419
    Should Be Equal As Strings  "${resp.json()}"  "${SESSION_EXPIRED}"

JD-TC-GetConsumerApptSchedulesBySchedule_id-UH2
