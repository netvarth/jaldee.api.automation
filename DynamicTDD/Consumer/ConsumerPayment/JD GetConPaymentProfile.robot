...
JD-TC-GetPaymentProfile-1
    ...
    ${resp}=   Create Bank Details   ${bank_name}  ${bank_ac}   ${ifsc_code}  ${name}  ${name}  ${branch}  ${businessFilingStatus[1]}  ${accountType[1]}  ${pan_num}  ${CUSERNAME7}
    ...
    ${resp}=   Get conspayment profiles  ${pid}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
