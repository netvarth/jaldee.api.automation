...
JD-TC-ResubmitServiceOptionForAppointment-1
...
JD-TC-ResubmitServiceOptionsForAppointment-2
    [Documentation]  Resubmit service option for appointment taken from provider side

    clear_customer   ${PUSERNAME29}
...
JD-TC-ResubmitServoiceOptionsForAppointment-3
    [Documentation]  Resubmit serviceoptions for appointment after starting appointment
    ...
    ${qnr_resp}=  Get Service Options By Service  ${s_id}  ${consumer}  ${account_id}
    Log  ${qnr_resp.content}
    Should Be Equal As Strings    ${qnr_resp.status_code}    200

    ${fudata}=  db.fileUploadDT   ${qnr_resp.json()}  ${FileAction[0]}  ${Questionnaireid3}  ${pdffile}
    Log  ${fudata}
...
JD-TC-ResubmitServoiceOptionsForAppointment-4
    [Documentation]  Resubmit serviceoptions for appointment after completing appointment
   ...
   ${resp}=  Get Appointment By Id   ${apptid1}
    ...
    ${fudata}=  db.fileUploadDT   ${qnr_resp.json()}  ${FileAction[0]}  ${Questionnaireid4}  ${pdffile}
    Log  ${fudata}
    ${data}=  db.QuestionnaireAnswers   ${qnr_resp.json()}   ${self}   &{fudata}
    Log  ${data}
    ${resp}=  Consumer Validate Questionnaire  ${account_id}  ${data}
...
JD-TC-ResubmitServoiceOptionsForAppointment-UH1
    [Documentation]  Resubmit serviceoptions for appointment after canceling appointment
   ...
   ${resp}=  Get Appointment By Id   ${apptid1}
    ...
    ${fudata}=  db.fileUploadDT   ${qnr_resp.json()}  ${FileAction[0]}  ${Questionnaireid5}  ${pdffile}
    Log  ${fudata}
    ${data}=  db.QuestionnaireAnswers   ${qnr_resp.json()}   ${self}   &{fudata}
    Log  ${data}
    ${resp}=  Consumer Validate Questionnaire  ${account_id}  ${data}
...
JD-TC-ResubmitServoiceOptionsForAppointment-UH2
    [Documentation]  Resubmit serviceoptions by provider login
...
JD-TC-ResubmitServoiceOptionsForAppointment-UH3
    [Documentation]  Resubmit serviceoptions without login

    ${resp}=  Imageupload.CApptResubmitServiceOption  ${empty}  ${account_id}   ${apptid1}   ${data}  ${pdffile}  ${jpgfile}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  419
    Should Be Equal As Strings  ${resp.json()}  ${SESSION_EXPIRED}

JD-TC-ResubmitServoiceOptionsForAppointment-UH4
    [Documentation]  Resubmit serviceoptions with invalid Appointment id

    ${cookie}  ${resp}=  Imageupload.conLogin  ${CUSERNAME9}   ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings   ${resp.status_code}    200

    ${inv_appmt_id}=  Generate Random String  3  [NUMBERS] 

    ${resp}=  Imageupload.CApptResubmitServiceOption  ${cookie}  ${account_id}   ${inv_appmt_id}   ${data}  ${pdffile}  ${jpgfile}
    Log  ${resp.content}
