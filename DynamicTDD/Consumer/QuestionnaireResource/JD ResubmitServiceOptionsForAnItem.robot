...
JD-TC-ResubmitServiceOptinsForAnItem-1
    [Documentation]  Reubmit Service option for item
    ...
    ${resp}=    Get Questionnaire By Id    ${p_id}   ${id}
    Log   ${resp.content}
    Should Be Equal As Strings   ${resp.status_code}    200
    ...
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME11}.${test_mail}
    ${landMark}=  FakerLibrary.street name 
    ${address}=  Create Dictionary   phoneNumber=${CUSERNAME11}    firstName=${fname}   lastName=${lname}   email=${C_email}    address=${district}   city=${city}   postalCode=${pin}    landMark=${landMark}   countryCode=${countryCodes[0]}
    ...
    # ${resp}=   Create Order For HomeDelivery   ${cookie}  ${account_id}    ${self}    ${catalogid1}     ${bool[1]}    ${address}    ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME11}    ${C_email}  ${countryCodes[1]}  ${emptylist}  ${item_id1}  ${item_quantity1}
    # Log  ${resp.content}
    # Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-ResubmitServiceOptinsForAnItem-UH1
    [Documentation]  Resubmit service option for item where order is canceled
...
JD-TC-ResubmitServiceOptinsForAnItem-UH2
    [Documentation]  Resubmit service options for item usimg random item number
...
JD-TC-ResubmitServiceOptinsForAnItem-3
    [Documentation]  Resubmit service options for item with another user
...
JD-TC-ResubmitServiceOptinsForAnItem-UH4
    [Documentation]  Resubmit service options for item with another provider
