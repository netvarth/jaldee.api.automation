...
JD-TC-GetServiceOptionsForItem-1
    [Documentation]  Get Service Options by uuid for item
    ...
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME16}.ynwtest@netvarth.com
    ...
    ${resp}=   Create Order For HomeDelivery   ${cookie}  ${account_id}    ${self}    ${catalogid1}     ${bool[1]}    ${address}    ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME16}    ${C_email}  ${countryCodes[1]}  ${emptylist}  ${item_id1}  ${item_quantity1}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetServiceOptionsForItem-UH1
...
JD-TC-GetServiceOptionsForItem-UH2
    [Documentation]    Get Service Options With invalid itemid

    ${resp}=  Consumer Login  ${CUSERNAME16}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${inv_item_id}=  Generate Random String  3  [NUMBERS] 

    ${resp}=     Get service options for an item  ${inv_item_id}  ${account_id}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  ${id0}

JD-TC-GetServiceOptionsForItem-UH3
