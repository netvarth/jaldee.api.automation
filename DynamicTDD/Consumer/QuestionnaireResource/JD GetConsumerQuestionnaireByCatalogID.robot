...
*** Test Cases ***
JD-TC-GetConsumerQuestionnaireByCatalogId-1
    ...
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME33}.ynwtest@netvarth.com
    ...
    ${resp}=   Create Order For HomeDelivery   ${cookie}  ${account_id}    ${self}    ${catalogid}     ${bool[1]}    ${address}    ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME33}    ${C_email}  ${countryCodes[1]}  ${emptylist}  ${item_id1}  ${item_quantity1}  ${item_id2}  ${item_quantity2}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetConsumerQuestionnaireByCatalogId-2
...
JD-TC-GetConsumerQuestionnaireByCatalogId-UH1
...
JD-TC-GetConsumerQuestionnaireByCatalogId-UH2
...
JD-TC-GetConsumerQuestionnaireByCatalogId-3
    [Documentation]  Get questionnaire by catalog id for Order taken from provider side
    ...
    ${resp}=   Create Order By Provider For HomeDelivery    ${cookie}  ${cid31}   ${cid31}   ${catalogid}   ${boolean[1]}   ${address}  ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME33}    ${C_email}  ${orderNote}  ${countryCodes[1]}  ${item_id1}   ${item_quantity1}  ${item_id2}   ${item_quantity2}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${orderid}=  Get Dictionary Values  ${resp.json()}
    Set Suite Variable  ${orderid1}  ${orderid[0]}
...
JD-TC-GetConsumerQuestionnaireByCatalogId-UH3
    [Documentation]  Get questionnaire by invalid catalog id

    ${resp}=  Consumer Login  ${CUSERNAME33}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 

    # ${inv_catalog_id}=  Generate Random String  16  [LETTERS][NUMBERS]
    ${inv_catalog_id}=  Generate Random String  3  [NUMBERS] 

    ${resp}=  Get Questionnaire By CatalogID        ${inv_catalog_id}   ${account_id}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}    200
    # Should Be Equal As Strings  ${resp.status_code}    401
    # Should Be Equal As Strings  ${resp.json()}   ${LOGIN_NO_ACCESS_FOR_URL}
...
JD-TC-GetConsumerQuestionnaireByCatalogId-UH4
    [Documentation]  Get questionnaire by another catalog id from the same provider
    ...
    # ${resp}=   Create Order For HomeDelivery   ${cookie}  ${account_id}    ${self}    ${catalogid1}     ${bool[1]}    ${address}    ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME34}    ${C_email}  ${countryCodes[1]}  ${emptylist}  ${item_id1}  ${item_quantity1}
    # Log  ${resp.content}
    # Should Be Equal As Strings    ${resp.status_code}    200
    # ${orderid}=  Get Dictionary Values  ${resp.json()}
...
JD-TC-GetConsumerQuestionnaireByCatalogId-UH5
    [Documentation]  Get questionnaire by another provider's catalog id
    ...
    # ${resp}=   Create Order For HomeDelivery   ${cookie}  ${account_id1}    ${self}    ${catalogid1}     ${bool[1]}    ${address}    ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME34}    ${C_email}  ${countryCodes[1]}  ${emptylist}  ${item_id1}  ${item_quantity1}  ${item_id2}  ${item_quantity2}
    # Log  ${resp.content}
    # Should Be Equal As Strings    ${resp.status_code}    200
    # ${orderid}=  Get Dictionary Values  ${resp.json()}
    # Set Suite Variable  ${orderid2}  ${orderid[0]}
    ...
    ${resp}=  Get Questionnaire By CatalogID   ${catalogid1}   ${account_id1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}    200
    # Should Be Equal As Strings  ${resp.status_code}    401
    # Should Be Equal As Strings  ${resp.json()}   ${LOGIN_NO_ACCESS_FOR_URL}