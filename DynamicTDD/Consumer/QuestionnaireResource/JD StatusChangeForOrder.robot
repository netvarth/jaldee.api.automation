...
JD-TC-StatusChangeForServiceOptionOrder-1
    [Documentation]  Status Change For service options order

    clear_Item      ${PUSERNAME68}
   ...
   ${files_data}=  Create List
    ...
    ${resp}=   Change Status Of Service Option Order    ${account_id}   ${orderid1}  @{files_data}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-StatusChangeForServiceOptionOrder-UH2
    [Documentation]  Status Change For service options order by provider login.

    ${cookie}  ${resp}=  Imageupload.spLogin  ${PUSERNAME68}   ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings   ${resp.status_code}    200
...
JD-TC-StatusChangeForServiceOptionOrder-UH3
    [Documentation]  Status Change For service options order without consumer login
...
JD-TC-StatusChangeForServiceOptionOrder-UH4
    [Documentation]  Status Change For service options order with invalid order id
    ...
    ${inv_order_id}=  Generate Random String  3  [NUMBERS] 

    ${resp}=   Change Status Of Service Option Order    ${account_id}   ${inv_order_id}  @{files_data}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  419
    Should Be Equal As Strings  ${resp.json()}  ${SESSION_EXPIRED}