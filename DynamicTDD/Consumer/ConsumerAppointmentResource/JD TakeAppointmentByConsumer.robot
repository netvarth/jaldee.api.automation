...
JD-TC-Take Appointment-9
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${pid01}  ${resp.json()['id']}
    ...
    ${eTime1}=  add_timezone_time  ${tz1}  4  15  
...
JD-TC-Take Appointment-UH12
    ...
    ${tz2}=   db.get_Timezone_by_lat_long   ${latti}  ${longi}
    Set Suite Variable  ${tz2}
    ${sTime1}=  add_timezone_time  ${tz2}  1  30  
    ${eTime1}=  add_timezone_time  ${tz2}  2  30  
    ${parking}    Random Element     ${parkingType} 
    ${24hours}    Random Element    ['True','False']
    ${url}=   FakerLibrary.url
    ...
    ${sTime1}=  add_timezone_time  ${tz1}  3  15  
    ${delta}=  FakerLibrary.Random Int  min=10  max=60
    ${eTime1}=  add_two   ${sTime1}  ${delta}
...
JD-TC-Take Appointment-21
    ...
    ${resp}=  Create User  ${firstname}  ${lastname}  ${dob}  ${Genderlist[0]}  ${B_Email}${BUSERPH0}.ynwtest@netvarth.com   ${userType[0]}  ${pin}  ${countryCodes[0]}  ${BUSERPH0}  ${dep_id}  ${sub_domain_id}  ${bool[0]}  ${countryCodes[0]}  ${whpnum}  ${countryCodes[0]}  ${tlgnum}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Take Appointment-UH14
    ...
    Set Test Variable  ${email_id}  ${P_Email}${PUSERNAME_D}.ynwtest@netvarth.com
