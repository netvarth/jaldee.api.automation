...
JD-TC-GetCatalogItem-1
    [Documentation]  Get an item from a catalog.
    ...
    Should Be Equal As Strings  ${resp.json()['displayName']}           ${displayName1}
    Should Be Equal As Strings  ${resp.json()['itemName']}              ${itemName1}
    Should Be Equal As Strings  ${resp.json()['itemCode']}              ${itemCode1}  
    Should Be Equal As Strings  ${resp.json()['status']}                ${status[0]}
    Should Be Equal As Strings  ${resp.json()['price']}                 ${price1}.0    
    Should Be Equal As Strings  ${resp.json()['promotionalPriceType']}  ${promotionalPriceType[1]}
    Should Be Equal As Strings  ${resp.json()['promotionalPrice']}      ${promoPrice1}.0

JD-TC-GetCatalogItem-2
    [Documentation]  Get an item from catalog by provider.
...
JD-TC-GetCatalogItem-3
    [Documentation]  Get an item from catalog having multiple items.
    ...
    Should Be Equal As Strings  ${resp.json()['itemCode']}              ${itemCode2}  
    Should Be Equal As Strings  ${resp.json()['status']}                ${status[0]}
    Should Be Equal As Strings  ${resp.json()['price']}                 ${price1}.0    
    Should Be Equal As Strings  ${resp.json()['promotionalPriceType']}  ${promotionalPriceType[1]}
    Should Be Equal As Strings  ${resp.json()['promotionalPrice']}      ${promoPrice1}.0

JD-TC-GetCatalogItem-UH1
    [Documentation]  Get an item not in the catalog.
...
JD-TC-GetCatalogItem-UH2
    [Documentation]  Get an item from another provider's catalog by provider.
    ...
    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME134}.ynwtest@netvarth.com
...
JD-TC-GetCatalogItem-UH3

    [Documentation]  Get an item from catalog without login
...
JD-TC-GetCatalogItem-UH4

    [Documentation]  Get an item with invalid catalog id. 
    ...
    ${resp}=  Get Item By Catalog  -11   ${item_id1}  
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${NO_CATALOG_FOUND}"

JD-TC-GetCatalogItem-UH5

    [Documentation]  Get an item with invalid item id. 

    ${resp}=  Consumer Login  ${CUSERNAME10}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    
    # ${item_id1}=  Random Int  min=0   max=200
