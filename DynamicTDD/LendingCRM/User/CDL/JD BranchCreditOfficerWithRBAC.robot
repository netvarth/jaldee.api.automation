...
JD-TC-BranchCreditOfficerWithRBAC-3
    ...
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    Set Suite Variable  ${part_id}  ${resp.json()['id']}
    ...
    ${resp}=    Partner Verify Otp for phone   ${consumernumber}   ${OtpPurpose['ConsumerVerifyPhone']}   ${custid}    ${Custfname}    ${Custlname}    ${consumernumber}    ${countryCodes[0]}    ${locId}    ${CustomerPhoto}    ${kyc_list1}    gender=${gender}  dob=${dob}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    Set Suite VAriable  ${loanid}    ${resp.json()['id']}
    Set Suite VAriable  ${loanuid}    ${resp.json()['uid']}
