...
JD-TC-SupportUser-1
    ...
    [Documentation]  Support User - Create Branch
...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+6478945
...
# ....Support User...
    ...
    ${role1}=           Create Dictionary  id=${role_id13}  roleName=${role_name13}  defaultRole=${bool[1]}   scope=${user_scope}   capabilities=${capabilities}
    ${user_roles}=      Create List        ${role1}

    ${user_ids}=        Create List        ${SPT} 
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}   200
    Should Be Equal As Strings             ${resp.json()['userRoles'][0]['id']}                 ${role_id13}
    Should Be Equal As Strings             ${resp.json()['userRoles'][0]['roleName']}           ${role_name13}
    Should Be Equal As Strings             ${resp.json()['userRoles'][0]['defaultRole']}        ${bool[1]}
    Should Be Equal As Strings             ${resp.json()['userRoles'][0]['capabilities']}       ${capability13}
    Should Be Equal As Strings             ${resp.json()['defaultRoleName']}                    ${role_name13}
...
# .....Create Dealer By Sales Officer.......
    ...
    ${resp}=    Reset LoginId  ${SPT}  ${SPT_USERNME}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Forgot Password   loginId=${SPT_USERNME}  password=${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    202

    ${resp}=    Account Activation  ${SPT_USERNME}  ${OtpPurpose['ProviderResetPassword']}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${key} =   db.Verify Accnt   ${SPT_USERNME}    ${OtpPurpose['ProviderResetPassword']}
    Set Suite Variable   ${key}
    ...
    ${resp}=    Connect with other login  ${SPT_USERNME}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-SupportUser-2
    ...
    [Documentation]  Support User - approval request
...
JD-TC-SupportUser-3
    ...
    [Documentation]  Support User - Partner approved
...
JD-TC-SupportUser-4
    ...
    [Documentation]  Support User - Update Sales and Credit officer
...
JD-TC-SupportUser-5
    ...
    [Documentation]  Support User - Activate Partner 
...
JD-TC-SupportUser-6
    ...
    [Documentation]  Support User - Create CDL Enq 
...
JD-TC-SupportUser-7
    ...
    [Documentation]  Support User - Create Loan Application
...
JD-TC-SupportUser-9
    ...
    [Documentation]  Support User - Equifax, Mafil and Credit score
...
JD-TC-SupportUser-10

    [Documentation]  Support User - Equifax score
...
JD-TC-SupportUser-11

    [Documentation]  Support User - Mafil score
...
JD-TC-SupportUser-12

    [Documentation]  Support User - Cibil score
...
JD-TC-SupportUser-15

    [Documentation]  Support User - Scheme Approval
