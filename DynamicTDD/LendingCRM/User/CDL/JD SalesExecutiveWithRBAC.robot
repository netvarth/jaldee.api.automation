...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+54671573
...
# .....Create Dealer By Sales Officer.......
    ...
    ${resp}=    Reset LoginId  ${SE}  ${SE_USERNAME}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Forgot Password   loginId=${SE_USERNAME}  password=${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    202

    ${resp}=    Account Activation  ${SE_USERNAME}  ${OtpPurpose['ProviderResetPassword']}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${key} =   db.Verify Accnt   ${SE_USERNAME}    ${OtpPurpose['ProviderResetPassword']}
    ...
    ${resp}=    Connect with other login  ${SE_USERNAME}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
