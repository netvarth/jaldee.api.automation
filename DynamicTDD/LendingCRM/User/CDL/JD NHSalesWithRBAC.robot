...
# .....Create Dealer By NH Sales .......
    ...
    ${resp}=    Reset LoginId  ${NHSO}  ${NHSO_USRNME}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Forgot Password   loginId=${NHSO_USRNME}  password=${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    202

    ${resp}=    Account Activation  ${NHSO_USRNME}  ${OtpPurpose['ProviderResetPassword']}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${key} =   db.Verify Accnt   ${NHSO_USRNME}    ${OtpPurpose['ProviderResetPassword']}
    ...
    ${resp}=    Connect with other login  ${NHSO_USRNME}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
