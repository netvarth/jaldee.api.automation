...
# ..... SignUp Business Head
    ...
    ${NBFCMUSERNAME1}=  Evaluate  ${MUSERNAME}+1647894
...
# ....... Manual Approval .......
    ...
    Log  ${time.content}
    Should Be Equal As Strings  ${time.status_code}  200
    Set Suite Variable      ${datetime10}    ${time.json()} 
...
JD-TC-Loan Approval-2
    ...
    ${UNABLE_TO_CHANGE_INSTERNAL_STATUS}=   Replace String  ${UNABLE_TO_CHANGE_INSTERNAL_STATUS}  {}  ${LoanApplicationSpInternalStatus[5]}
...
JD-TC-Loan Approval-3
    ...
    [Documentation]               Approve loan - where loan uid is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=  salesofficer Approval        ${empty}    ${sch1}     ${tenu1}    ${noOfAdvanceEmi}   ${dayofmonth}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422
...
JD-TC-Loan Approval-4
    ...
    [Documentation]               Approve loan - where loan uid is invalid
    ...
    ${resp}=  salesofficer Approval        ${fake}    ${sch1}     ${tenu1}    ${noOfAdvanceEmi}   ${dayofmonth}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422
    Should Be Equal As Strings             ${resp.json()}  ${INVALID_LOAN_APPLICATION_ID}
...
JD-TC-Loan Approval-5
    ...
    [Documentation]               Approve loan - where scheme uid is empty (gettinh 422 bcoz its already approved)

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=  salesofficer Approval        ${loanuid}    ${empty}     ${tenu1}    ${noOfAdvanceEmi}   ${dayofmonth}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422

JD-TC-Loan Approval-6
                                  
    [Documentation]               Approve loan - where scheme uid is invalid  (gettinh 422 bcoz its already approved)
    ...
    ${resp}=  salesofficer Approval        ${loanuid}    ${fake}     ${tenu1}    ${noOfAdvanceEmi}   ${dayofmonth}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422

JD-TC-Loan Approval-7
                                  
    [Documentation]               Approve loan - where number of emi is empty  (gettinh 422 bcoz its already approved)

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=  salesofficer Approval        ${loanuid}    ${sch1}     ${empty}    ${noOfAdvanceEmi}   ${dayofmonth}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422

JD-TC-Loan Approval-8
                                  
    [Documentation]               Approve loan - where number of emi is invalid  (gettinh 422 bcoz its already approved)
    ...
    ${resp}=  salesofficer Approval        ${loanuid}    ${sch1}     ${tenu1}    ${empty}   ${dayofmonth}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422

JD-TC-Loan Approval-9
                                  
    [Documentation]               Approve loan - where number of emi due day is empty  (gettinh 422 bcoz its already approved)

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=  salesofficer Approval        ${loanuid}    ${sch1}     ${tenu1}    ${noOfAdvanceEmi}   ${empty}    partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}  422

JD-TC-Loan Approval-10
                                  
    [Documentation]               Approve loan - without login
