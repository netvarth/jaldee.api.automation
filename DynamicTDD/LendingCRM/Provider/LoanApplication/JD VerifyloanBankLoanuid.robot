...
JD-TC-VerifyLoanBank-UH2
    ...
    ${resp}=    Verify loan Bank   ${EMPTY}   ${bankAccountNo2}     ${bankIfsc}    bankName=${bankName}
    Log  ${resp.content}
...
JD-TC-VerifyLoanBank-UH3
    ...
    ${INVALID_ORIGIN_UID}=  Format String   ${INVALID_ORIGIN_UID}    ${loan_uids}

    ${resp}=    Verify loan Bank   ${loan_uids}     ${bankAccountNo2}    ${bankIfsc}    bankName=${bankName}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}    ${INVALID_ORIGIN_UID}
...
JD-TC-VerifyLoanBank-UH4
    ...
    ${resp}=    Verify loan Bank   ${loanuid}     ${SPACE}    ${bankIfsc}    bankName=${bankName}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
...
JD-TC-VerifyLoanBank-UH5
    ...
    ${resp}=    Verify loan Bank   ${loanuid}     ${bankAccountNo2}    ${EMPTY}    bankName=${bankName}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
