...
JD-TC-Loan_Application_Action_Required-UH8
    ...
    ${resp}=  Verify Phone and Create Loan Application with customer details  ${phone2}  ${OtpPurpose['ConsumerVerifyPhone']}  ${custid2}  ${locId}   ${CustomerPhoto}   ${kyc_list1}  firstName=${fname}  lastName=${lname}  phoneNo=${phone2}  countryCode=${countryCodes[0]}  gender=${gender}  dob=${dob}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
