...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+6475124
...
# ....... Verify Pan Number .......
    ...
    ${panAttachment}  Create Dictionary   action=${LoanAction[0]}  owner=${cust_id}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}
    Log  ${panAttachment}
    Set Suite Variable      ${panAttachment}

    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${cust}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200
...
JD-TC-RequstForPanValidation-UH1
    ...
    [Documentation]               requst for pan validation - whcich is already validated
...
JD-TC-RequstForPanValidation-UH2
    ...
    [Documentation]               requst for pan validation - where id is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${resp}=   Requst For Pan Validation   ${empty}    ${loanuid}    ${cust}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200
...
JD-TC-RequstForPanValidation-UH3
    ...
    [Documentation]               requst for pan validation - where id is invalid
    ...
    ${resp}=   Requst For Pan Validation   ${fake}    ${loanuid}    ${cust}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200
...
JD-TC-RequstForPanValidation-Uh4
    ...
    [Documentation]               requst for pan validation - uid is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${resp}=   Requst For Pan Validation   ${cust_id}    ${empty}    ${cust}    ${pan}    ${panAttachment}
    Log  ${resp.content}
...
JD-TC-RequstForPanValidation-Uh5
    ...
    [Documentation]               requst for pan validation - where uid is invalid
...
JD-TC-RequstForPanValidation-Uh6
    ...
    [Documentation]               requst for pan validation - where phone is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${empty}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
    Should Be Equal As Strings             ${resp.json()}    ${INVALID_KYC_ID}
...
JD-TC-RequstForPanValidation-Uh7
    ...
    [Documentation]               requst for pan validation - where phone is invalid
    ...
    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${fake}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
    Should Be Equal As Strings             ${resp.json()}    ${INVALID_KYC_ID}
...
JD-TC-RequstForPanValidation-UH8
    ...
    [Documentation]               requst for pan validation - where pan is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${cust}    ${empty}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
...
JD-TC-RequstForPanValidation-UH9
    ...
    [Documentation]               requst for pan validation - where pan is invalid
    ...
    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${cust}    ${fake}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200

JD-TC-RequstForPanValidation-UH10
                                  
    [Documentation]               requst for pan validation - where action in remove

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${aadhaarAttachment}=                 Create Dictionary   action=${LoanAction[1]}  owner=${cust_id}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}   driveId=${driveId2}   ownerType=${ownerType[0]}   type=photo
...
JD-TC-RequstForPanValidation-UH11
                                  
    [Documentation]               requst for pan validation - where owner is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${panAttachment}=                 Create Dictionary   action=${LoanAction[0]}  owner=${empty}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}   driveId=${driveId2}   ownerType=${ownerType[0]}   type=photo

    ${resp}=                               Requst For Aadhar Validation    ${cust_id}    ${loanuid}    ${cust}    ${aadhaar}    ${aadhaarAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200

JD-TC-RequstForPanValidation-UH12
                                  
    [Documentation]               requst for pan validation - where owner is invalid
    ...
    ${panAttachment}=                 Create Dictionary   action=${LoanAction[0]}  owner=${fake}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}   driveId=${driveId2}   ownerType=${ownerType[0]}   type=photo

    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${cust}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200

JD-TC-RequstForPanValidation-UH13
                                  
    [Documentation]               requst for pan validation - where file name is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${panAttachment}=                 Create Dictionary   action=${LoanAction[0]}  owner=${cust_id}  fileName=${empty}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}   driveId=${driveId2}   ownerType=${ownerType[0]}   type=photo
...
JD-TC-RequstForPanValidation-UH14
                                  
    [Documentation]               requst for pan validation - where file size is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${panAttachment}=                 Create Dictionary   action=${LoanAction[0]}  owner=${cust_id}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}   driveId=${driveId2}   ownerType=${ownerType[0]}   type=photo

    ${resp}=   Requst For Pan Validation   ${cust_id}    ${loanuid}    ${cust}    ${pan}    ${panAttachment}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200

JD-TC-RequstForPanValidation-UH15
                                  
    [Documentation]               requst for pan validation - where caption is empty
...
JD-TC-RequstForPanValidation-UH16
                                  
    [Documentation]               requst for pan validation - where file type is empty

    ${resp}=  Encrypted Provider Login     ${BM_USERNAME}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}      200

    ${panAttachment}=                 Create Dictionary   action=${LoanAction[0]}  owner=${cust_id}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${empty}  order=${order}   driveId=${driveId2}   ownerType=${ownerType[0]}   type=photo
...
JD-TC-RequstForPanValidation-UH17
                                  
    [Documentation]               requst for pan validation - whithout login
