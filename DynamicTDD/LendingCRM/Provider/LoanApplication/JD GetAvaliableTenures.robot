...
JD-TC-Get Avaliable Tenures-UH1
                                  
    [Documentation]               Get Avaliable Scheme with invalid loanid.
    
    ${resp}=  Encrypted Provider Login  ${HLMUSERNAME10}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${loanid}=    Random Int  min=100   max=200
...
JD-TC-Get Avaliable Tenures-UH2
                                  
    [Documentation]               Get Avaliable Tenures without login.

    ${resp}=    Get Avaliable Tenures    ${loanuid}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  419
    Should Be Equal As Strings   ${resp.json()}  ${SESSION_EXPIRED}

JD-TC-Get Avaliable Tenures-UH3
                                  
    [Documentation]               Get Avaliable Tenures with consumer login.
