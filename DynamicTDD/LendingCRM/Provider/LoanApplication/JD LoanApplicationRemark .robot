...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+2467542
...
JD-TC-LoanApplicationRemark-UH1
    ...
    [Documentation]              add same LoanApplication Remark - uid is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=    LoanApplication Remark     ${empty}  ${remarks}
    Log  ${resp.content}
...
JD-TC-LoanApplicationRemark-UH2
    ...
    [Documentation]              add same LoanApplication Remark - uid is invalid
    ...
    ${resp}=    LoanApplication Remark     ${fake}  ${remarks}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
    Should Be Equal As Strings             ${resp.json()}    ${INVALID_LOAN_APPLICATION_ID}
...
JD-TC-LoanApplicationRemark-3
    ...
    [Documentation]              add same LoanApplication Remark - remark is empty
...
JD-TC-LoanApplicationRemark-UH3
    ...
    [Documentation]              add same LoanApplication Remark - without login
...
JD-TC-LoanApplicationRemark-UH4
    ...
    [Documentation]              add same LoanApplication Remark - with another provider login
