...
JD-TC-Get loan Bank Details Aadhaar-2
    ...
    ${resp}=  Verify Phone and Create Loan Application with customer details  ${P_phone}  ${OtpPurpose['ConsumerVerifyPhone']}  ${custid}  ${locId}  ${CustomerPhoto}   ${kyc_list1}  firstName=${firstName}  lastName=${lastName}  phoneNo=${P_phone}  countryCode=${countryCodes[0]}  gender=${gender}  dob=${dob}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
