...
# <----------------------------- Bank Details ------------------------------------------> 
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable    ${tenu1}    ${resp.json()[6]['id']}

    ${resp}=  salesofficer Approval    ${loanuid}    ${Schemeid1}    ${tenu1}   2  2 
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Get Loan EMI Details-UH1
                                  
    [Documentation]              Just Create a loan  And try to-Get Loan EMI Details.
    ...
    ${resp}=    Get Loan EMI Details    ${loanuid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
