...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+2647249
...
JD-TC-GenerateotpForPhoneNum-UH1
                                  
    [Documentation]               Generate otp for phone Num - phone number is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=                               Generate Loan Application Otp for Phone Number    ${empty}  ${countryCodes[0]}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
    Should Be Equal As Strings             ${resp.json()}         ${INVALID_INPUT}

JD-TC-GenerateotpForPhoneNum-UH2
                                  
    [Documentation]               Generate otp for phone Num - phone number is invalid
    ...
    ${resp}=                               Generate Loan Application Otp for Phone Number    ${fake}  ${countryCodes[0]}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422

JD-TC-GenerateotpForPhoneNum-UH3
                                  
    [Documentation]               Generate otp for phone Num - country code is invalid
    ...
    ${resp}=                               Generate Loan Application Otp for Phone Number    ${cust}  ${fake}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200

JD-TC-GenerateotpForPhoneNum-UH4
                                  
    [Documentation]               Generate otp for phone Num - country code is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=                               Generate Loan Application Otp for Phone Number    ${cust}  ${empty}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200

JD-TC-GenerateotpForPhoneNum-UH5
                                  
    [Documentation]               Generate otp for phone Num - without login
...
JD-TC-GenerateotpForPhoneNum-UH6
                                  
    [Documentation]               Generate otp for phone Num -  with consumer login
