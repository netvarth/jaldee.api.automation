...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+6473492
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-UH1
    ...
    ${resp}=    Otp for Consumer Loan Acceptance Phone    ${inv}    12    ${loanuid}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}       ${ENTER_VALID_OTP}


JD-TC-GenerateOtpforConsumerAcceptancePhone-UH2
                                  
    [Documentation]              Generate Otp for Consumer Acceptance where the status is already accepted
    ...
    ${resp}=    Otp for Consumer Acceptance Phone   ${cust}    ${email}     ${countryCodes[3]}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200

    ${resp}=    Otp for Consumer Loan Acceptance Phone    ${cust}    12    ${loanuid}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}       ${STATUS_IS_ALREADY_CONSUMERACCEPTED}


JD-TC-GenerateOtpforConsumerAcceptancePhone-Uh3
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-UH4
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-UH5
