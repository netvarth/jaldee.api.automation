...
${order}    0
${fileSize}  0.00458
${email2}    adityan.pk@netvarth.com
${cc}   +91
${cc12}   +96
${otp}    12
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-1
                                  
    [Documentation]              Generate Otp for Consumer Acceptance Phone
    ...
    ${resp}    upload file to temporary location    ${file_action[0]}    ${provider_id}    ${ownerType[0]}    ${name}    ${pdffile}    ${fileSize}    ${caption}    ${fileType}    ${EMPTY}    ${order}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-2
    ...
    # ${resp}=    Otp for Consumer Loan Acceptance Phone    ${phoneNo}    ${otp}   ${loanuid}
    # Log  ${resp.content}
    # Should Be Equal As Strings     ${resp.status_code}    200


JD-TC-GenerateOtpforConsumerAcceptancePhone-3
    ...
    ${resp}=    Otp for Consumer Acceptance Phone    gegs     ${email2}   ${cc}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200

    # ${resp}=    Otp for Consumer Loan Acceptance Phone    4eyde    12    ${loanuid}
    # Log  ${resp.content}
    # Should Be Equal As Strings     ${resp.status_code}    200


JD-TC-GenerateOtpforConsumerAcceptancePhone-4
    ...
    ${resp}=    Otp for Consumer Acceptance Phone   ${phoneNo}    ${email2}     ${cc12}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-5
                                  
    [Documentation]              Generate Otp for Consumer Acceptance Phone  with consumer login
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-6
...
JD-TC-GenerateOtpforConsumerAcceptancePhone-7
                                  
    [Documentation]              Generate Otp for Consumer Acceptance Phone   without login
