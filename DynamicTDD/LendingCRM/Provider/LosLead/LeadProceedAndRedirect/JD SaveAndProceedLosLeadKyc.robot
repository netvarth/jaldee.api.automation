...
JD-TC-SaveAndProceedLosLeadKyc-1

    [Documentation]  Save And Proceed LOS Lead KYC -
...
JD-TC-SaveAndProceedLosLeadKyc-UH1

    [Documentation]  Save And Proceed LOS Lead KYC - which already Proceed

    ${resp}=   Encrypted Provider Login  ${PUSERNAME39}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${INVALID_LEAD_STAGE}=   Replace String  ${INVALID_LEAD_STAGE}  {}   ${Sname33}

    ${resp}=    Save And Proceed LOS Lead KYC  ${lead_uid}  ${stageuid11}  lead=${lead}  remarks=${remarks}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}     422
    Should Be Equal As Strings      ${resp.json()}        ${INVALID_LEAD_STAGE}


JD-TC-SaveAndProceedLosLeadKyc-UH2

    [Documentation]  Save And Proceed LOS Lead KYC - where lead uid is invalid
    ...
    ${resp}=    Save And Proceed LOS Lead KYC  ${inv}  ${stageuid22}
    Log  ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     422
    Should Be Equal As Strings    ${resp.json()}            ${INVALID_X_ID}


JD-TC-SaveAndProceedLosLeadKyc-UH3

    [Documentation]  Save And Proceed LOS Lead KYC - where stage uid is invalid
    ...
    ${resp}=    Save And Proceed LOS Lead KYC  ${lead_uid}  ${inv}
    Log  ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     422
    Should Be Equal As Strings    ${resp.json()}            ${INVALID_X_ID}


JD-TC-SaveAndProceedLosLeadKyc-UH4

    [Documentation]  Save And Proceed LOS Lead KYC - without login

    ${resp}=    Save And Proceed LOS Lead KYC  ${lead_uid}  ${stageuid22}
    Log  ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}   419
    Should Be Equal As Strings    ${resp.json()}        ${SESSION_EXPIRED}


JD-TC-SaveAndProceedLosLeadKyc-UH5

    [Documentation]  Save And Proceed LOS Lead KYC - with another provider login
