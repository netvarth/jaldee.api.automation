...
JD-TC-ChangeLeadProgress-1

    [Documentation]             Change Lead Progress
    ...
    ${P2name}=    FakerLibrary.name
    Set Suite Variable      ${P2name}

    ${resp}=    Create Lead Progress LOS  ${P2name}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Suite Variable      ${progress_id2}      ${resp.json()['id']}

    ${resp}=    Change Lead Progress LOS  ${lead_uid}  ${progress_id2} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-ChangeLeadProgress-UH1
    ...
    [Documentation]             Change Lead Progress where lead_uid is invalid 
    ...
    ${resp}=    Change Lead Progress LOS  ${fake}  ${progress_id2} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   422
    Should Be Equal As Strings    ${resp.json()}        ${INVALID_LEAD_ID}

JD-TC-ChangeLeadProgress-UH2

    [Documentation]             Change Lead Progress where progress is invalid 
    ...
    ${resp}=    Change Lead Progress LOS  ${lead_uid}  ${fake} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   422
    Should Be Equal As Strings    ${resp.json()}        ${Invalid_Progress_ID}

JD-TC-ChangeLeadProgress-UH3

    [Documentation]             Change Lead Progress without login
...
JD-TC-ChangeLeadProgress-UH4

    [Documentation]             Change Lead Progress using another provider lof=gin
