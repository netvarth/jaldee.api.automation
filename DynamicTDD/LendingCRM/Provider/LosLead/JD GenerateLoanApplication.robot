...
JD-TC-GenerateLoanApplication-1
                                  
    [Documentation]               Generate Loan Application
...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+3145752
...
# .... Create Branch1....
    ...
    FOR    ${i}    IN RANGE  0  3
        ${bpin}=  get_pincode
        ${kwstatus}  ${resp} =   Run Keyword And Ignore Error  Get LocationsByPincode  ${bpin}
        IF    '${kwstatus}' == 'FAIL'
                Continue For Loop
    ...
    Log  ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}    200
    Set Suite Variable  ${bcity}      ${resp.json()[0]['PostOffice'][0]['District']}   
    Set Suite Variable  ${bstate}     ${resp.json()[0]['PostOffice'][0]['State']}    
    Set Suite Variable  ${bdistrict}  ${resp.json()[0]['PostOffice'][0]['District']}   
    Set Suite Variable  ${bpin}       ${resp.json()[0]['PostOffice'][0]['Pincode']}
...
# ....... Partner Details .......
    ...
    ${resp}=    Partner Details            ${partuid1}    ${dealername}    ${phone}    ${email}    ${description}     ${Ptypeid}    ${Pcategoryid}    ${address1}    ${address2}    ${bpin}    ${partnerCity}    ${bdistrict}    ${bstate}    ${aadhaar}    ${pan}    ${gstin}    ${branch}    ${partnerAliasName}    ${aadhaarAttachments}    ${panAttachments}    ${gstAttachments}    ${licenceAttachments}    ${partnerAttachments}    ${storeAttachments}   
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200 
...
# ....... Login Sales Office and Create Lead .......
    ...
    ${resp}=    Generate Loan Application LOS  ${lead_uid}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Suite Variable            ${loan_id}            ${resp.json()['id']}       
    Set Suite Variable            ${loan_uid}           ${resp.json()['uid']}

JD-TC-GenerateLoanApplication-UH1
                                  
    [Documentation]               Generate Loan Application - invalid lead id
    ...
    ${resp}=    Generate Loan Application LOS  ${fake}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   422
    Should Be Equal As Strings    ${resp.json()}        ${INVALID_X_ID}

JD-TC-GenerateLoanApplication-UH2
                                  
    [Documentation]               Generate Loan Application - without login
...
JD-TC-GenerateLoanApplication-UH3
                                  
    [Documentation]               Generate Loan Application - another provider login
