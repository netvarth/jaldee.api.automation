...
JD-TC-GetOrderCountByCriteria-1
    ...
    Set Suite Variable  ${email_id}  ${firstname}${PUSERNAME162}.ynwtest@netvarth.com
    ...
    ${firstname}=  FakerLibrary.first_name
    Set Suite Variable  ${email}  ${firstname}${CUSERNAME24}.ynwtest@netvarth.com
    ${EMPTY_List}=  Create List
    Set Suite Variable  ${EMPTY_List}

    ${resp}=   Create Order For HomeDelivery  ${cookie}  ${accId}    ${self}    ${CatalogId1}   ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME24}    ${email}   ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order For Pickup   ${cookie}  ${accId}    ${mem_id}    ${CatalogId1}   ${bool[1]}  ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME24}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order For Pickup  ${cookie}  ${accId}    ${self}    ${CatalogId2}   ${bool[1]}  ${sTime1}    ${eTime1}   ${DAY2}    ${CUSERNAME24}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Get Consumer Order Count By Criteria  
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-2
    ...
    ${accId1}=  get_acc_id  ${PUSERNAME163}
    Set Test Variable  ${accId1} 

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME163}.ynwtest@netvarth.com
    ...
    ${item_quantity1}=  FakerLibrary.Random Int  min=${minQuantity}   max=${maxQuantity}
    ${firstname}=  FakerLibrary.first_name
    Set Test Variable  ${email}  ${firstname}${CUSERNAME24}.ynwtest@netvarth.com

    ${resp}=   Create Order For HomeDelivery   ${cookie}   ${accId1}    ${self}    ${CatalogId1}   ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME24}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Get Consumer Order Count By Criteria   account-eq=${accId}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  2
    
    ${resp}=   Get Consumer Order Count By Criteria   account-eq=${accId1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  1

JD-TC-GetOrderCountByCriteria-3
    ...
    ${resp}=   Get Consumer Order Count By Criteria   homeDelivery-eq=${bool[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  2
...
JD-TC-GetOrderCountByCriteria-4
    ...
    ${resp}=   Get Consumer Order Count By Criteria   storePickup-eq=${bool[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetOrderCountByCriteria-5
    ...
    ${resp}=   Get Consumer Order Count By Criteria   storePickup-eq=${bool[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  2
...
JD-TC-GetOrderCountByCriteria-6
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderStatus-eq=${orderStatuses[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}  3
...
JD-TC-GetOrderCountByCriteria-7
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderStatus-eq=${orderStatuses[2]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-8
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderDate-eq=${DAY1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()}   3
...
#     ${resp}=   Get Consumer Order Count By Criteria   orderDate-eq=${DAY2}
#     Log   ${resp.json()}
#     Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-10
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderNumber-eq=${order_no1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

   Should Be Equal As Strings    ${resp.json()}   1

    ${resp}=   Get Consumer Order Count By Criteria   orderNumber-eq=${order_no2}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings    ${resp.json()}   1

    ${resp}=   Get Consumer Order Count By Criteria   orderNumber-eq=${order_no4}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-11
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderNumber-eq=${order_no3}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-12
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderMode-eq=${order_mode[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-13
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderMode-eq=${order_mode[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-14
    ...
    ${resp}=   Get Consumer Order Count By Criteria   orderMode-eq=${order_mode[2]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-15
    ...
    ${resp}=   Get Consumer Order Count By Criteria   firstName-eq=${fname}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()}    2
...
JD-TC-GetOrderCountByCriteria-16
    ...
    ${resp}=   Get Consumer Order Count By Criteria   lastName-eq=${lname}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()}    2
...
JD-TC-GetOrderCountByCriteria-17
    ...
    ${resp}=   Get Consumer Order Count By Criteria   email-eq=${email}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetOrderCountByCriteria-18
    ...
    ${resp}=   Get Consumer Order Count By Criteria   phoneNumber-eq=${CUSERNAME24}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings    ${resp.json()}    3

JD-TC-GetOrderCountByCriteria-19
    ...
    ${resp}=   Get Consumer Order Count By Criteria   jaldeeId-eq=${jcons_id1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings    ${resp.json()}    2

JD-TC-GetOrderCountByCriteria-20
    ...
    ${resp}=   Get Consumer Order Count By Criteria   lastName-eq=${lname1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings    ${resp.json()}    1
  
JD-TC-GetOrderCountByCriteria-21
    ...
    ${resp}=   Get Consumer Order Count By Criteria   firstName-eq=${fname1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings    ${resp.json()}    1

JD-TC-GetOrderCountByCriteria-22
...
JD-TC-GetOrderCountByCriteria-23
    ...
    ${resp}=   Get Consumer Order Count By Criteria   account-eq=000
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
