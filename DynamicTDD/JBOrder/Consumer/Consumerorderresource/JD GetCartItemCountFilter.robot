...
JD-TC-Get Cart Item Count Filter-1
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${HLPUSERNAME38}
    ...
    ${PhoneNumber}=  Evaluate  ${PUSERNAME}+205187748
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    ${resp}=    Get Cart Item Count- Filter   providerConsumerId-eq=${cid}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
