...
JD-TC-Get Invoice Count Filter-1

    [Documentation]  Get Invoice using filters
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${HLPUSERNAME27}
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Suite Variable    ${invoiceNum}    ${resp.json()[0]['invoiceNum']}
    ...
    ${resp}=    Get invoice Count- Filter   providerconsumerId-eq=${cid}   invoiceNum-eq=${invoiceNum}   paymentStatus-eq= ${paymentStatus[0]}   orderUid-eq=${orderUid}    amountDue-eq=${Total}   netRate-eq=${Total}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200