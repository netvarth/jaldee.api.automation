...
JD-TC-Get sp item category Count Filter-1

    [Documentation]  Get item using category count filter using status
...
JD-TC-Get sp item category Count Filter-UH1

    [Documentation]  Get sp item category Count Filter using status as Element Should Be Disabled
...
JD-TC-Get sp item category Count Filter-UH2

    [Documentation]  Get sp item category Count Filter   using invalid account id

    ${resp}=    ProviderConsumer Login with token   ${primaryMobileNo}    ${accountId}  ${token} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Get sp item category Count Filter      ${primaryMobileNo}   status-eq=${toggle[1]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()}  0

JD-TC-Get sp item category Count Filter-3

    [Documentation]  create 3 item with category and filter
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${PUSERNAME106}
    ...
    ${resp}=  Create Item Type   ${categoryName}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Test Variable      ${typeCode1}     ${resp.json()}

    ${resp}=  Create Item Type   ${Name}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Test Variable      ${typeCode2}     ${resp.json()}

    # ...... Create Category .......
    ${resp}=  Create Item Category   ${TypeName}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Test Variable      ${categoryCode1}     ${resp.json()}

    ${resp}=  Create Item Category   ${Name}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Test Variable      ${categoryCode2}     ${resp.json()}
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()[0]['categoryCode']}                                              ${categoryCode2}
    Should Be Equal As Strings    ${resp.json()[0]['categoryName']}                                            ${Name}
    Should Be Equal As Strings    ${resp.json()[0]['status']}                                                      ${toggle[0]}
    Should Be Equal As Strings    ${resp.json()[1]['categoryCode']}                                              ${categoryCode1}
    Should Be Equal As Strings    ${resp.json()[1]['categoryName']}                                            ${TypeName}
    Should Be Equal As Strings    ${resp.json()[1]['status']}                                                      ${toggle[0]}
    Should Be Equal As Strings    ${resp.json()[2]['categoryCode']}                                              ${categoryCode}
...
JD-TC-Get sp item category Count Filter-4

    [Documentation]  In the sales order catalog where inventory manager is on ,then Get sp item category Count Filter
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${PUSERNAME105}
    Set Test Variable    ${accountId} 
