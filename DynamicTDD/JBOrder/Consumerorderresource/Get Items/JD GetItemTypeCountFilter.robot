...
JD-TC-Get sp item type Count Filter-1
...
# -------------------------------- Create store type -----------------------------------
    ...
    ${resp}=  Create SalesOrder Inventory Catalog-InvMgr False   ${store_id}   ${Name}  ${boolean[1]}   inventoryCatalog=${inv_cat_encid}  onlineSelfOrder=${boolean[1]}  walkInOrder=${boolean[0]}   storePickup=${boolean[1]}  homeDelivery=${boolean[1]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    ${resp}=    Get sp item type Count Filter     ${accountId}   status-eq=${toggle[0]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${len}    1

JD-TC-Get sp item type Count Filter-2

    [Documentation]  Get sp item type count filter using status as Element Should Be Disabled
...
JD-TC-Get sp item type Count Filter-UH1

    [Documentation]  Get sp item type Count Filter  using invalid account id
...
JD-TC-Get sp item type Count Filter-3
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${PUSERNAME100}
...
JD-TC-Get sp item type Count Filter-4
...
# -------------------------------- Create store type -----------------------------------
    ...
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${accountId}=  get_acc_id  ${PUSERNAME101}
    Set Test Variable    ${accountId} 
