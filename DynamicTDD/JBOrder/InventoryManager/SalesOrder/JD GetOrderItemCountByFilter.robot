...
JD-TC-Get Order Item Count By Filter-2
    ...
    ${resp}=    Get Order Item count By Filter        sorderUid-eq=${SO_Uid}   
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Should Be Equal As Strings    ${resp.json()}              ${s_len}  
...
JD-TC-Get Order Item Count By Filter-4
    [Documentation]   Try to get sales order item count with sorderCatalogEncId param.

    ${resp}=  Encrypted Provider Login  ${HLPUSERNAME8}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Get Order Item count By Filter        sorderCatalogEncId-eq=${SO_Cata_Encid} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Should Be Equal As Strings    ${resp.json()}              ${s_len} 
...
JD-TC-Get Order Item Count By Filter-5
    ...
    ${resp}=    Get Order Item count By Filter        storeId-eq=${Stidd}  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Should Be Equal As Strings    ${resp.json()}              ${s_len} 

JD-TC-Get Order Item Count By Filter-6
    [Documentation]   Try to get sales order item count with sorderCatalogItemId param.

    ${resp}=  Encrypted Provider Login  ${HLPUSERNAME8}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Get Order Item count By Filter        sorderCatalogItemId-eq=${SO_itemEncIds}  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-Get Order Item Count By Filter-7
    ...
    ${resp}=    Get Order Item count By Filter        sorderCatalogItemEncId-eq=${SO_itemEncIds}  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Should Be Equal As Strings    ${resp.json()}              ${s_len} 
