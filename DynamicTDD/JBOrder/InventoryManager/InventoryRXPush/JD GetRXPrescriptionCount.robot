...
JD-TC-GetRXPrescriptionCount-1
    ...
    ${PUSERNAME_E}=  Evaluate  ${PUSERNAME}+35788754
...
# ------------------------------Create SalesOrder Catalog Item-invMgmt True-------------------------------
    ...
    ${frequency}=       Random Int  min=66  max=70
    ...
    ${resp}=    RX Update Prescription Item  ${pitm_id}  ${displayName1}  ${duration1}  ${quantity1}  ${description}  ${item1}  ${dos}  ${frequency_id}  ${prescription_id}   itemDosage=${dos}
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}             200
