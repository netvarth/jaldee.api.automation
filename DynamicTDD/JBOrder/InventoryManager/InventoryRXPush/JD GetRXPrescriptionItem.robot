...
JD-TC-GetRXPrescriptionItem-1
    ...
    ${PUSERNAME_E}=  Evaluate  ${PUSERNAME}+35789754
...
# ------------------------------Create SalesOrder Catalog Item-invMgmt True-------------------------------
    ...
    ${frequency}=       Random Int  min=71  max=75
    ...
    ${resp}=    RX Create Prescription Item  ${displayName1}  ${duration1}  ${quantity1}  ${description}  ${item1}  ${dos}  ${frequency_id}  ${prescription_id}   itemDosage=${dos}
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}             200
