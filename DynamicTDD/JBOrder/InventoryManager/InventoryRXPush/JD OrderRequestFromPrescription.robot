...
JD-TC-OrderRequest-UH5
    ...
    ${FIELD_CANT_BE_EMPTY}=  format String   ${FIELD_CANT_BE_EMPTY}   Prescription items
...
JD-TC-OrderRequest-UH7
    [Documentation]    "Order Request: If the same request is declined,  try to place the order again with the declined request."
    ...
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}             200
    Set Suite Variable      ${sorder_uid1}   ${resp.json()[0]['uid']}

    ${resp}=    Update SOrder Status  ${sorder_uid1}  ${pushedStatus[2]}
    Log  ${resp.json()}         
    Should Be Equal As Strings            ${resp.status_code}    200
