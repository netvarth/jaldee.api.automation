...
# ------------------------------Create SalesOrder Catalog Item-invMgmt True-------------------------------
    ...
    ${quantity1}=        Random Int  min=1  max=10
    ${medicineName1}=    FakerLibrary.firstName

    ${resp}=    RX Create Prescription  ${cid}  ${doc1}  ${medicineName1}  ${duration1}  ${quantity1}  ${description}  ${item1}  ${dos}  ${frequency_id}  ${html}
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     200
    ...
    Should Be Equal As Strings      ${resp.json()['mrPrescriptionItemsDtos'][0]['medicineName']}        ${medicineName1}
    ...
    ${resp}=    RX Update Prescription  ${prescription_id}  ${cid}  ${doc1}  ${medicineName2}  ${duration}  ${quantity}  ${description}  ${item1}  ${dos}  ${frequency_id}  ${html}
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     200
