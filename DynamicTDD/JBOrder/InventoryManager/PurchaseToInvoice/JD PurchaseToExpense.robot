...
JD-TC-Purchase To Expense-1
    ...
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}                 200
    Set Suite Variable              ${netTotal}           ${resp.json()['totalNetRate']}
...
JD-TC-Purchase To Expense-2
    ...
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}                 200
    Set Test Variable              ${netTotal}           ${resp.json()['totalNetRate']}
...
JD-TC-Purchase To Expense-UH2
    ...
    ${PURCHASE_CANT_PUSHE_TO_FINANCE_INVALID_STATUS}=        format String   ${PURCHASE_CANT_PUSHE_TO_FINANCE_INVALID_STATUS}   ${PurchaseStatus[1]}
    ${resp}=    Purchase Push to Finance    ${purchaseId}    
    Log   ${resp.content}
...
JD-TC-Purchase To Expense-UH3
    ...
    ${PURCHASE_CANT_PUSHE_TO_FINANCE_INVALID_STATUS}=        format String   ${PURCHASE_CANT_PUSHE_TO_FINANCE_INVALID_STATUS}   ${PurchaseStatus[0]}
    ${resp}=    Purchase Push to Finance    ${purchaseId}    
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}                     422
    Should Be Equal As Strings      ${resp.json()}             ${PURCHASE_CANT_PUSHE_TO_FINANCE_INVALID_STATUS}
...
JD-TC-Purchase To Expense-UH4
    ...
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}                     422
    Should Be Equal As Strings      ${resp.json()}             ${ALREADY_PUSHED_TO_FINANCE}
  


JD-TC-Purchase To Expense-3

    [Documentation]  Push purchase to finance field is automatic.account doesnot have tax then check expense
    ...
    # ............... Create item ...............
    ${resp}=    Create Item Inventory  ${name}  shortDesc=${shortDesc}   internalDesc=${internalDesc}      categoryCode=${categoryCode}  categoryCode2=${categoryCode}  typeCode=${typeCode}  typeCode2=${typeCode}  hsnCode=${hsnCode}  manufacturerCode=${manufacturerCode}  sku=${sku}  isBatchApplicable=${boolean[1]}  isInventoryItem=${boolean[1]}  itemGroups=${itemGroups}  itemSubGroups=${itemGroups}    composition=${composition}  itemUnits=${itemUnits}  attachments=${attachments}     
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}    200
    Set Test Variable              ${itemEncId1}  ${resp.json()}
    ...
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     200
    Set Suite Variable              ${purchaseId3}           ${resp.json()}

    ${resp}=    Get Purchase By Uid  ${purchaseId3} 
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}                 200
  
    # .... Update Purchase status as In review ......
    ${resp}=    Update Purchase Status  ${PurchaseStatus[1]}  ${purchaseId3} 
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     200
    ...
    ${resp}=    Update Purchase Status  ${PurchaseStatus[2]}  ${purchaseId3} 
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}     200
...
JD-TC-Purchase To Expense-UH5
    ...
    [Documentation]  Push purchase to finance field is automatic.then try to push purchase manually
    ${resp}=  Encrypted Provider Login  ${HLPUSERNAME36}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200


    ${resp}=    Purchase Push to Finance    ${purchaseId3}    
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}                     422
    Should Be Equal As Strings      ${resp.json()}             ${ALREADY_PUSHED_TO_FINANCE}
