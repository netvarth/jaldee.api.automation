...
JD-TC-Update Cash Payment For Sales Order-2
    ...
    ${half_Total}=  Evaluate  ${netTotal}/2
    ${half_Total}=  Convert To Number  ${half_Total}   1

    ${note}=  FakerLibrary.name

    ${resp}=  Update cash payment- Sales Order   ${SO_Inv}  ${acceptPaymentBy[0]}  ${half_Total}  ${note}  ${paymentRefId}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
