...
# --------------------------- Create SalesOrder Inventory Catalog-InvMgr True --------------------------
    ...
    ${resp}=  Create SalesOrder Inventory Catalog-InvMgr True   ${store_id}  ${Store_name}  ${boolean[1]}  ${inv_cat_encid_List}      onlineSelfOrder=${boolean[1]}  walkInOrder=${boolean[1]}  storePickup=${boolean[1]}  courierService=${boolean[0]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
# ----------------------------------------- Take sales order ------------------------------------------------
   ...
   ${note}=  FakerLibrary.name
    ${resp}=   Create Catalog Item Batch-invMgmt False   ${SO_itemEncIds}     ${note}     ${price}   inventoryItemBatch=${encidd}    
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
