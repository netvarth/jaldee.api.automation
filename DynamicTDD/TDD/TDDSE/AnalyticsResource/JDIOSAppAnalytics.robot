...
JD-TC-iosLevelAnalytics-1

    [Documentation]   take ONLINE_TOKEN for a provider through CONSUMER_APP and check account level analytics for IOS_TOKENS and WEB_TOKENS.
    ...
    ${resp}=  Get Account Level Analytics  ${tokenAnalyticsMetrics['IOS_TOKENS']}  ${DAY1}  ${DAY1}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['value']}     ${ios_token_len}
...
JD-TC-iosLevelAnalytics-2

    [Documentation]   take WALK_IN_TOKEN for a provider through SP_APP and check account level analytics for IOS_TOKENS and WEB_TOKENS.
    ...
    Set Suite Variable   ${ios_walkin_token_ids}
    ...
    Set Suite Variable   ${ios_walkin_token_len}
    ...
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['value']}     ${ios_walkin_token_len}
    ...
    ${resp}=   Get Account Level Analytics  ${tokenAnalyticsMetrics['WEB_TOKENS']}  ${DAY1}  ${DAY2}  ${analyticsFrequency[0]}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}       ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues']}    []
...
JD-TC-iosLevelAnalytics-3

    [Documentation]   take PHONE_TOKEN for a provider through SP_APP and check account level analytics for IOS_TOKENS and WEB_TOKENS.
    
    ${resp}=   App ProviderLogin  ${ios_sp_headers}  ${PUSERPH0}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${ios_phonein_token_ids}=  Create List
    Set Suite Variable   ${ios_phonein_token_ids}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        ${desc}=   FakerLibrary.word
        ${resp}=  App Add To Waitlist with mode  ${ios_sp_headers}  ${waitlistMode[2]}  ${cid${a}}  ${s_id}  ${q_id1}  ${DAY2}  ${desc}  ${bool[1]}  ${cid${a}} 
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ${wid}=  Get Dictionary Values  ${resp.json()}
        Set Test Variable  ${wid${a}}  ${wid[0]}

        Append To List   ${ios_phonein_token_ids}  ${wid${a}}
    ...
    Log List   ${ios_phonein_token_ids}

    ${ios_phonein_token_len}=   Evaluate  len($ios_phonein_token_ids) + $ios_walkin_token_len
    Set Suite Variable   ${ios_phonein_token_len}
    ...
    Should Be Equal As Strings  ${resp.status_code}  200  
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['frequency']}                    ${analyticsFrequency[0]}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['value']}     ${ios_phonein_token_len}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['amount']}    ${def_amt}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['metricId']}  ${tokenAnalyticsMetrics['IOS_TOKENS']}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][0]['dateFor']}   ${DAY2}
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.json()['metricValues'][1]['value']}     ${ios_walkin_token_len}
