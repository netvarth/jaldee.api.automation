...
JD-TC-Get reimbursement byInvoiceId-1
    ...
    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+45223
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Suite Variable  ${totalAmount}   ${resp.json()['totalAmount']}
    ...
    Should Be Equal As Strings  ${resp.json()['service'][0]['price']}  ${totalAmount}
    ...
    Should Be Equal As Strings  ${resp.json()['netTotal']}  ${totalAmount}
    Should Be Equal As Strings  ${resp.json()['netRate']}  ${totalAmount}
    Should Be Equal As Strings  ${resp.json()['amountDue']}  ${totalAmount}
...
JD-TC-Get reimbursement byInvoiceId -UH1
    [Documentation]  Get reimbursement byInvoiceId without login      
    ${resp}=  Get Reimbursement By InvoiceId  ${invoice_id}
    Should Be Equal As Strings    ${resp.status_code}   419
    Should Be Equal As Strings   "${resp.json()}"   "${SA_SESSION_EXPIRED}"
    
JD-TC-Get reimbursement byInvoiceId -UH2
    [Documentation]   Consumer do Get reimbursement byInvoiceId
...
JD-TC-Get reimbursement byInvoiceId -UH3
    [Documentation]   Provider do Get reimbursement byInvoiceId
    ${resp}=   Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD} 
    Should Be Equal As Strings    ${resp.status_code}   200
    ${resp}=  Get Reimbursement By InvoiceId  ${invoice_id}
    Should Be Equal As Strings    ${resp.status_code}   419
...
# ------------------ Update notes to a reimbursement report --------------------------------------

JD-TC-UpdateReimburseReportByInvoiceId-2
    [Documentation]  Provider apply a coupon after waitlist ,done payment and settil bill,create report then get reprt by invoice_id   and update invoice with a note. 
    ...
    ${resp}=  Update Reimbursement By InvoiceId      ${invoice_id}  ${note}
    Log    ${resp.json()}    
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Should Be Equal As Strings  ${resp.json()[0]['paidBy']['id']}  2
    Should Be Equal As Strings  ${resp.json()[0]['paidBy']['userName']}  ${uname}
    Should Be Equal As Strings  ${resp.json()[0]['notes']}  ${note}

JD-TC-UpdateReimburseReportByInvoiceId-4
    [Documentation]  update note again.
    ...
    ${resp}=  Update Reimbursement By InvoiceId      ${invoice_id}  ${note1}
    Log    ${resp.json()}    
    Should Be Equal As Strings  ${resp.status_code}  200
