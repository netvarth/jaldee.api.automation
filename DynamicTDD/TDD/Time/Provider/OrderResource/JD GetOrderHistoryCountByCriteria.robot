...
JD-TC-Getorderhistorycountbycriteria-1
    ...
    ${cookie2}  ${resp}=  Imageupload.conLogin  ${CUSERNAME2}   ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings   ${resp.status_code}    200

    ${resp}=   Create Order For HomeDelivery    ${cookie2}  ${accId}    ${self}    ${CatalogId1}     ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME2}    ${email0}  ${countryCodes[0]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order For HomeDelivery    ${cookie2}  ${accId}    ${self}    ${CatalogId2}     ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME2}    ${email0}  ${countryCodes[0]}  ${EMPTY_List}  ${item_id2}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${item_quantity1}=  FakerLibrary.Random Int  min=${minQuantity}   max=${maxQuantity}
    ${firstname}=  FakerLibrary.first_name
    Set Suite Variable  ${email2}  ${firstname}${CUSERNAME2}.${test_mail}

    ${resp}=   Create Order For HomeDelivery    ${cookie2}  ${accId}    ${self}    ${CatalogId2}     ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME2}    ${email2}  ${countryCodes[0]}  ${EMPTY_List}  ${item_id2}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${cookie3}  ${resp}=  Imageupload.conLogin  ${CUSERNAME3}   ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings   ${resp.status_code}    200

    ${resp}=   Create Order For HomeDelivery    ${cookie3}  ${accId}    ${self}    ${CatalogId1}     ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME3}    ${email}  ${countryCodes[0]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order For HomeDelivery    ${cookie3}  ${accId}    ${self}    ${CatalogId2}     ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME3}    ${email}  ${countryCodes[0]}  ${EMPTY_List}  ${item_id2}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=  Get Order History count by Criterias   orderDate-eq=${DAY1} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Should Be Equal As Integers  ${resp.json()}  5
         
    
JD-TC-Getorderhistorycountbycriteria-2
    ...
    ${resp}=  Get Order History count by Criterias   homeDelivery-eq=${bool[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Should Be Equal As Integers  ${resp.json()}  5
         
JD-TC-Getorderhistorycountbycriteria-3
    ...
    ${resp}=  Get Order History count by Criterias   homeDelivery-eq=${bool[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Should Be Equal As Strings    ${resp.json()}    0
 
    
JD-TC-Getorderhistorycountbycriteria-4
    ...
    ${resp}=  Get Order History count by Criterias   firstName-eq=${fname}
    Log  ${resp.json()}
    Should Be Equal As Integers  ${resp.json()}  3
         
JD-TC-Getorderhistorycountbycriteria-5
    ...
    ${resp}=  Get Order History count by Criterias   lastName-eq=${lname1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  2
         
JD-TC-Getorderhistorycountbycriteria-6
    [Documentation]    Get order By email
    ${resp}=  Encrypted Provider Login  ${PUSERNAME108}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Get Order History count by Criterias   email-eq=${email0}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  2
         
    
JD-TC-Getorderhistorycountbycriteria-7
    ...
    ${resp}=  Get Order History count by Criterias   phoneNumber-eq=${CUSERNAME3}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  2
         
    
JD-TC-Getorderhistorycountbycriteria-8
    ...
    ${resp}=  Get Order History count by Criterias   jaldeeId-eq=${jcons_id1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  3
         
# JD-TC-Getorderhistorycountbycriteria-9
#     [Documentation]    Get order By jaldeeconsumer
#     ${resp}=  Encrypted Provider Login  ${PUSERNAME108}  ${PASSWORD}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200

#     ${resp}=  Get Order History count by Criterias   jaldeeConsumer-eq=${jdconID}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200 
#     Should Be Equal As Integers  ${resp.json()}  2
         
JD-TC-Getorderhistorycountbycriteria-10
    ...
    ${resp}=  Get Order History count by Criterias   orderMode-eq=${order_mode[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Should Be Equal As Integers  ${resp.json()}  5
         
JD-TC-Getorderhistorycountbycriteria-11
    ...
    ${resp}=  Get Order History count by Criterias   orderNumber-eq=${ordernumber1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  1
         
JD-TC-Getorderhistorycountbycriteria-12
    ...
    ${resp}=  Get Order History count by Criterias   uid-eq=${orderid4}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Integers  ${resp.json()}  1
         
    
JD-TC-Getorderhistorycountbycriteria-13
    [Documentation]    Get order By storepickup is false

    ${resp}=  Encrypted Provider Login  ${PUSERNAME108}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Get Order History count by Criterias  storePickup-eq=${bool[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Integers  ${resp.json()}  5
         
JD-TC-Getorderhistorycountbycriteria-14
    [Documentation]    Get order By order status is order recived

    ${resp}=  Encrypted Provider Login  ${PUSERNAME108}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Get Order History count by Criterias  orderStatus-eq=${orderStatuses[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  5
         

JD-TC-Getorderhistorycountbycriteria-15
    ...
    ${cookie20}  ${resp}=  Imageupload.conLogin  ${CUSERNAME20}   ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings   ${resp.status_code}    200

    ${resp}=   Create Order For HomeDelivery    ${cookie20}  ${accId3}    ${self}    ${CatalogId3}     ${bool[1]}    ${address}    ${sTime3}    ${eTime3}   ${DAY1}    ${CUSERNAME20}    ${email}  ${countryCodes[0]}  ${EMPTY_List}  ${item_id3}    ${item_quantity1}  ${item_id4}    ${item_quantity1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-Getorderhistorycountbycriteria-16
    ...
    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME146}.${test_mail}
    ...
    ${StatusList1}=  Create List  ${orderStatuses[0]}  ${orderStatuses[1]}   ${orderStatuses[3]}  ${orderStatuses[11]}   ${orderStatuses[12]}
    ...
    ${resp}=  Create Catalog For ShoppingCart   ${catalogName}  ${catalogDesc}   ${catalogSchedule}   ${orderType}   ${paymentType}   ${StatusList1}   ${catalogItem}   ${minNumberItem}   ${maxNumberItem}    ${cancelationPolicy}   catalogStatus=${catalogStatus1}   pickUp=${pickUp}   homeDelivery=${homeDelivery}   showPrice=${boolean[1]}   advanceAmount=${advanceAmount}   showContactInfo=${boolean[1]}   howFar=${far}   howSoon=${soon}   preInfo=${preInfo}   postInfo=${postInfo}    
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Get Order History count by Criterias  storePickup-eq=${bool[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  1
...
JD-TC-Getorderhistorycountbycriteria-17
...
JD-TC-Getorderhistorycountbycriteria-18
    ...
    ${resp}=   Create Order By Provider For HomeDelivery    ${cid20}   ${cid20}   ${CatalogId1}   ${boolean[1]}   ${address}  ${sTime3}    ${eTime3}   ${DAY1}    ${CUSERNAME20}    ${email}  ${orderNote}  ${countryCode[0]}  ${EMPTY_List}  ${item_id3}   ${item_quantity1}  ${item_id4}   ${item_quantity1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=  Get Order History count by Criterias  uid-eq=${orderid1} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Should Be Equal As Integers  ${resp.json()}  1
