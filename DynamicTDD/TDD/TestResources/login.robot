...
JD-TC-UpdateBProfile
    ...
    Log    ${locid}
    # ${loc_data}=  Set Variable  ${resp.json()['location']}
    # # Log  ${loc_data}
    # # # ${loc_data}=  json.dumps  ${resp.json()['location']}
    # # ${loctype} =    Evaluate    type($loc_data).__name__
    # # ${parsed_data}=  Evaluate  json.loads('''${loc_data}''')  json
    # ${parsed_data}=    Evaluate    json.loads('${loc_data}')    json
    # Log  ${parsed_data}
    # ${locid}=    Set Variable    ${parsed_data[0]['id']}
    # Log    ${locid}
    # # ${locid}=  Evaluate  ${parsed_data}[0]["id"]
    # # ${locid}=  Set Variable  ${parsed_data}[0]['id']
    # # ${locid}=   Set Variable  ${loc_data[0]['id']}
    # # ${locid}=   Set Variable  ${resp.json()['location'][0]['id']}
