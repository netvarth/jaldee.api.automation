...
JD-TC-Getorderbycriteria-1
    ...
    ${resp}=  Get Order By Criterias   orderDate-eq=${DAY1} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    Verify Response List    ${resp}  4    uid=${orderid5}  
...
JD-TC-Getorderbycriteria-4
    ...
    ${resp}=  Get Order By Criterias   firstName-eq=${fname}
    Log  ${resp.json()}
    Verify Response List    ${resp}  0    uid=${orderid1}  
    Verify Response List    ${resp}  1    uid=${orderid2}
    Verify Response List    ${resp}  2    uid=${orderid3}   
...
JD-TC-Getorderbycriteria-5
    ...
    ${resp}=  Get Order By Criterias   lastName-eq=${lname1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
...
JD-TC-Getorderbycriteria-6
    ...
    ${resp}=  Get Order By Criterias   email-eq=${email0}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid1}  
    Verify Response List    ${resp}  1    uid=${orderid2}
...
JD-TC-Getorderbycriteria-7
    ...
    ${resp}=  Get Order By Criterias   phoneNumber-eq=${CUSERNAME3}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid4}  
    Verify Response List    ${resp}  1    uid=${orderid5}
...
JD-TC-Getorderbycriteria-8
    ...
    ${resp}=  Get Order By Criterias   jaldeeId-eq=${jcons_id1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid1}  
    Verify Response List    ${resp}  1    uid=${orderid2}
...
JD-TC-Getorderbycriteria-9
    ...
    ${resp}=  Get Order By Criterias   jaldeeConsumer-eq=${jdconID}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid4}  
    Verify Response List    ${resp}  1    uid=${orderid5}
...
JD-TC-Getorderbycriteria-10
    ...
    ${resp}=  Get Order By Criterias   orderMode-eq=${order_mode[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1}  
    Verify Response List    ${resp}  1    uid=${orderid2}
...
JD-TC-Getorderbycriteria-11
    ...
    ${resp}=  Get Order By Criterias   orderNumber-eq=${ordernumber1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid1}  
...
JD-TC-Getorderbycriteria-12
    ...
    ${resp}=  Get Order By Criterias   uid-eq=${orderid4}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response List    ${resp}  0    uid=${orderid4}  
...
JD-TC-Getorderbycriteria-13
    ...
    ${resp}=  Get Order By Criterias   storePickup-eq=${bool[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response List    ${resp}  0    uid=${orderid1}  
    Verify Response List    ${resp}  1    uid=${orderid2}
...
JD-TC-Getorderbycriteria-15
    [Documentation]    Update Delivery Charge By provider for home delivery.check bill after order taxable and non taxable items.then get by day and order number
    ...
    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME123}.${test_mail}
    ...
    ${resp}=  Get Order By Criterias   uid-eq=${orderid7} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid7}  
...
JD-TC-Getorderbycriteria-16
    ...
    ${resp}=  Get Order By Criterias   uid-eq=${orderid6}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
...
JD-TC-Getorderbycriteria-17
    ...
    ${resp}=  Get Order By Criterias   orderStatus-eq=${StatusList[4]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid2}     
    ...
    Verify Response List    ${resp}  0    uid=${orderid2}
    # Verify Response List    ${resp}  1    uid=${orderid6}  
    Should Be Equal As Strings  ${resp.json()[0]['orderStatus']}        ${orderStatuses[12]}
...
JD-TC-Getorderbycriteria-18
    ...
    ${startDate2}=  db.add_timezone_date  ${tz1}  5  
    ${endDate2}=  db.add_timezone_date  ${tz1}  25      

    ${sTime3}=  add_timezone_time  ${tz1}  0  15  

    ${eTime3}=  add_timezone_time  ${tz1}  1  00   
    ...
    ${resp}=   Create Order By Provider For HomeDelivery    ${cookie}  ${cid20}   ${cid20}   ${CatalogId1}   ${boolean[1]}   ${address}  ${sTime3}    ${eTime3}   ${DAY1}    ${CUSERNAME27}    ${email}  ${orderNote}  ${countryCodes[1]}  ${item_id3}   ${item_quantity1}  ${item_id4}   ${item_quantity1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=  Get Order By Criterias   orderNumber-eq=${ordernumber} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1}   

    ${resp}=  Get Order By Criterias   uid-eq=${orderid1} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1}   
...
JD-TC-Getorderbycriteria-UH1
    [Documentation]    Get order By order status packing

    ${resp}=  Encrypted Provider Login  ${PUSERNAME108}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Get Order By Criterias   orderStatus-eq=${orderStatuses[4]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}    []
  
    
JD-TC-Getorderbycriteria-UH2
...
JD-TC-Getorderbycriteria-UH3
    [Documentation]  Get order  with consumer login
    ${resp}=  Consumer Login  ${CUSERNAME2}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-Getorderbycriteria-UH4
    [Documentation]    Get order By using another provider orderid
    ${resp}=  Encrypted Provider Login  ${PUSERNAME108}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
