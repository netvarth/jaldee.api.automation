...
JD-TC-Get Catalog Batch Filter-2
    ...
    ${resp}=   Get Catalog Item Batch List   sorderCatalogItemEncId-eq=${SO_itemEncIds}   price-eq=${price1}    
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${netRate}=  Convert To Number  ${resp.json()[0]['price']}    1
    Should Be Equal As Strings    ${netRate}    ${price1}
...
JD-TC-Get Catalog Batch Filter-6
    ...
    Should Be Equal As Strings    ${resp.json()[1]['spItem']['name']}    ${name}
...
#     ${resp}=    Create Item Inventory  ${name}   isBatchApplicable=${boolean[1]}   
#     Log   ${resp.json()}
#     Should Be Equal As Strings    ${resp.status_code}    200
#     Set Suite Variable  ${itemEncId1}  ${resp.json()}
