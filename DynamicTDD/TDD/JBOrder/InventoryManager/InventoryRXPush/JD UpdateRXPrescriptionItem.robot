...
JD-TC-UpdateRXPrescriptionItem-1
    ...
    ${PUSERNAME_E}=  Evaluate  ${PUSERNAME}+45789753
...
# ------------------------------Create SalesOrder Catalog Item-invMgmt True-------------------------------
    ...
    ${frequency}=       Random Int  min=111  max=115
    ...
    ${resp}=    RX Create Prescription Item  ${displayName1}  ${duration1}  ${quantity1}  ${description}  ${item1}  ${dos}  ${frequency_id}  ${prescription_id}
    Log   ${resp.content}
    Should Be Equal As Strings      ${resp.status_code}             200
