...
JD-TC-GetInvAuditlogById-1
    ...
    ${Catalog_Name1}=    FakerLibrary.last name
    Set Suite Variable  ${Catalog_Name1}

    ${resp}=  Create Inventory Catalog   ${Catalog_Name1}  ${store_id}   
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()[0]['uid']}       ${encid1}
    ...
    Should Be Equal As Strings  ${resp.json()[0]['description']}    Inventory Catalog Created ${Catalog_Name1}
    ...
    Set Suite Variable  ${invCatalog_id1}  ${resp.json()[0]['id']}



    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${encid}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[2]} 
    Should Be Equal As Strings  ${resp.json()['auditContext']}       ${InventoryAuditContext[3]} 
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[0]} 
    Should Be Equal As Strings  ${resp.json()['subject']}    Inventory Catalog Created
    Should Be Equal As Strings  ${resp.json()['description']}    Inventory Catalog Created ${Catalog_Name}
...
JD-TC-GetInvAuditlogById-2
    ...
    ${resp}=  Update Inventory Catalog   ${Catalog_Name2}  ${store_id}   ${encid1}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id2}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id2}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${encid1}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[2]} 
    Should Be Equal As Strings  ${resp.json()['auditContext']}       ${InventoryAuditContext[3]} 
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[1]} 
    Should Be Equal As Strings  ${resp.json()['subject']}    Inventory Catalog updated
    Should Be Equal As Strings  ${resp.json()['description']}    Inventory Catalog updated ${Catalog_Name2}
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}
...
JD-TC-GetInvAuditlogById-3
    ...
    ${resp}=  Update Inventory Catalog status   ${encid1}  ${InventoryCatalogStatus[1]}   
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id3}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id3}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${encid1}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[2]} 
    Should Be Equal As Strings  ${resp.json()['auditContext']}       ${InventoryAuditContext[3]} 
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[3]} 
    Should Be Equal As Strings  ${resp.json()['subject']}    Inventory Catalog status updated
    Should Be Equal As Strings  ${resp.json()['description']}    Inventory Catalog ${Catalog_Name2} status updated to ${InventoryCatalogStatus[1]}
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}
...
JD-TC-GetInvAuditlogById-4
    ...
    ${resp}=  Update Inventory Catalog status   ${encid1}  ${InventoryCatalogStatus[0]}   
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id4}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id4}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${purchaseId}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[0]} 
    Should Be Equal As Strings  ${resp.json()['auditContext']}       ${InventoryAuditContext[0]} 
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[0]} 
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['subject']}    Purchase created
    Should Be Equal As Strings  ${resp.json()['description']}    Purchase created for ${vender_name} 
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}
...
JD-TC-GetInvAuditlogById-5
    ...
    # ${purchaseItemDtoList1}=        Create purchaseItemDtoList  ${ic_id}  ${quantity}  ${freeQuantity}   ${amount}  ${discountAmount}  ${discountPercentage}  500  ${taxableAmount}  ${taxAmount}  ${netTotal}   ${expiryDate}  ${mrp}  ${batchNo}  ${cgst}  ${sgst}  ${iu_id}  encId=${purchase_EncId}
    ...
    Should Be Equal As Strings      ${resp.json()['inventoryCatalog']['encId']}      ${encid1}
    Should Be Equal As Strings      ${resp.json()['uid']}      ${purchaseId}
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id5}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id5}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${purchaseId}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[0]} 
    ...
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[1]} 
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['subject']}    Purchase updated
    Should Be Equal As Strings  ${resp.json()['description']}    Purchase updated for ${vender_name} 
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}
...
JD-TC-GetInvAuditlogById-6
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id6}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id6}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${purchaseId}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[0]} 
    ...
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[3]} 
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['subject']}    Purchase status updated       
    # Should Be Equal As Strings  ${resp.json()['description']}    Purchase ${purchaseReferenceNo} status updated to ${PurchaseStatus[1]}  ignore_case=True
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}   
...
JD-TC-GetInvAuditlogById-7
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id7}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id7}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${remarks_encid}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[3]} 
    # Should Be Equal As Strings  ${resp.json()['auditContext']}       ${InventoryAuditContext} 
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[0]} 
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['subject']}    Remark Created
    Should Be Equal As Strings  ${resp.json()['description']}    Remark Created
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}   
    Should Contain              ${resp.json()['dateTime']}   ${datetime01} 
...
JD-TC-GetInvAuditlogById-8
    ...
    ${quantity}=  Convert To Number  ${quantity}  1
    ${invCatalog}=  Create Dictionary   encId=${encid1} 
    ...
    ${resp}=  Create Stock Adjustment   ${locId1}  ${store_id}   ${encid1}   ${remarks_encid}      ${data} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id8}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id8}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${Stock_uid}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[1]} 
    Should Be Equal As Strings  ${resp.json()['auditContext']}       ${InventoryAuditContext[2]} 
    Should Be Equal As Strings  ${resp.json()['auditLogAction']}       ${InventoryAuditLogAction[0]} 
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['subject']}    Stock Adjusted
    Should Be Equal As Strings  ${resp.json()['description']}    Stock adjustment created 
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}   
    Should Contain              ${resp.json()['dateTime']}   ${datetime01}  
...
JD-TC-GetInvAuditlogById-9
    ...
    ${resp}=  Update Stock Adjustment  ${Stock_uid}    ${locId1}  ${store_id}   ${encid1}   ${remarks_encid}      ${data} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Set Test Variable  ${invCatalog_id9}  ${resp.json()[0]['id']}

    ${resp}=   Get Inventory Auditlog By id    ${invCatalog_id9}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}             200
    Should Be Equal As Strings  ${resp.json()['uid']}       ${Stock_uid}
    Should Be Equal As Strings  ${resp.json()['auditType']}       ${InventoryAuditType[1]} 
    ...
    Should Be Equal As Strings  ${resp.json()['userType']}   ${userType[0]}
    Should Be Equal As Strings  ${resp.json()['subject']}    Stock Adjusted
    Should Be Equal As Strings  ${resp.json()['description']}    Stock adjustment updated
    Should Be Equal As Strings  ${resp.json()['userId']}   ${user_id}   
    Should Contain              ${resp.json()['dateTime']}   ${datetime01}