...
JD-TC-GetFutureOrderCountByCriteria-1
    ...
    Set Suite Variable  ${email_id}  ${firstname}${PUSERNAME157}.${test_mail}
    ...
    ${firstname}=  FakerLibrary.first_name
    Set Suite Variable  ${email}  ${firstname}${CUSERNAME22}.${test_mail}
    ${EMPTY_List}=  Create List
    Set Suite Variable  ${EMPTY_List}

    ${resp}=   Create Order For HomeDelivery   ${cookie}  ${accId}    ${self}    ${CatalogId1}   ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME22}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order For Pickup   ${cookie}  ${accId}    ${mem_id}    ${CatalogId1}   ${bool[1]}  ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME22}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-2
    ...
    ${accId1}=  get_acc_id  ${PUSERNAME158}
    Set Test Variable  ${accId1} 

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME158}.${test_mail}
    ...
    ${item_quantity1}=  FakerLibrary.Random Int  min=${minQuantity}   max=${maxQuantity}
    ${firstname}=  FakerLibrary.first_name
    Set Test Variable  ${email}  ${firstname}${CUSERNAME22}.${test_mail}

    ${resp}=   Create Order For HomeDelivery   ${cookie}   ${accId1}    ${self}    ${CatalogId1}   ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME22}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order For HomeDelivery   ${cookie}   ${accId1}    ${self}    ${CatalogId1}   ${bool[1]}    ${address}    ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME22}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id2}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Get Future Order Count   account-eq=${accId}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()}     1
    
    ${resp}=   Get Future Order Count   account-eq=${accId1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-3
    ...
    ${resp}=   Get Future Order Count   homeDelivery-eq=${bool[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-4
    ...
    ${resp}=   Get Future Order Count   storePickup-eq=${bool[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-5
    ...
    ${resp}=   Get Future Order Count   storePickup-eq=${bool[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     2

JD-TC-GetFutureOrderCountByCriteria-6
    ...
    ${resp}=   Get Future Order Count   orderStatus-eq=${orderStatuses[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-7
    ...
    ${resp}=   Get Future Order Count   orderStatus-eq=${orderStatuses[2]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-8
    ...
    ${resp}=   Get Future Order Count   orderDate-eq=${DAY2}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-9
    ...
    ${resp}=   Get Future Order Count   orderNumber-eq=${order_no3}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     1

    ${resp}=   Get Future Order Count   orderNumber-eq=${order_no4}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     1

    ${resp}=   Get Future Order Count   orderNumber-eq=${order_no5}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-11
    [Documentation]    Get Future Order Count details by order number.

    ${resp}=  Consumer Login  ${CUSERNAME22}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=   Get Future Order Count   orderNumber-eq=${order_no1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings    ${resp.json()}    0

    ${resp}=   Get Future Order Count   orderNumber-eq=${order_no2}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-12
    ...
    ${resp}=   Get Future Order Count   orderMode-eq=${order_mode[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-13
    ...
    ${resp}=   Get Future Order Count   orderMode-eq=${order_mode[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-14
    ...
    ${resp}=   Get Future Order Count   orderMode-eq=${order_mode[2]}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-15
    ...
    ${resp}=   Get Future Order Count   firstName-eq=${fname}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     3
    
JD-TC-GetFutureOrderCountByCriteria-16
    ...
    ${resp}=   Get Future Order Count   lastName-eq=${lname}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     3
    
JD-TC-GetFutureOrderCountByCriteria-17
    ...
    ${resp}=   Get Future Order Count   email-eq=${email}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     1
   
JD-TC-GetFutureOrderCountByCriteria-18
    ...
    ${resp}=   Get Future Order Count   phoneNumber-eq=${CUSERNAME22}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     3

JD-TC-GetFutureOrderCountByCriteria-19
    ...
    ${resp}=   Get Future Order Count   jaldeeId-eq=${jcons_id1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    Should Be Equal As Strings  ${resp.json()}     3
 
JD-TC-GetFutureOrderCountByCriteria-20
    ...
    ${resp}=   Get Future Order Count   lastName-eq=${lname1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-21
    ...
    ${resp}=   Get Future Order Count   firstName-eq=${fname1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-23
    ...
    ${resp}=   Get Future Order Count   account-eq=000
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderCountByCriteria-UH2
    ...
    ${resp}=   Get Future Order Count   orderDate-eq=${DAY1}
    Log   ${resp.json()}
