...
JD-TC-Get sp item manufacturer Count Filter-1

    [Documentation]  Get sp item manufacturer Count Filter using status
...
# -------------------------------- Create store type -----------------------------------
    ...
    ${resp}=    Create Item Inventory  ${displayName1}    isBatchApplicable=${boolean[1]}    isInventoryItem=${bool[1]}   manufacturerCode=${manufacturerCode}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
# -------------------------------- Add a provider Consumer -----------------------------------
    ...
    ${resp}=    Get sp item manufacturer Filter     ${accountId}   status-eq=${toggle[0]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${len}=  Get Length  ${resp.json()}

    ${resp}=    Get sp item manufacturer Count Filter     ${accountId}   status-eq=${toggle[0]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
