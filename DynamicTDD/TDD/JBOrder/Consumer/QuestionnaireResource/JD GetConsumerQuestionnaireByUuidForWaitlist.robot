...
JD-TC-GetConsumerQuestionnaireByUuidForWaitlist-1
    [Documentation]  Get questionnaire by uuid for wailtlist here waitlist taken from provider side
    ...
    Check Questions   ${resp}   ${qnrid1}   ${sheet1}

JD-TC-GetConsumerQuestionnaireByUuidForWaitlist-2
    [Documentation]  consumer get questionnaire for waitlist taken from consumer side canceled waitlist
    ...
    ${resp}=  Get Consumer Questionnaire By uuid For Waitlist    ${wid}   ${account_id}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetConsumerQuestionnaireByUuidForWaitlist-3
    [Documentation]  consumer get questionnaire for waitlist taken from consumer side
...
JD-TC-GetConsumerQuestionnaireByUuidForWaitlist-UH1
...
JD-TC-GetConsumerQuestionnaireByUuidForWaitlist-UH2
...
JD-TC-GetConsumerQuestionnaireByUuidForWaitlist-UH3
    [Documentation]  Get questionnaire by invalid waitlistid

    ${resp}=  Consumer Login  ${CUSERNAME13}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 

    ${resp}=  Get Consumer Questionnaire By uuid For Waitlist    000   ${account_id}
    Log  ${resp.content}
