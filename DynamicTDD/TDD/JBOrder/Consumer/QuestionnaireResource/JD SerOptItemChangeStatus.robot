...
${pyfile}       /ebs/TDD/cleardb1.py 
${docfile}      /ebs/TDD/docsample.doc 
${txtfile}      /ebs/TDD/defaultpaymentprofile.txt 
${shfile}       /ebs/TDD/example.sh 
${bmpfile}      /ebs/TDD/first.bmp 
${jsonfile}     /ebs/TDD/Gallery.json
${csvfile}      /ebs/TDD/locations.csv 
${oggfile}      /ebs/TDD/oggfile.ogg
...
JD-TC-StatusChangeForServiceOptionItem-1
    [Documentation]  Change Status for Service Options Item
   ...
   ${resp}=   Get Service
    ...
    ${fudata}=  db.fileUploadDT   ${resp.json()}  ${FileAction[0]}  ${Questionnaireid}  ${mp3file}  ${mp3file}   ${pngfile}  ${jpgfile}
    Log  ${fudata}
    ...
    ${resp}=    Imageupload.CSubmitSerOptForItem   ${cookie}    ${item_id1}   ${orderid1}    ${account_id}   ${data}  ${mp3file}  ${mp3file}    ${pngfile}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
   ...
   ${files_data}=  Create List
    ...
    ${resp}=   Change Status Of Service Option Item  ${account_id}  ${orderid1}  @{files_data}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    ${resp}=    Imageupload.CResubmitSerOptForItem   ${cookie}   ${orderid1}    ${account_id}   ${data}  ${jpgfile}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-StatusChangeForServiceOptionItem-UH2
    ...
    ${resp}=    Imageupload.CSubmitSerOptForItem   ${cookie}    ${item_id1}   ${orderid1}    ${account_id}   ${data}  ${mp3file}  ${mp3file}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
