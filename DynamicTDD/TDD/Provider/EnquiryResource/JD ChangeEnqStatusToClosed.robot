...
*** Test Cases ***
JD-TC-ChangeEnqStatusToClosed-1
    [Documentation]   Change Enquiry Status new to Closed
...
*** COMMENT ***
    ...
    ${resp}=    Change Task Status to Closed   ${en_uid}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-ChangeEnqStatusToClosed-UH1
    [Documentation]   Change Enquiry Status new to Closed with invalid enquiry id
    ...
    ${resp}=    Change Task Status to Closed   ${inv_enqu_id}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings    ${resp.json()}    ${INV_ENQ_ID}

JD-TC-ChangeEnqStatusToClosed-UH2
    [Documentation]   Change Enquiry Status new to Closed with another provider login
...
JD-TC-ChangeEnqStatusToClosed-UH3
    [Documentation]   Change Enquiry Status new to Closed with consumer login
...
JD-TC-ChangeEnqStatusToClosed-UH4
    [Documentation]   Change Enquiry Status new to Closed without login
...
JD-TC-ChangeEnqStatusToClosed-UH5
    [Documentation]   Change Enquiry Status new to Closed without enquiry id

    ${resp}=   Encrypted Provider Login  ${PUSERNAME62}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Change Task Status to Closed   ${empty}
    Log   ${resp.content}
