...
*** Test Cases ***
JD-TC-ChangeEnqStatusToRejected-1
    [Documentation]   Change Enquiry Status new to Rejected
...
*** COMMENT ***
    ...
    ${resp}=    Change Task Status to Rejected   ${en_uid}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-ChangeEnqStatusToRejected-UH1
    [Documentation]   Change Enquiry Status new to Rejected with invalid enquiry id
    ...
    ${resp}=    Change Task Status to Rejected   ${inv_enqu_id}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings    ${resp.json()}    ${INV_ENQ_ID}

JD-TC-ChangeEnqStatusToRejected-UH2
    [Documentation]   Change Enquiry Status new to Rejected with another provider login
...
JD-TC-ChangeEnqStatusToRejected-UH3
    [Documentation]   Change Enquiry Status new to Rejected with consumer login
...
JD-TC-ChangeEnqStatusToRejected-UH4
    [Documentation]   Change Enquiry Status new to Rejected without login
...
JD-TC-ChangeEnqStatusToRejected-UH5
    [Documentation]   Change Enquiry Status new to Rejected without enquiry id

    ${resp}=   ProviderLogin  ${PUSERNAME62}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Change Task Status to Rejected   ${empty}
    Log   ${resp.content}
