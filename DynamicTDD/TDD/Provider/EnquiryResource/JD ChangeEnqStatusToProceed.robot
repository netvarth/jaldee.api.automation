...
*** Test Cases ***
JD-TC-ChangeEnqStatusToProceed-1
...
*** COMMENT ***
    ...
    ${resp}=    Change Task Status to Proceed   ${en_uid}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-ChangeEnqStatusToProceed-UH1
    [Documentation]   Change Enquiry Status new to Proceed with invalid enquiry id
    ...
    ${resp}=    Change Task Status to Proceed   ${inv_enqu_id}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings    ${resp.json()}    ${INV_ENQ_ID}

JD-TC-ChangeEnqStatusToProceed-UH2
    [Documentation]   Change Enquiry Status new to Proceed with another provider login
...
JD-TC-ChangeEnqStatusToProceed-UH3
    [Documentation]   Change Enquiry Status new to Proceed with consumer login
...
JD-TC-ChangeEnqStatusToProceed-UH4
    [Documentation]   Change Enquiry Status new to Proceed without login
...
JD-TC-ChangeEnqStatusToProceed-UH5
    [Documentation]   Change Enquiry Status new to Proceed without enquiry id

    ${resp}=   ProviderLogin  ${PUSERNAME62}  ${PASSWORD} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Change Task Status to Proceed   ${empty}
    Log   ${resp.content}
