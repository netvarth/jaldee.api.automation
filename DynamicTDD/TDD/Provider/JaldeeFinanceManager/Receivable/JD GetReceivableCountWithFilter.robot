...
JD-TC-CreateReceivable-1
    ...
    [Documentation]  Create Receivable with valid details.
    ...
    ${resp}=  Get Receivable List with Count filter
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Get Receivable List with Count filter    receivableCategoryId-eq=${category_id3}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200