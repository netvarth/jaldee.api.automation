...
Upload Finance Receivable Attachment
    [Arguments]    ${receivable_uid}      @{vargs}
   ...
   ${data}=  json.dumps  ${data}
   Check And Create YNW Session
   ${resp}=  PUT On Session  ynw  /provider/jp/finance/receivable/${receivable_uid}/attachments  data=${data}  expected_status=any
   RETURN  ${resp}
...
JD-TC-UploadReceivableAttachment-1

    [Documentation]  Create a Receivable then Upload Receivable Attachment.
    ...
    ${resp}=  Upload Finance Receivable Attachment   ${receivable_uid}     ${Attachments}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
