...
JD-TC-AccountLevelAnalyticsforWaitlist-3
    ...
    ${walkin_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids)
    Set Suite Variable   ${walkin_token_len}
...
JD-TC-AccountLevelAnalyticsforWaitlist-4
    ...
    ${online_token_len}=  Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - $walkin_token_len
...
JD-TC-AccountLevelAnalyticsforWaitlist-6
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${vs_waitlist_ids}  ${cwid${a}}
...
JD-TC-AccountLevelAnalyticsforWaitlist-7
    ...
    ${phonein_token_len}=  Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - $walkin_token_len - $online_token_len 
...
JD-TC-AccountLevelAnalyticsforWaitlist-10
    ...
    ${walkin_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - $phonein_token_len - $online_token_len
    Set Suite Variable   ${walkin_token_len}
...
JD-TC-AccountLevelAnalyticsforWaitlist-13
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${prepay_waitlist_ids}  ${online_prepay_wl_ids[${a}]}
    ...
    ${online_token_len}=  Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - $walkin_token_len - $phonein_token_len
    Set Suite Variable   ${online_token_len}
    ${checkedin_token_len}=  Evaluate  $online_token_len + $phonein_token_len
    Set Suite Variable   ${checkedin_token_len}
...
JD-TC-AccountLevelAnalyticsforWaitlist-14
    ...
    ${tot_prepaywl_len}=  Get Length  ${prepay_waitlist_ids}
    ${tot_wl_len}=  Get Length  ${waitlist_ids}
...
JD-TC-AccountLevelAnalyticsforWaitlist-17
    ...
    FOR   ${i}  IN RANGE   ${tot_wl_len}
        ...
        ${resp}=  Get Waitlist By Id  ${waitlist_ids[${i}]}
        Log  ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        Log Many  ${resp.json()['waitlistStatus']}  ${resp.json()['waitlistMode']}
          
        Continue For Loop If   '${resp.json()['waitlistStatus']}' != '${wl_status[0]}'
        ${resp1}=  Run Keyword If  '${resp.json()['waitlistStatus']}' == '${wl_status[0]}'   Get Waitlist By Id  ${waitlist_ids[${i+${count}}]}
        Run Keyword If   '${resp1}' != '${None}'  Log  ${resp1.content}
        Run Keyword If   '${resp1}' != '${None}'  Should Be Equal As Strings  ${resp1.status_code}  200
    ...
    FOR   ${a}  IN RANGE   ${i}  ${i+${count}}
        ...
        Append To List   ${cancelled_wl_ids}  ${waitlist_ids[${a}]}
...
JD-TC-AccountLevelAnalyticsforWaitlist-18
    ...
    FOR   ${i}  IN RANGE   ${tot_wl_len}
        ...
        ${resp1}=  Run Keyword If  '${resp.json()['waitlistStatus']}' == '${wl_status[1]}'   Get Waitlist By Id  ${waitlist_ids[${i+${count}}]}
        Run Keyword If   '${resp1}' != '${None}'  Log  ${resp1.content}
        Run Keyword If   '${resp1}' != '${None}'  Should Be Equal As Strings  ${resp1.status_code}  200
    ...
    ${walkin_token_len}=  Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - $online_token_len - $phonein_token_len
    Set Suite Variable   ${walkin_token_len}
    # ${arrived_token_len}=  Evaluate  $walkin_token_len - $count
...
JD-TC-AccountLevelAnalyticsforWaitlist-19
    ...
    # sleep  05s
    Log Many  ${online_token_len}  ${checkedin_token_len}
    ${online_token_len}=  Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - $walkin_token_len - $phonein_token_len
    Set Suite Variable   ${online_token_len}
    # ${checkedin_token_len}=  Evaluate  $checkedin_token_len + $count
    ${checkedin_token_len}=  Evaluate  $online_token_len + $phonein_token_len - $cancelled_token_len
    Set Suite Variable   ${checkedin_token_len}
...
JD-TC-AccountLevelAnalyticsforWaitlist-21
    ...
    Log Many  ${waitlist_ids}  ${prepay_waitlist_ids}  ${vs_waitlist_ids}  ${cancelled_wl_ids}  ${rescheduled_wl_ids}
    # ${total_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - len($cancelled_wl_ids) - len($rescheduled_wl_ids)
    ${total_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids)
    ${today_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - len($rescheduled_wl_ids)
    ${active_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - len($cancelled_wl_ids)
    ${today_active_token_len}=   Evaluate  len($waitlist_ids) + len($prepay_waitlist_ids) + len($vs_waitlist_ids) - len($cancelled_wl_ids) - len($rescheduled_wl_ids)
...
JD-TC-AccountLevelAnalyticsforWaitlist-23
    ...
    ${checkedin_token_len}=  Evaluate  $online_token_len + $phonein_token_len - $cancelled_token_len/2 - $reschedule_len/2
...
JD-TC-AccountLevelAnalyticsforWaitlist-24
    ...
    ${phonein_prepay_wl_ids}=  Create List
    Set Suite Variable   ${phonein_prepay_wl_ids}
    ...
    FOR   ${a}  IN RANGE   ${count}
        ...
        Append To List   ${phonein_prepay_wl_ids}  ${cwid${a}}
    ...
    Log List   ${phonein_prepay_wl_ids}
