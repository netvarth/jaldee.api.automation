...
JD-TC-Get Account Contact information-6
    ...
    Set Suite Variable  ${sud_domain_id}   ${iscorp_subdomains[0]['subdomainId']}
    ${firstname_A}=  FakerLibrary.first_name
    Set Suite Variable  ${firstname_A}
    ${lastname_A}=  FakerLibrary.last_name
    Set Suite Variable  ${lastname_A}
    ${BUSERNAME_K}=  Evaluate  ${PUSERNAME}+405211222
    ${highest_package}=  get_highest_license_pkg
    ${resp}=  Account SignUp  ${firstname_A}  ${lastname_A}  ${None}  ${domains}  ${sub_domains}  ${BUSERNAME_K}    ${highest_package[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Account Activation  ${BUSERNAME_K}  0
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Account Set Credential  ${BUSERNAME_K}  ${PASSWORD}  ${OtpPurpose['ProviderSignUp']}  ${BUSERNAME_K}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Encrypted Provider Login  ${BUSERNAME_K}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Test Variable  ${pro_id3}  ${resp.json()['id']}
    Append To File  ${EXECDIR}/data/TDD_Logs/numbers.txt  ${BUSERNAME_K}${\n}
    Set Suite Variable  ${BUSERNAME_K}
    Append To File  ${EXECDIR}/data/TDD_Logs/numbers.txt  ${BUSERNAME_K}${\n}
    ${pid3}=  get_acc_id  ${BUSERNAME_K}  
    ${resp}=  Get Account contact information
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()['account']}              ${pid3}
    Should Be Equal As Strings  ${resp.json()['primaryPhoneNumber']}  ${BUSERNAME_K}
    ...
    Should Be Equal As Strings  ${resp.json()['basicInfo']['mobile']}        ${BUSERNAME_K}
    Should Be Equal As Strings  ${resp.json()['basicInfo']['emailVerified']}   ${bool[0]} 
    Should Be Equal As Strings  ${resp.json()['basicInfo']['phoneVerified']}   ${bool[1]} 
    ...
    Should Be Equal As Strings  ${resp.json()[0]['eventType']}        ${EventType[0]}
    Should Be Equal As Strings  ${resp.json()[0]['email']}            []
    Should Be Equal As Strings  ${resp.json()[0]['sms'][0]['number']}           ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[0]['sms'][0]['countryCode']}      ${countryCode_CC0}
    Should Be Equal As Strings  ${resp.json()[0]['pushMsg'][0]['number']}       ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[0]['pushMsg'][0]['countryCode']}  ${countryCode_CC0}
    # Should Be Equal As Strings  ${resp.json()[0]['sms'][0]}           ${BUSERNAME_K}
    # Should Be Equal As Strings  ${resp.json()[0]['pushMsg'][0]}       ${BUSERNAME_K}
    comment   WAITLIST-CANCEL
    Should Be Equal As Strings  ${resp.json()[1]['resourceType']}    ${NotificationResourceType[0]}
    Should Be Equal As Strings  ${resp.json()[1]['eventType']}       ${EventType[1]}
    Should Be Equal As Strings  ${resp.json()[1]['email']}            []
    Should Be Equal As Strings  ${resp.json()[1]['sms'][0]['number']}           ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[1]['sms'][0]['countryCode']}      ${countryCode_CC0}
    Should Be Equal As Strings  ${resp.json()[1]['pushMsg'][0]['number']}       ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[1]['pushMsg'][0]['countryCode']}  ${countryCode_CC0}
    # Should Be Equal As Strings  ${resp.json()[1]['sms'][0]}         ${BUSERNAME_K}
    # Should Be Equal As Strings  ${resp.json()[1]['pushMsg'][0]}     ${BUSERNAME_K}
    comment   APPOINTMENTADD
    Should Be Equal As Strings  ${resp.json()[2]['resourceType']}    ${NotificationResourceType[1]}
    Should Be Equal As Strings  ${resp.json()[2]['eventType']}       ${EventType[7]}
    Should Be Equal As Strings  ${resp.json()[2]['email']}           []
    Should Be Equal As Strings  ${resp.json()[2]['sms'][0]['number']}           ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[2]['sms'][0]['countryCode']}      ${countryCode_CC0}
    Should Be Equal As Strings  ${resp.json()[2]['pushMsg'][0]['number']}       ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[2]['pushMsg'][0]['countryCode']}  ${countryCode_CC0}
    # Should Be Equal As Strings  ${resp.json()[2]['sms'][0]}           ${BUSERNAME_K}
    # Should Be Equal As Strings  ${resp.json()[2]['pushMsg'][0]}       ${BUSERNAME_K}
    comment   APPOINTMENT-CANCEL
    Should Be Equal As Strings  ${resp.json()[3]['resourceType']}     ${NotificationResourceType[1]}
    Should Be Equal As Strings  ${resp.json()[3]['eventType']}         ${EventType[8]}
    Should Be Equal As Strings  ${resp.json()[3]['email']}             []
    Should Be Equal As Strings  ${resp.json()[3]['sms'][0]['number']}           ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[3]['sms'][0]['countryCode']}      ${countryCode_CC0}
    Should Be Equal As Strings  ${resp.json()[3]['pushMsg'][0]['number']}       ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[3]['pushMsg'][0]['countryCode']}  ${countryCode_CC0}
    # Should Be Equal As Strings  ${resp.json()[3]['sms'][0]}            ${BUSERNAME_K}
    # Should Be Equal As Strings  ${resp.json()[3]['pushMsg'][0]}        ${BUSERNAME_K}
    comment   DONATION
    Should Be Equal As Strings  ${resp.json()[4]['resourceType']}     ${NotificationResourceType[3]}
    Should Be Equal As Strings  ${resp.json()[4]['eventType']}       ${EventType[10]}
    Should Be Equal As Strings  ${resp.json()[4]['email']}           []
    Should Be Equal As Strings  ${resp.json()[4]['sms'][0]['number']}           ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[4]['sms'][0]['countryCode']}      ${countryCode_CC0}
    Should Be Equal As Strings  ${resp.json()[4]['pushMsg'][0]['number']}       ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[4]['pushMsg'][0]['countryCode']}  ${countryCode_CC0}
    # Should Be Equal As Strings  ${resp.json()[4]['sms'][0]}          ${BUSERNAME_K}
    # Should Be Equal As Strings  ${resp.json()[4]['pushMsg'][0]}      ${BUSERNAME_K}
    comment   LICENSE
    Should Be Equal As Strings  ${resp.json()[5]['resourceType']}    ${NotificationResourceType[2]}
    Should Be Equal As Strings  ${resp.json()[5]['eventType']}       ${EventType[9]}
    Should Be Equal As Strings  ${resp.json()[5]['email']}           []
    Should Be Equal As Strings  ${resp.json()[5]['sms'][0]['number']}           ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[5]['sms'][0]['countryCode']}      ${countryCode_CC0}
    Should Be Equal As Strings  ${resp.json()[5]['pushMsg'][0]['number']}       ${BUSERNAME_K} 
    Should Be Equal As Strings  ${resp.json()[5]['pushMsg'][0]['countryCode']}  ${countryCode_CC0}
    # Should Be Equal As Strings  ${resp.json()[5]['sms'][0]}          ${BUSERNAME_K}
    # Should Be Equal As Strings  ${resp.json()[5]['pushMsg'][0]}      ${BUSERNAME_K}
...
JD-TC-Get Account Contact information-7
    ...
    # ${resp}=  Encrypted Provider Login   ${BUSERNAME0}  ${PASSWORD}
    # Log  ${resp.content}
    # Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${BUSERNAME_L}=  Evaluate  ${PUSERNAME}+405314222
    ${highest_package}=  get_highest_license_pkg
    ${resp}=  Account SignUp  ${firstname_B}  ${lastname_B}  ${None}  ${domains}  ${sub_domains}  ${BUSERNAME_L}    ${highest_package[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Account Activation  ${BUSERNAME_L}  0
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Account Set Credential  ${BUSERNAME_L}  ${PASSWORD}  ${OtpPurpose['ProviderSignUp']}  ${BUSERNAME_L}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Encrypted Provider Login  ${BUSERNAME_L}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Suite Variable  ${subdomain}  ${resp.json()['subSector']}
    Set Test Variable  ${pro_id3}  ${resp.json()['id']}
    Append To File  ${EXECDIR}/data/TDD_Logs/numbers.txt  ${BUSERNAME_L}${\n}
    Set Suite Variable  ${BUSERNAME_L}
    Append To File  ${EXECDIR}/data/TDD_Logs/numbers.txt  ${BUSERNAME_L}${\n}
    ${pid4}=  get_acc_id  ${BUSERNAME_L}  
    ${resp}=  Get Account contact information
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings  ${resp.json()['account']}            ${pid4}
    Should Be Equal As Strings  ${resp.json()['primaryPhoneNumber']}  ${BUSERNAME_L}
    ...
    ${resp}=  Create User  ${firstname2}  ${lastname2}  ${dob2}  ${Genderlist[0]}  ${P_Email}${PUSERNAME_U1}.${test_mail}   ${userType[0]}  ${pin2}  ${countryCodes[0]}  ${PUSERNAME_U1}  ${dep_id}  ${sud_domain_id}  ${bool[0]}  ${countryCodes[0]}  ${whpnum}  ${countryCodes[0]}  ${tlgnum}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
