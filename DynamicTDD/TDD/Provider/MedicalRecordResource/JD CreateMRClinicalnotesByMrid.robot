...
JD-TC-Createclinicalnotes-1
    [Documentation]   Create clinicalnotes for a waitlist(Walk-in).
    ...
    ${PUSERNAME_C}=  Evaluate  ${PUSERNAME}+7899041
    ...
    # Should Be Equal As Strings  ${resp.json()[0]['observations']}           ${observations1}
    # Should Be Equal As Strings  ${resp.json()[0]['vaccinationHistory']}     ${vaccinationHistory1}

JD-TC-Createclinicalnotes-2
 
    [Documentation]   Create clinicalnotes with consultation mode EMAIL for waitlist.
    ...
    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    ${PUSERNAME_D}=  Evaluate  ${PUSERNAME}+7850049
    ${highest_package}=  get_highest_license_pkg
    ...
    ${strt_time1}=   db.add_timezone_time  ${tz2}  1  00
    Set Suite Variable   ${strt_time1}
    ${end_time}=    db.add_timezone_time  ${tz2}  3  00  
    ${parallel}=   Random Int  min=1   max=1
    ${capacity}=  Random Int   min=10   max=20
    ...
    ${symptoms2}=      FakerLibrary.name
    ${allergies2}=     FakerLibrary.word
    ${diagnosis2}=     FakerLibrary.sentence
    ${complaints2}=    FakerLibrary.sentence
    ${misc_notes2}=    FakerLibrary.word
    ${observations2}=  FakerLibrary.sentence
    ${vaccinationHistory2}=  FakerLibrary.sentence
...
JD-TC-Createclinicalnotes-3

    [Documentation]   Create clinicalnotes for a waitlist(online-in).
...
JD-TC-Createclinicalnotes-4
    [Documentation]   Create clinicalnotes with consultation mode PHONE for waitlist.
...
JD-TC-Createclinicalnotes-5
    [Documentation]   Create clinicalnotes with consultation mode VIDEO for waitlist.
...
JD-TC-Createclinicalnotes-6
    [Documentation]   Create clinicalnote with empty details.
    ...
    ${resp}=  Create MR clinical notes by mr id   ${mr_id}    ${EMPTY}  ${EMPTY}  
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    # Should Be Equal As Strings  ${resp.json()['symptoms']}               ${EMPTY}
    # Should Be Equal As Strings  ${resp.json()['allergies']}              ${EMPTY}
    # Should Be Equal As Strings  ${resp.json()['diagnosis']}              ${EMPTY}
    # Should Be Equal As Strings  ${resp.json()['complaints']}             ${EMPTY}
    # Should Be Equal As Strings  ${resp.json()['misc_notes']}             ${EMPTY}
    # Should Be Equal As Strings  ${resp.json()['observations']}           ${EMPTY}
    # Should Be Equal As Strings  ${resp.json()['vaccinationHistory']}     ${EMPTY}
   
   
JD-TC-Createclinicalnotes-7
    [Documentation]   Create clinicalnotes for family member's waitlist(online check-in).
...
JD-TC-Createclinicalnotes-8
    [Documentation]   Create clinicalnotes for appointment(walkin).
...
JD-TC-Createclinicalnotes-9
    [Documentation]   Create clinicalnotes for appointment(online)
...
JD-TC-Createclinicalnotes-10
    [Documentation]  Create clinicalnotes for family member's appointment(online check-in).
...
JD-TC-Createclinicalnotes-11
    [Documentation]  Provider takes appointment for a consumer and reschedules it to a later time in the same day and then create clinicalnotes
...
JD-TC-Createclinicalnotes-12
    [Documentation]  Provider takes appointment for a consumer and create Mr clinicalnotes and reschedules it to a later time in the same day and then create another clinicalnotes
    ...
    Should Be Equal As Strings   ${resp.json()['availableSlots'][1]['noOfAvailbleSlots']}   1
...
JD-TC-Createclinicalnotes-13
    [Documentation]  User Create clinicalnotes (waitlist for current day)
    ...
    ${MUSERNAME_E}=  Evaluate  ${MUSERNAME}+8840215
    ${highest_package}=  get_highest_license_pkg
    ...
    ${PUSERNAME_U1}=  Evaluate  ${PUSERNAME}+886441
...
JD-TC-Createclinicalnotes-14
    [Documentation]   Create clinicalnotes for MR having clinicalnotes.
...
JD-TC-Createclinicalnotes-UH1
    [Documentation]   Create clinicalnotes for a canceled waitlist (After MR creation provider cancel the waitlist and try to create mr clinicalnotes)
    ...
    ${resp}=  Create MR clinical notes by mr id   ${mr_id3}   ${type}  ${clinicalNote}    ${attachment1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings   "${resp.json()}"   "${CAN_NOT_CRETAE_CLINICAL_NOTE_WL}"


JD-TC-Createclinicalnotes-UH2
    [Documentation]  Create clinicalnotes for a canceled family member's appointment (After MR creation consumer cancel the appointment and try to create mr clinicalnotes)
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings   "${resp.json()}"   "${CAN_NOT_CRETAE_CLINICAL_NOTE_APPT}"


JD-TC-Createclinicalnotes-UH3
    [Documentation]   Create clinicalnotes using another provider's MR.
...
JD-TC-Createprescription-UH4
    [Documentation]   Create clinicalnotes without login.
...
JD-TC-Createclinicalnotes-UH5
    [Documentation]   Create clinicalnotes with consumer login.
...
JD-TC-Createclinicalnotes-UH6
 
    [Documentation]   Create clinicalnotes for a future waitlist(walk-in).
...
JD-TC-Createclinicalnotes-UH7
    [Documentation]  Provider takes appointment for a consumer and reschedules it to another day and try to create clinicalnotes
