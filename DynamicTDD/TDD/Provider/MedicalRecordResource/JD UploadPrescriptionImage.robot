...
JD-TC-Uploadprescription-1
    ...
    ${PUSERNAME_C}=  Evaluate  ${PUSERNAME}+7861331
...
JD-TC-Uploadprescription-2
 
    [Documentation]   upload prescription image with mr consultation mode EMAIL for online waitlist  
    ...
    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    ${PUSERNAME_D}=  Evaluate  ${PUSERNAME}+7850638
    ${highest_package}=  get_highest_license_pkg
...
JD-TC-Uploadprescription-UH1

    [Documentation]  Provider check to upload prescription image with another provider mrid
...
JD-TC-Uploadprescription-UH2

    [Documentation]  Provider check to upload prescription image with invalid mrid
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=   uploadPrescriptionImage   000   ${cookie}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422

JD-TC-Uploadprescription-UH3

    [Documentation]  Consumer check to upload prescription image  
...
JD-TC-Uploadprescription-UH4
    [Documentation]   upload prescription for a canceled waitlist (After mr creation provider cancel the waitlist and try to Upload prescription)
