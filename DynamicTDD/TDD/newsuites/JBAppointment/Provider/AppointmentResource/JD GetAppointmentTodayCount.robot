...
JD-TC-GetAppointmentTodayCount-1
    ...
    ${resp}=  AddCustomer  ${CUSERNAME8}  firstName=${fname2}   lastName=${lname2}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetAppointmentTodayCount-2
    ...
    # clear_location_n_service  ${PUSERNAME179}
    ...
    ${resp}=    ProviderConsumer Login with token   ${CUSERNAME15}    ${pid}  ${token2} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-GetAppointmentTodayCount-3
    ...
    ${branch}=   db.get_place
    ${resp}=   Update Account Payment Settings   ${bool[0]}  ${bool[0]}  ${bool[1]}  ${PUSERNAME179}   ${pan_num}  ${bank_ac}  ${bank_name}  ${ifsc_code}  ${name}  ${name}  ${branch}  ${businessFilingStatus[1]}  ${accountType[1]}   
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=   Update Account Payment Settings   ${bool[1]}  ${bool[0]}  ${bool[1]}  ${PUSERNAME179}   ${pan_num}  ${bank_ac}  ${bank_name}  ${ifsc_code}  ${name}  ${name}  ${branch}  ${businessFilingStatus[1]}  ${accountType[1]}   
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-GetAppointmentTodayCount-4
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${fname2}   ${resp.json()[0]['firstName']}
    ...
    ${resp}=    ProviderConsumer Login with token   ${CUSERNAME7}    ${pid}  ${token2} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-GetAppointmentTodayCount-6
    ...
    Comment   ${CUSERNAME7} has taken appointment for this provider from consumer side in the last 5 cases.
