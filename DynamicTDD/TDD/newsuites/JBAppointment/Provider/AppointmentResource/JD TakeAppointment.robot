...
JD-TC-Take Appointment-7
    ...
    [Documentation]  Provider takes appointment for consumer for End date
...
JD-TC-Take Appointment-9
    ...
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer   ${cid}  ${s_id1}  ${sch_id}  ${DAY3}  ${cnote}  ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
#...........signup a provider.......
    ...
    ${PH_Number}=    Evaluate    f'{${PH_Number}:0>7d}'
    Log  ${PH_Number}
    Set Suite Variable  ${PUSERNAME_A}  555${PH_Number}
    ...
    ${resp}=  Account SignUp  ${fname}  ${lname}  ${None}  ${domain}  ${subdomain}  ${PUSERNAME_A}  ${licpkgid}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    202

    ${resp}=  Account Activation  ${PUSERNAME_A}   ${OtpPurpose['ProviderSignUp']}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer   ${cid}  ${s_id1}  ${sch_id2}  ${DAY3}  ${cnote}  ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Take Appointment-12
    ...
    ${cnote}=   FakerLibrary.word
    ${resp}=  Take Appointment For Consumer   ${cid}  ${s_id}  ${sch_id2}  ${DAY4}  ${cnote}  ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Take Appointment-24
    ...
    ${account_id}=  get_acc_id  ${PUSERNAME345}
...
JD-TC-Take Appointment-30
    ...
    ${duration}=  FakerLibrary.Random Int  min=1  max=${maxval}
    ${bool1}=  Random Element  ${bool}
    ${resp}=  Create Appointment Schedule  ${schedule_name}  ${recurringtype[4]}  ${list}  ${DAY1}  ${DAY2}  ${EMPTY}  ${sTime1}  ${eTime1}  ${parallel}  ${parallel}  ${lid}  ${duration}  ${bool1}  ${s_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${sch_id}  ${resp.json()}
...
JD-TC-Take Appointment-37
    ...
    ${account_id}=  get_acc_id  ${billable_providers[3]}
...
#     ${maxval}=  Convert To Integer   ${delta/2}
    # ${duration}=  FakerLibrary.Random Int  min=1  max=${maxval}
#     ${bool1}=  Random Element  ${bool}
#     ${resp}=  Create Appointment Schedule  ${schedule_name}  ${recurringtype[1]}  ${list}  ${DAY3}  ${DAY2}  ${EMPTY}  ${sTime1}  ${eTime1}  ${parallel}  ${parallel}  ${lid}  ${duration}  ${bool1}  ${s_id}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200
#     Set Test Variable  ${sch_id}  ${resp.json()}
...
JD-TC-Take Appointment-UH30
   ...
   # ${resp}=    Get Next Available Appointment Schedule    ${sch_id}
    ${resp}=  Get Appointment Schedule ById  ${sch_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}  id=${sch_id}   name=${schedule_name}  apptState=${Qstate[0]}
    ...
    Should Be Equal As Strings  ${resp.status_code}  424
    Should Be Equal As Strings   "${resp.json()}"   "${APPOINTMET_SLOT_NOT_AVAILABLE}"
    #Verify Response    ${resp}     uid=${apptid1}     appmtTime=${slot1}
...
JD-TC-Take Appointment-43
    ...
    ${resp}=  Create Service  ${SERVICE1}  ${description}  2   ${status[0]}    ${btype}    ${bool[1]}    ${notifytype[2]}   ${min_pre}  ${Total}  ${bool[0]}  ${bool[0]}    maxBookingsAllowed=3
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${s_id}  ${resp.json()}
