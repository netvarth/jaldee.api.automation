...
JD-TC-GetConsumerAppointments-1
    ...
    ${resp}=  Consumer SignUp  ${firstname}  ${lastname}  ${address}  ${SIGNUP_CONS1}  ${CUSERPH_SECOND}  ${dob}  ${gender}   ${CUSERMAIL2}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Set Suite Variable   ${slot2}   ${resp.json()['availableSlots'][1]['time']}
    ...
    ${resp}=  GetCustomer  phoneNo-eq=${SIGNUP_CONS1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetConsumerAppointments-2
    ...
    Set Suite Variable   ${slot3}   ${resp.json()['availableSlots'][2]['time']}
...
JD-TC-GetConsumerAppointments-6
    ...
    Set Suite Variable   ${slot4}   ${resp.json()['availableSlots'][0]['time']}
...
JD-TC-GetConsumerAppointments-8
    ...
    Log Many  ${slot1}  ${slot2}  ${slot3}  ${slot4}  

    ${resp}=  Get Consumer Appointments    apptTime-eq=${slot4}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    Should Contain 	${resp.text} 	${slot4} 	
    ${len}=  Get Length  ${resp.json()}

    FOR  ${i}  IN RANGE   ${len}

        IF  '${resp.json()[${i}]['appmtTime']}' == '${slot4}' 
        ...
        Should Be Equal As Strings  ${resp.json()[${i}]['uid']}                                    ${apptid4}
        Should Be Equal As Strings  ${resp.json()[${i}]['consumer']['userProfile']['firstName']}   ${f_Name1}
        Should Be Equal As Strings  ${resp.json()[${i}]['consumer']['userProfile']['lastName']}    ${l_Name1}
...
JD-TC-GetConsumerAppointments-11
    ...
    Log Many  ${slot1}  ${slot2}  ${slot3}  ${slot4}  ${slot21}
    # ${slot1_time}=  Get Substring   ${slot4}  0  5
    ${slot4_time}=  Fetch From Left  ${slot4}  -

    ${resp}=  Get Consumer Appointments    apptstartTime-eq=${slot4_time}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    # ${string_resp}= 	Convert JSON To String 	${resp.content}
    Should Contain 	${resp.text} 	${apptid4} 	
    ${len}=  Get Length  ${resp.json()}

    FOR  ${i}  IN RANGE   ${len}

        IF  '${resp.json()[${i}]['uid']}' == '${apptid4}' 
        ...
        ELSE IF  '${resp.json()[${i}]['uid']}' == '${apptid1}'
