...
JD-TC-GetFutureAppointmentCount-1
    ...
    [Documentation]  Get consumer's future appointments count.
    ...
    ${PUSERNAME_F}=  Evaluate  ${PUSERNAME}+5566033
    ...
    Set Test Variable  ${email_id}  ${P_Email}${PUSERNAME_F}.${test_mail}
    ...
    ${emails1}=  Emails  ${name3}  Email  ${P_Email}${PUSERNAME_F}.${test_mail}  ${views}
    ${bs}=  FakerLibrary.bs
    ...
    ${duration}=  FakerLibrary.Random Int  min=1  max=${maxval}
    ${bool1}=  Random Element  ${bool}
    ${resp}=  Create Appointment Schedule  ${schedule_name}  ${recurringtype[1]}  ${list}  ${DAY3}  ${DAY4}  ${EMPTY}  ${sTime2}  ${eTime2}  ${parallel}  ${parallel}  ${lid}  ${duration}  ${bool1}  ${s_id1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Get Consumer Future Appointments Count 
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetFutureAppointmentCount-3
	...
	[Documentation]  Filter Future Appointments count by service id.
    ...
    ${resp}=  Get Consumer Future Appointments Count   service-eq=${s_id2}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetFutureAppointmentCount-4
    ...
    ${resp}=  Get Consumer Future Appointments Count   appointmentEncId-eq=${A_uuid1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Get Consumer Future Appointments Count   appointmentEncId-eq=${A_uuid5}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
    ...
    ${resp}=  Get Consumer Future Appointments Count   appointmentEncId-eq=${A_uuid4}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetFutureAppointmentCount-5
	...
	[Documentation]  Filter future Appointment count by first name.
    ...
    ${resp}=  Get Consumer Future Appointments Count   firstName-eq=${f_Name}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  2
...
JD-TC-GetFutureAppointmentCount-6
	...
	[Documentation]  Filter future Appointment count by last name.
    ...
    ${resp}=  Get Consumer Future Appointments Count   lastName-eq=${l_Name}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  2
...
JD-TC-GetFutureAppointmentCount-7
	...
	[Documentation]  Filter future Appointments count by schedule id.
    ...
    ${resp}=  Get Consumer Future Appointments Count   schedule-eq=${sch_id1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
    ...
    ${resp}=  Get Consumer Future Appointments Count   schedule-eq=${sch_id4}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-8
	...
	[Documentation]  Get consumer's future appointments count where appointment taken by consumer(apptBy).
    ...
    ${resp}=  Get Consumer Future Appointments Count   apptBy-eq=CONSUMER
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  4
...
JD-TC-GetFutureAppointmentCount-9
	...
	[Documentation]  Filter consumer future appointments count by appointment time.
    ...
    ${resp}=  Get Consumer Future Appointments Count   apptTime-eq=${slot1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
    ...
    ${resp}=  Get Consumer Future Appointments Count   apptTime-eq=${slot3}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-10
	...
	[Documentation]  Filter consumer future Appointments count by paymentstatus(partially paid).
    ...
    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    ${PUSERNAME_C}=  Evaluate  ${PUSERNAME}+5566034
    ${highest_package}=  get_highest_license_pkg
    ...
    ${resp}=  Get Consumer Future Appointments Count   paymentStatus-eq=${paymentStatus[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-11
	...
	[Documentation]  Filter consumer future Appointments count by location.
    ...
    ${resp}=  Get Consumer Future Appointments Count   location-eq=${lid}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  2
...
JD-TC-GetFutureAppointmentCount-12
	...
	[Documentation]  Filter consumer future Appointments conut by appointment start time.
    ...
    ${resp}=  Get Consumer Future Appointments Count   apptstartTime-eq=${slot1_time}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-13
    ...
    ${resp}=  Get Consumer Future Appointments Count    apptStatus-eq=${apptStatus[2]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-14
    ...
    ${resp}=  Get Consumer Future Appointments Count    apptStatus-eq=${apptStatus[4]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-15
    ...
    ${resp}=  Get Consumer Future Appointments Count    apptStatus-eq=${apptStatus[5]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-16
	...
	[Documentation]  Filter consumer future Appointments count by appointment Date.
    ...
    ${resp}=  Get Consumer Future Appointments Count    date-eq=${DAY2}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()}  1
...
JD-TC-GetFutureAppointmentCount-18
    ...
    [Documentation]  Get consumer's future appointments count when today date given.
    ...
    ${resp}=  Get Consumer Future Appointments Count   date-eq=${DAY1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
...
JD-TC-GetFutureAppointmentCount-UH1
    ...
    [Documentation]  Get consumer's future appointments count without consumer login
