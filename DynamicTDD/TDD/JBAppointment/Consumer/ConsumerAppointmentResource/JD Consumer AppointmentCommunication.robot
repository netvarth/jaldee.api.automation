...
*** Keywords ***
Consumer Appointment Communication
    [Arguments]   ${accId}   ${uuid}  ${msg}  ${messageType}   ${attachments}   &{kwargs}
    ${message}=  Create Dictionary  msg=${msg}    messageType=${messageType} 
    ${data}=    Create Dictionary   message=${message}   attachments=${attachments}
    FOR  ${key}  ${value}  IN  &{kwargs}
        Set To Dictionary  ${data}   ${key}=${value}
    END
    ${data}=  json.dumps  ${data}
    Check And Create YNW Session
    ${resp}=  POST On Session  ynw   url=consumer/appointment/communicate/${uuid}?account=${accId}    data=${data}  expected_status=any   
    Check Deprication  ${resp}  Consumer Appointment Communication
    RETURN  ${resp}
...
#     Should Be Equal As Strings    ${resp.status_code}   200
    ...
    IF   '${resp.content}' == '${emptylist}'
        ${resp1}=  AddCustomer  ${CUSERNAME21}  firstName=${consumerFirstName}
        Log  ${resp1.content}
    ...
    ${fileSize}    Convert To Number   ${fileSize}
    ...
    ${attachments}=  Create Dictionary  owner=${cid}  fileName=${fileName}  fileSize=${fileSize}  fileType=${fileType1}  order=${order}  driveId=${driveId}  action=${file_action[0]}  ownerName=${consumerFirstName}   type=photo   ownerType=${ownerType[1]}    caption=${caption1}   file=${file}
    ${attachments}=   Create List  ${attachments}

    ${msg}=  Fakerlibrary.sentence
    ${resp}=   Consumer Appointment Communication  ${pid}   ${apptid1}   ${msg}  ${messageType[3]}  ${attachments}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
