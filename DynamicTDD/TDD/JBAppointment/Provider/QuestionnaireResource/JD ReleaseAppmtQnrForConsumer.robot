...
JD-TC-ReleaseAppmtQnrForConsumer-1
    [Documentation]  Release Appointment for Consumer where release status is released
...
JD-TC-ReleaseAppmtQnrForConsumer-UH2
    ...
    ${resp}=    Release Appmt Qnr For Consumer    ${empty}   ${apptid1}   ${QnrReleaseStatus[2]}
    Log   ${resp.content}
...
JD-TC-ReleaseAppmtQnrForConsumer-UH3
    ...
    ${resp}=    Release Appmt Qnr For Consumer    ${questId}   ${empty}   ${QnrReleaseStatus[2]}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
...
JD-TC-ReleaseAppmtQnrForConsumer-UH4
    ...
    ${resp}=    Release Appmt Qnr For Consumer    ${questId}   ${apptid1}   ${empty}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
...
JD-TC-ReleaseAppmtQnrForConsumer-UH5
    ...
    ${resp}=    Release Appmt Qnr For Consumer    ${inv_questId}   ${apptid1}   ${QnrReleaseStatus[1]}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-ReleaseAppmtQnrForConsumer-UH6
    ...
    ${inv_apptid}=  Generate Random String  3  [NUMBERS]

    ${resp}=    Release Appmt Qnr For Consumer    ${questId}   ${inv_apptid}   ${QnrReleaseStatus[1]}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
...
JD-TC-ReleaseAppmtQnrForConsumer-4
    ...
    # ${CUSERNAME8}    Generate random string    10    123456789
    # ${CUSERNAME8}    Convert To Integer  ${CUSERNAME8}

    # ${resp}=  AddCustomer  ${CUSERNAME8}
        
    # Log   ${resp.json()}
    # Should Be Equal As Strings  ${resp.status_code}  200
    # Set Suite Variable    ${CUSERNAME8}

    ${resp}=    Send Otp For Login    ${CUSERNAME8}    ${accountId}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME8}    ${accountId}  ${token} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
