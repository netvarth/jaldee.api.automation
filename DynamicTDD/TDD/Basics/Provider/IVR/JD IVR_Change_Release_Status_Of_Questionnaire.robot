...
JD-TC-IVRChangeReleaseStatusOfQuestionnaire-1

    [Documentation]  IVR Change Release Status Of Questionnaire
    ...
    ${resp}=    ivr_user_details    ${account_id}  ${countryCodes[1]}  ${myoperator_id}  ${PUSERNAME170}  ${countryCodes[1]}${PUSERNAME170}  ${user_id}  ${user_name}
...
#........  user answered    ........
    ...
    ${clid_user}=    Convert To String    ${countryCodes[1]}${PUSERNAME170}
...
#.......  call finished      .........                                                                                                                                                                                                                                                                                                   
    ...
    ${resp}=    Get Ivr By Uid    ${ivruid}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${qnr}=  Get Dictionary Values  ${resp.json()['qnrList']}
    Should Be Equal As Strings    ${qnr[0]}    ${QnrReleaseStatus[2]}

    ${resp}=    IVR Change Release Status Of Questionnaire    ${QnrReleaseStatus[1]}    ${ivruid}    ${Qid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Should Be Equal As Strings  ${resp.status_code}  200
    ${qnr}=  Get Dictionary Values  ${resp.json()['qnrList']}
    Should Be Equal As Strings    ${qnr[0]}    ${QnrReleaseStatus[1]}