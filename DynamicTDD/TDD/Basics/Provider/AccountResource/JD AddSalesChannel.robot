...
*** Variables ***
${INVALID_id1}           abcd111
${INVALID_scDiscCodes}   AAA11
${INVALID_SC_repid}      ABCD123


*** Test Cases ***

JD-TC-Add SalesChannel1
...
JD-TC-Add SalesChannel-UH1

    [Documentation]   Add sales channel using consumer id
...
JD-TC-Add SalesChannel-UH2

    [Documentation]  Add Sales channel without Login
    ${resp}=  Add SalesChannel   ${sc_code}
     Log  ${resp.json()}
...
JD-TC-Add SalesChannel-UH3
    ...
    ${SC_DOES_NOT_EXISTS}=   Format String   ${SC_NOT_EXISTS}   ${INVALID_id1}
    Should Be Equal As Strings   ${resp.json()}   ${SC_DOES_NOT_EXISTS}
...
JD-TC-Add SalesChannel-UH4
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    ${SCRep_DOES_NOT_EXISTS}=   Format String   ${SC_REP_BY_ID_NOT_EXIST}   ${INVALID_SC_repid}
    Should Be Equal As Strings   ${resp.json()}   ${SCRep_DOES_NOT_EXISTS}
...
JD-TC-Add SalesChannel-UH5
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings   ${resp.json()}   ${SC_FRMT}
...
JD-TC-Add SalesChannel-UH6
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings   ${resp.json()}   ${SC_PARTNER_ID_ALREADY_ADDED}
...
JD-TC-Add SalesChannel-UH9
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings   ${resp.json()}   ${ENTER_VALID_SC_CODE} 
