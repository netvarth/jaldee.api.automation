...
JD-TC-Get_Member_QNR-1

    [Documentation]  Get Member QNR
    ...
    ${accountId}=    get_acc_id       ${PUSERNAME84}
   ...
   FOR  ${i}  IN RANGE   1  ${j}
    ...
    ${resp}=    Get Provider Member Qnr  ${memberid1}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200


JD-TC-Get_Member_QNR-UH1

    [Documentation]  Get Member QNR with another provider login
...
JD-TC-Get_Member_QNR-UH2

    [Documentation]  Get Member QNR where member id is invalid 

    ${resp}=  Encrypted Provider Login  ${PUSERNAME84}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${fakemember}=  FakerLibrary.RandomNumber

    ${resp}=    Get Provider Member Qnr  ${fakemember}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
    Should Be Equal As Strings  ${resp.json()}    ${MEMBER_NOT_EXISTS}


JD-TC-Get_Member_QNR-2

    [Documentation]  Get Member QNR Before submit
