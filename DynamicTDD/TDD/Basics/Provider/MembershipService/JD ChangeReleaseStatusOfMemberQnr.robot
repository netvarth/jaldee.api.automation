...
JD-TC-Change_Release_Status_Member_QNR-1

    [Documentation]  Change Release Status Member QNR
   ...
   FOR  ${i}  IN RANGE   1  ${j}
    ...
    ${resp}=    Change Release Status Of Member Questionnaire    ${QnrReleaseStatus[1]}    ${memberid1}    ${Quid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Change_Release_Status_Member_QNR-2

    [Documentation]  Change Release Status to unreleased

    ${resp}=    ProviderConsumer Login with token   ${number1}    ${accountId}  ${token} 
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Change Release Status Of Member Questionnaire    ${QnrReleaseStatus[2]}    ${memberid1}    ${Quid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200

JD-TC-Change_Release_Status_Member_QNR-3

    [Documentation]  Change Release Status to unreleased where status is already unreleased
...
JD-TC-Change_Release_Status_Member_QNR-UH1

    [Documentation]  Change Release Status where number id is invalid
...
JD-TC-Change_Release_Status_Member_QNR-UH2
    ...
    [Documentation]  Change Release Status where Qnr id is invalid
    ...
    ${resp}=    Change Release Status Of Member Questionnaire    ${QnrReleaseStatus[1]}    ${memberid1}    ${fakemem}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
    Should Be Equal As Strings    ${resp.json()}    ${QUESTIONNAIRE_NOT_EXIST}

JD-TC-Change_Release_Status_Member_QNR-UH3

    [Documentation]  Change Release Status where Qnr id is empty
    ...
    ${resp}=    Change Release Status Of Member Questionnaire    ${QnrReleaseStatus[1]}    ${memberid1}    ${empty}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404

JD-TC-Change_Release_Status_Member_QNR-UH4
...
JD-TC-Change_Release_Status_Member_QNR-UH5

    [Documentation]  Change Release Status without login
