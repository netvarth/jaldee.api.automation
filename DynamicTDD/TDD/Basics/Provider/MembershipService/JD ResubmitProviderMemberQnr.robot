...
JD-TC-Resubmit_Member_QNR-1
   ...
   FOR  ${i}  IN RANGE   1  ${j}
    ...
    ${resp}=    Resubmit Provider Member Qnr  ${memberid1}   ${data}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Resubmit_Member_QNR-UH1
    ...
    [Documentation]  Resubmit Member QNR where member id is invalid or wrong 

    ${resp}=  Encrypted Provider Login  ${PUSERNAME76}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Resubmit Provider Member Qnr  3258   ${data}
    Log  ${resp.content}
...
JD-TC-Resubmit_Member_QNR-UH5
    ...
    [Documentation]  Resubmit Member QNR without submit
    ...
    Set Suite Variable  ${user_id}  ${decrypted_data['id']}
    # Set Suite Variable    ${user_id}    ${resp.json()['id']}
    ${accountId}=    get_acc_id       ${PUSERNAME79}
    Set Suite Variable    ${accountId}
