...
JD-TC-CustomViewAPIS-2
   ...
   #${resp}=  Toggle Department Enable
    ...
    Verify Response  ${resp}  id=${u_id}  firstName=${firstname}  lastName=${lastname}  address=${address}  mobileNo=${PUSERNAME0}  dob=${dob}  gender=${Genderlist[0]}  userType=${userType[0]}  status=ACTIVE  email=${P_Email}${PUSERNAME0}.${test_mail}  state=${state}  deptId=${depid02}  subdomain=${userSubDomain}
