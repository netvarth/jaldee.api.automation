...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+9874621
...
JD-TC-BranchManager-7
    ...
    Set Suite Variable   ${cu_email}     ${lname}${C_Email}.test@jaldee.com
    ${cust_email}=  String.RemoveString  ${cu_email}    ${SPACE}
...
# ....... Save Customer Details .......
    ...
    ${resp}=    Save Customer Details      ${loanuid}  ${dealerfname}  ${dealerlname}  ${cust_email}  ${dob}  ${Genderlist[1]}  ${cust_id}  ${kyc_list_cust}  ${locations}    ${CustomerPhoto}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200
...
# ....... Draft Loan Details .......
    ...
    ${resp}=    Draft Loan Application     ${loanuid}  ${dealerfname}  ${dealerlname}  ${cust}  ${cust_email}  ${dob}  ${Genderlist[1]}  ${countryCodes[0]}  ${cust_id}  ${invoiceAmount}  ${invoiceAmount}  ${downpaymentAmount}  ${requestedAmount}  ${remarks}  ${emiPaidAmountMonthly}  ${boolean[0]}  ${referralEmployeeCode}  ${boolean[0]}  ${LoanApplicationKycListSave}  ${type}  ${loanProducts}  ${loanproductcatid1}   ${loanproductSubcatid}  ${locations}  ${partner}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    200
...
JD-TC-BranchManager-UH4
    ...
    ${resp}=    Update BranchMaster    ${branchid}    ${branchCode}    ${branchName}    ${locId}    ${status[0]}    
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  ${resp.content}       "${NO_PERMISSION_FOR_REQUEST}"
