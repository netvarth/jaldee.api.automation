...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+2314854
...
# .....Create Dealer By Sales Officer.......
    ...
    ${resp}=    Reset LoginId  ${RM}  ${RM_USERNAME}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=    Forgot Password   loginId=${RM_USERNAME}  password=${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    202

    ${resp}=    Account Activation  ${RM_USERNAME}  ${OtpPurpose['ProviderResetPassword']}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${key} =   db.Verify Accnt   ${RM_USERNAME}    ${OtpPurpose['ProviderResetPassword']}
    Set Suite Variable   ${key}
    ...
    ${resp}=    Connect with other login  ${RM_USERNAME}  ${PASSWORD}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
