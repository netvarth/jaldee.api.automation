...
JD-TC-Activate Partner-1
                                  
    [Documentation]              Activate Partner
...
# <<<--------------------update partner ------------------------------------------------------->>>
    ...
    ${resp}=    Partner Details    ${partuid1}    ${partnerName}    ${P_phone}    ${email}    ${description}     ${Ptypeid}    ${Pcategoryid}    ${address1}    ${address2}    ${pin}    ${partnerCity}    ${district}    ${state}    ${aadhaar}    ${pan}    ${gstin}    ${branch}    ${partnerAliasName}    ${aadhaarAttachments}    ${panAttachments}    ${gstAttachments}    ${licenceAttachments}    ${partnerAttachments}    ${storeAttachments}   
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
...
JD-TC-Activate Partner-UH1
                                  
    [Documentation]               Activate partner which is already activated
...
JD-TC-ActivatePartner-UH2
                                  
    [Documentation]               Activate Partner where partner is not approved
...
# <<<--------------------update partner ------------------------------------------------------->>>
    ...
    ${aadhaarAttachments}=    Create Dictionary   action=${LoanAction[0]}  owner=${Pid2}  fileName=${pdffile}  fileSize=${fileSize}  caption=${caption3}  fileType=${fileType3}  order=${order}
    ${aadhaarAttachments}=    Create List  ${aadhaarAttachments}

    ${panAttachments}=    Create Dictionary   action=${LoanAction[0]}  owner=${Pid2}  fileName=${pngfile}  fileSize=${fileSize}  caption=${caption2}  fileType=${fileType2}  order=${order}
    ${panAttachments}=    Create List  ${panAttachments}

    ${gstAttachments}=    Create Dictionary   action=${LoanAction[0]}  owner=${Pid2}  fileName=${jpgfile}  fileSize=${fileSize}  caption=${caption1}  fileType=${fileType1}  order=${order}
    ${gstAttachments}=    Create List  ${gstAttachments}

    ${licenceAttachments}=    Create Dictionary   action=${LoanAction[0]}  owner=${Pid2}  fileName=${jpgfile2}  fileSize=${fileSize}  caption=${caption4}  fileType=${fileType4}  order=${order}
    ${licenceAttachments}=    Create List  ${licenceAttachments}

    ${partnerAttachments}=    Create Dictionary   action=${LoanAction[0]}  owner=${Pid2}  fileName=${gif}  fileSize=${fileSize}  caption=${caption5}  fileType=${fileType5}  order=${order}
    ${partnerAttachments}=    Create List  ${partnerAttachments}

    ${storeAttachments}=    Create Dictionary   action=${LoanAction[0]}  owner=${Pid2}  fileName=${xlsx}  fileSize=${fileSize}  caption=${caption6}  fileType=${fileType6}  order=${order}
    ${storeAttachments}=    Create List  ${storeAttachments}

    ${resp}    Update Partner Aadhar    ${aadhaar}    ${Puid2}    ${LoanAction[0]}    ${Pid2}    ${pdffile}    ${fileSize}    ${caption3}    ${fileType3}    ${order}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}    Update Partner Pan    ${pan}    ${Puid2}    ${LoanAction[0]}    ${Pid2}    ${pdffile}    ${fileSize}    ${caption3}    ${fileType3}    ${order}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
    ...
    ${resp}    Update Partner Bank    ${bankAccountNo}    ${bankIfsc}    ${bankName}    ${Puid2}    ${LoanAction[0]}    ${Pid2}    ${pdffile}    ${fileSize}    ${caption3}    ${fileType3}    ${order}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
    ...
    ${resp}    Verify Partner Bank    ${Pid2}    ${Puid2}     ${bankAccountNo}    ${bankIfsc}     bankName=${bankName}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Partner Details    ${Puid2}    ${partnerName}    ${P_phone}    ${email}    ${description}     ${Ptypeid}    ${Pcategoryid}    ${address1}    ${address2}    ${pin}    ${partnerCity}    ${district}    ${state}    ${aadhaar}    ${pan}    ${gstin}    ${branch}    ${partnerAliasName}    ${aadhaarAttachments}    ${panAttachments}    ${gstAttachments}    ${licenceAttachments}    ${partnerAttachments}    ${storeAttachments}   
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
...
# <<<--------------------update partner ------------------------------------------------------->>>
    ...
    ${resp}=   Partner Approval Request    ${Puid2}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
    ...
    ${resp}=    Activate Partner    ${Puid2}     ${bool[1]}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}    ${PARTNER_SALES_OFFICERS_REQUIRED}


JD-TC-ActivatePartner-UH3
                                  
    [Documentation]               Activate Partner where sales officer is not updated
    ...
    ${resp}=   Partner Approved    ${Puid2}    ${note}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
...
JD-TC-ActivatePartner-UH4
                                  
    [Documentation]               Activate Partner where credit officer is not updated
    ...
    ${resp}=    Update Sales Officer    ${Puid2}    ${Salesofficer}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}    ${PARTNER_CREDIT_OFFICERS_REQUIRED}


JD-TC-ActivatePartner-UH5
                                  
    [Documentation]               Activate Partner where active status is false
    ...
    ${resp}=    Update Credit Officer    ${Puid2}      ${Creditofficer}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200

    ${resp}=    Activate Partner    ${Puid2}     ${bool[0]}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}    ${ALREADY_UPDATED}


JD-TC-ActivatePartner-UH6
                                  
    [Documentation]               Activate Partner with fake partner id

    ${resp}=  Encrypted Provider Login  ${HLPUSERNAME2}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${partuid3}=    FakerLibrary.Random Number

    ${resp}=    Activate Partner    ${partuid3}     ${bool[0]}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
