...
# <<<--------------------update partner ------------------------------------------------------->>>
    ...
    ${resp}=    Identify Partner    ${P_phone}    ${account_id}    ${Pid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 

JD-TC-Identify-Partner-UH1
    ...
    ${resp}=    Identify Partner    ${empty}    ${account_id}    ${Pid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}    ${LOGINID_REQUIRED} 

JD-TC-Identify-Partner-UH2
    ...
    ${resp}=    Identify Partner    ${P_phone}    ${empty}    ${Pid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}    ${ACC_DOSENT_EXT} 

JD-TC-Identify-Partner-UH3
    ...
    ${resp}=    Identify Partner    ${P_phone}    ${account_id}    ${empty}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
