...
JD-TC-Approve Partner-1
    ...
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
    Should Be Equal As Strings   ${resp.json()["id"]}   ${Pid1}
    Should Be Equal As Strings   ${resp.json()["uid"]}   ${Puid1}
    Should Be Equal As Strings   ${resp.json()["partnerName"]}   ${firstName}
    Should Be Equal As Strings   ${resp.json()["partnerAliasName"]}   ${lastName}
    Should Be Equal As Strings   ${resp.json()["partnerMobile"]}   ${P_phone}



JD-TC-GetPartnerbyUID-UH1
                                  
    [Documentation]               get a partner after invalid uid
    ...
    ${resp}=   Get Partner by UID    ${inv_uid} 
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings   ${resp.json()}    ${INVALID_PARTNER_ID}


JD-TC-GetPartnerbyUID-UH3
                                  
    [Documentation]               get a partner without login
...
JD-TC-GetPartnerbyUID-UH4
                                  
    [Documentation]               get a partner with another user login
