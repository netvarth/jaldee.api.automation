...
JD-TC-GenerateEquifax-1

    [Documentation]             Generate Equifax
    ...
    ${resp}=    Generate EQUIFAX LOS  ${lead_uid} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200


JD-TC-GenerateEquifax-UH1

    [Documentation]             Generate Equifax- lead uid is invalid
    ...
    ${resp}=    Generate EQUIFAX LOS  ${fake} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   422
    Should Be Equal As Strings    ${resp.json()}        ${INVALID_X_ID}

JD-TC-GenerateEquifax-UH2

    [Documentation]             Generate Equifax- without login
...
JD-TC-GenerateEquifax-UH3

    [Documentation]             Generate Equifax- with another provider login
