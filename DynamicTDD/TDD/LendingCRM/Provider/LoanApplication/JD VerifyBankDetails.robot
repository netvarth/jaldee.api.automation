...
# ..... SignUp Business Head
    ...
    ${NBFCPUSERNAME1}=  Evaluate  ${PUSERNAME}+2497521
...
JD-TC-Verify Bank Details-UH1
    ...
    [Documentation]             Verify bank details - same again 
...
JD-TC-Verify Bank Details-UH2
    ...
    [Documentation]             Verify bank details - uid is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=    Verify loan Bank           ${empty}    ${bankAccountNo}    ${bankIfsc}    bankName=${bankName}    bankBranchName=${bankBranchName}    loanApplicationUid=${loanuid}
    Log  ${resp.content}
...
JD-TC-Verify Bank Details-UH3
    ...
    [Documentation]             Verify bank details - uid is invalid
    ...
    ${resp}=    Verify loan Bank           ${fake}    ${bankAccountNo}    ${bankIfsc}    bankName=${bankName}    bankBranchName=${bankBranchName}    loanApplicationUid=${loanuid}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422

JD-TC-Verify Bank Details-UH4
                                  
    [Documentation]             Verify bank details - account number is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=    Verify loan Bank           ${loanuid}    ${empty}    ${bankIfsc}    bankName=${bankName}    bankBranchName=${bankBranchName}    loanApplicationUid=${loanuid}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
...
JD-TC-Verify Bank Details-UH5
    ...
    [Documentation]             Verify bank details - ifsc is empty

    ${resp}=  Encrypted Provider Login     ${SO_USERNAME}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings             ${resp.status_code}    200

    ${resp}=    Verify loan Bank           ${loanuid}    ${bankAccountNo}    ${empty}    bankName=${bankName}    bankBranchName=${bankBranchName}    loanApplicationUid=${loanuid}
    Log  ${resp.content}
    Should Be Equal As Strings             ${resp.status_code}    422
...
JD-TC-Verify Bank Details-UH6
    ...
    [Documentation]             Verify bank details - without login 
