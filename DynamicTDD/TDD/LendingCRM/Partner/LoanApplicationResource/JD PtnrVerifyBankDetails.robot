...
JD-TC-Partner Verify Bank Details-1
                                  
    [Documentation]              Partner Verify Bank Details
...
JD-TC-Partner Verify Bank Details-UH1
                                  
    [Documentation]              Partner Verify Bank Details with invalid Loan uid

    ${resp}=    Login Partner with Password    ${account_id}  ${P_phone}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 

    ${floanid}=    FakerLibrary.Random Number

    ${INVALID_ORGIN_UID}=  Format String   ${INVALID_ORIGIN_UID}    ${floanid}

    ${resp}=    Verify Partner loan Bank   ${floanid}    ${bankName}    ${AccountNumber}    ${bankIfsc1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings  "${resp.json()}"  "${INVALID_ORGIN_UID}"


JD-TC-Partner Verify Bank Details-UH2
                                  
    [Documentation]              Partner Verify Bank Details with empty bank name
    ...
    ${resp}=    Verify Partner loan Bank   ${loanuid}    ${empty}    ${AccountNumber}    ${bankIfsc1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
...
JD-TC-Partner Verify Bank Details-UH3
                                  
    [Documentation]              Partner Verify Bank Details with empty account number
    ...
    ${resp}=    Verify Partner loan Bank   ${loanuid}    ${bankName}    ${empty}    ${bankIfsc1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}    ${ACCOUNR_NUMBER_REQ}


JD-TC-Partner Verify Bank Details-UH4
                                  
    [Documentation]              Partner Verify Bank Details with empty ifsc code
    ...
    ${resp}=    Verify Partner loan Bank   ${loanuid}    ${bankName}    ${AccountNumber}    ${empty}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
