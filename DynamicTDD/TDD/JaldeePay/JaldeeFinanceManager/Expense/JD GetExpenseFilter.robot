...
JD-TC-GetExpenseWithFilter-1
    ...
    Set Suite Variable  ${expenseStatus}   ${resp.json()['expenseStatus']}
 

    ${resp}=  Get Expense With Filter    categoryName-eq=${name}   userId-eq=${userId}  expenseStatus-eq=${expenseStatus}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-GetExpenseWithFilter-3
    ...
    Should Be Equal As Strings  ${resp.json()[0]['amount']}  ${amount}
    Should Be Equal As Strings  ${resp.json()[0]['expenseDate']}  ${expenseDate}
    Should Be Equal As Strings  ${resp.json()[0]['expenseFor']}  ${EMPTY}
    Should Be Equal As Strings  ${resp.json()[0]['itemList'][0]['quantity']}  ${quantity}
    Should Be Equal As Strings  ${resp.json()[0]['departmentList'][0]['deptId']}  ${deptId}
    Should Be Equal As Strings  ${resp.json()[0]['uploadedDocuments'][0]['fileName']}  ${pdffile}
JD-TC-GetExpenseWithFilter-4
...
JD-TC-GetExpenseWithFilter-UH1

    [Documentation]  GetExpenseWithFilter  without login.

    ${resp}=  Get Expense With Filter   
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  419
    Should Be Equal As Strings   ${resp.json()}   ${SESSION_EXPIRED}

JD-TC-GetExpenseWithFilter-UH2
