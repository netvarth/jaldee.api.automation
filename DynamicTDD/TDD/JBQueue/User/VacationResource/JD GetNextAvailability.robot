...
JD-TC-GetNextAvailability-1
    ...
    ${PUSERNAME_E1}=  Evaluate  ${PUSERNAME}+778805575
    ...
    ${ph1}=  Evaluate  ${PUSERNAME_E1}+1000880001
    ...
    ${resp}=  Get Vacation Next Availability   ${recurringtype[1]}  ${list}  ${DAY1}  ${DAY2}  ${EMPTY}  ${sTime1}  ${eTime1}  ${recurringtype[1]}  ${list}  ${CUR_DAY}  ${CUR_DAY}  ${EMPTY}  ${start_time}  ${end_time}  
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings   ${resp.json()['availableDate']}                     ${CUR_DAY}
    Should Be Equal As Strings   ${resp.json()['timeSlots'][0]['sTime']}             ${sTime1}  
    Should Be Equal As Strings   ${resp.json()['timeSlots'][0]['eTime']}             ${start_time}
