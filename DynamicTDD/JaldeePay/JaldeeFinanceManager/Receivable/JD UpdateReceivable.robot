...
Update Receivable
    ...
    [Arguments]    ${uid}    ${amount}    ${receivableCategoryId}   ${dueDate}   ${receivableLabel}     ${vendorId}       &{kwargs}
    ${data}=  Create Dictionary  amount=${amount}    receivableCategoryId=${receivableCategoryId}   dueDate=${dueDate}   receivableLabel=${receivableLabel}      vendorId=${vendorId}
    FOR  ${key}  ${value}  IN  &{kwargs}
        Set To Dictionary  ${data}   ${key}=${value}
    END
    ${data}=    json.dumps    ${data}   
    Check And Create YNW Session
    ${resp}=    PUT On Session    ynw    /provider/jp/finance/receivable/${uid}    data=${data}  expected_status=any    headers=${headers}
    [Return]  ${resp}
