...
JD-TC-RemoveServiceLevelDiscountForAppointmnet-2
    [Documentation]   Apply service level discount for future appointment(online),then remove that discount.
    ...
    ${cnote}=   FakerLibrary.name
    ${resp}=   Take Appointment For Provider   ${pid}  ${s_id}  ${sch_id}  ${DAY3}  ${cnote}   ${apptfor}    location=${lid}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-RemoveServiceLevelDiscountForAppointmnet-3
    [Documentation]   Try to remove that discount again.
...
JD-TC-RemoveServiceLevelDiscountForAppointmnet-4
    [Documentation]   Apply service level discount,where discount price is empty,then remove that discount.
    ...
    ${resp}=   Remove Service Level Discount for Appointment    ${apptid1}    ${discountId1}   ${discountprice}    ${discount1}    ${discount1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-RemoveServiceLevelDiscountForAppointmnet-5
    [Documentation]   Apply service level discount,where both note is empty,then remove that discount.
...
JD-TC-RemoveServiceLevelDiscountForAppointmnet-6
    [Documentation]   Apply service level discount,where both note is empty,then remove that discount with invalid discount id..
