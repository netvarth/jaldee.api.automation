...
JD-TC-WaitlistAdvancePaymentdetails-1
    ...
    ${resp}=  Create Service  ${SERVICE2}  ${description}   ${ser_durtn}  ${bool[0]}  ${ser_amount}  ${bool[1]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Set Suite Variable    ${s_id2}  ${resp.json()}
    ${resp}=  Create Service  ${SERVICE3}  ${description}   ${ser_durtn}  ${bool[0]}  ${ser_amount}  ${bool[1]}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Set Suite Variable    ${s_id3}  ${resp.json()}
    
    ${resp}=  Create Service  ${SERVICE5}  ${description}   ${ser_durtn}  ${bool[0]}  ${ser_amount}  ${bool[1]}
    Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-WaitlistAdvancePaymentdetails-4
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['netTotal']}                              ${amt}
    Should Be Equal As Strings  ${resp.json()['amountRequiredNow']}                     ${amt}
    Should Be Equal As Strings  ${resp.json()['jdnDiscount']}                           0.0
    Should Be Equal As Strings  ${resp.json()['couponDiscount']}                        0.0
...
JD-TC-WaitlistAdvancePaymentdetails-5
    ...
    ${amt}=  Evaluate   ${amt} + ${totalTaxAmount}
    ${nettotal}=  Evaluate  ${ser_amount} + ${totalTaxAmount} - ${pc_amount}
    ${nettotal}=  roundoff  ${nettotal}
