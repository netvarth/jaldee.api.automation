...
JD-TC- AppointmentAdvancePaymentdetails-1
    ...
    ${nettotal}=  Evaluate  ${ser_amount} + ${totalTaxAmount}
    ...
    Should Be Equal As Strings  ${resp.json()['servicePrice']}                          ${ser_amount}
...
JD-TC-AppointmentAdvancePaymentdetails-4
    ...
    ${amount}=    Convert To Number  ${amt}  2
    #  ${ser_amount1}=  Convert To Number  ${ser_amount1}  1
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['netTotal']}                              ${amount}
...
JD-TC-AppointmentAdvancePaymentdetails-5
    ...
    FOR   ${i}  IN RANGE   ${len}
        ${resp1}=   Get Service By Id  ${resp.json()[${i}]['id']}
        Log  ${resp1.content}
    ...
    Log Many  ${ser_amount}   ${min_pre}   ${pc_amount}  ${Jamount}
    ${totalTaxAmount}=  Evaluate  ${ser_amount} * ${gstpercentage[2]} / 100
    ${totalDiscount}=   Evaluate  ${pc_amount} + ${Jamount} 
    ${amt}=  Evaluate   ${min_pre} - ${totalDiscount}
    ${amt}=  Evaluate   ${amt} + ${totalTaxAmount}
    ${amount}=   Evaluate   ${ser_amount} - ${pc_amount}
    ${nettotal}=  Evaluate  ${amount} + ${totalTaxAmount}
    ${nettotal}=  roundoff  ${nettotal}
...
JD-TC-AppointmentAdvancePaymentdetails-6
    ...
    ${nettotal}=  Evaluate  ${taxableamt} + ${totalTaxAmount}
    ${nettotal}=  roundoff  ${nettotal}  
...
JD-TC-AppointmentAdvancePaymentdetails-7
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Numbers  ${resp.json()['netTotal']}                              ${nettotal}
    Should Be Equal As Strings  ${resp.json()['amountRequiredNow']}                     ${min_pre}
    Should Be Equal As Strings  ${resp.json()['jdnDiscount']}                           0.0
    Should Be Equal As Strings  ${resp.json()['couponDiscount']}                        0.0
    Should Be Equal As Strings  ${resp.json()['providerCouponDiscount']}                0.0
...
JD-TC-AppointmentAdvancePaymentdetails-UH6
    ...
    ${amt}=  Evaluate   ${ser_amount1} + ${totalTaxAmount}
    ${amount}=    Convert To Number  ${amt}  2
...
JD-TC-AppointmentAdvancePaymentdetails-8
    ...
    ${servicecharge}=   Pyfloat  right_digits=1  min_value=100  max_value=150
    # ${ser_amount1}=   Random Int   min=100   max=150
    # ${ser_amount1}=  Convert To Number  ${ser_amount1}  1
    Set Suite Variable    ${ser_amount1}  
    ${resp}=  Create Service  ServiceName5  ${description}   ${ser_durtn}  ${status[0]}  ${btype}  ${notify}   ${notifytype}  ${min_pre_percent}  ${servicecharge}  ${bool[1]}  ${bool[0]}  prePaymentType=${advancepaymenttype[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Set Suite Variable    ${s_id5}  ${resp.json()}
    ...
    ${bool1}=  Random Element  ${bool}
    ${list}=  Create List  1  2  3  4  5  6  7
    ${resp}=  Create Appointment Schedule  ${schedule_name}  ${recurringtype[1]}  ${list}  ${DAY1}  ${DAY2}  ${EMPTY}  ${sTime1}  ${eTime1}  ${parallel}    ${parallel}  ${lid}  ${duration}  ${bool1}  ${s_id5}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Get Appointment Schedule ById Consumer  ${sch_id4}   ${pid}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=  Get Next Available Appointment Slots By ScheduleId  ${sch_id4}   ${pid}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${no_of_slots}=  Get Length  ${resp.json()['availableSlots']}
    @{slots}=  Create List
    ...
    ${pre_amt}=   Evaluate    ${servicecharge} * ${min_pre_percent} / 100
    ${pre_amt}=    Convert To Number  ${pre_amt}  2

    ${cid}=  get_id  ${CUSERNAME3}   
    ${cnote}=   FakerLibrary.name
    # ${coupons}=  Create List   ${cupn_code}
    ${resp}=   Appointment AdvancePayment Details without Coupon   ${pid}  ${s_id5}  ${sch_id4}  ${DAY1}  ${cnote}   ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['netTotal']}                              ${servicecharge}
    Should Be Equal As Numbers  ${resp.json()['amountRequiredNow']}                     ${pre_amt}
    Should Be Equal As Strings  ${resp.json()['jdnDiscount']}                           0.0
    Should Be Equal As Strings  ${resp.json()['couponDiscount']}                        0.0
    # Should Be Equal As Strings  ${resp.json()['providerCouponDiscount']}                ${pc_amount}
    # Should Be Equal As Strings  ${resp.json()['totalDiscount']}                         ${pc_amount}
...
JD-TC-AppointmentAdvancePaymentdetails-9
    ...
    ${notify}    Random Element     ['True','False'] 
    ${ser_durtn}=   Random Int   min=2   max=10
    # ${min_pre_percent}=   Pyfloat  right_digits=1  min_value=100  max_value=100
    ${min_pre_percent}=   Convert To Number  100  1
    ${servicecharge}=   Pyfloat  right_digits=1  min_value=100  max_value=150
    # ${ser_amount1}=   Random Int   min=100   max=150
    # ${ser_amount1}=  Convert To Number  ${ser_amount1}  1
    Set Suite Variable    ${ser_amount1}  
    ${resp}=  Create Service  ServiceName6  ${description}   ${ser_durtn}  ${status[0]}  ${btype}  ${notify}   ${notifytype}  ${min_pre_percent}  ${servicecharge}  ${bool[1]}  ${bool[0]}  prePaymentType=${advancepaymenttype[0]}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    ${bool1}=  Random Element  ${bool}
    ${list}=  Create List  1  2  3  4  5  6  7
    ${resp}=  Create Appointment Schedule  ${schedule_name}  ${recurringtype[1]}  ${list}  ${DAY1}  ${DAY2}  ${EMPTY}  ${sTime1}  ${eTime1}  ${parallel}    ${parallel}  ${lid}  ${duration}  ${bool1}  ${s_id6}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}  id=${sch_id5}   name=${schedule_name}  apptState=${Qstate[0]}
    ...
    ${resp}=  Get Appointment Schedule ById Consumer  ${sch_id5}   ${pid}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=  Get Next Available Appointment Slots By ScheduleId  ${sch_id5}   ${pid}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${no_of_slots}=  Get Length  ${resp.json()['availableSlots']}
    @{slots}=  Create List
    ...
    ${cnote}=   FakerLibrary.name
    # ${coupons}=  Create List   ${cupn_code}
    ${resp}=   Appointment AdvancePayment Details without Coupon   ${pid}  ${s_id6}  ${sch_id5}  ${DAY1}  ${cnote}   ${apptfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['netTotal']}                              ${servicecharge}
    Should Be Equal As Numbers  ${resp.json()['amountRequiredNow']}                     ${pre_amt}
