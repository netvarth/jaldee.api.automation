...
JD-TC-GetPaymentModes-1

    [Documentation]  Get payment modes by consumer for bill payment purpose.
    ...
    ${resp}=   Get payment modes  ${pid}   ${ser_id1}   ${Payment_Purpose[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    

JD-TC-GetPaymentModes-UH1

    [Documentation]  Get payment modes by consumer without create a payment profile for the provider.
    ...
    ${resp}=   Get payment modes  ${pid}   ${ser_id}   ${Payment_Purpose[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
