...
JD-TC-GetVacation-3
    ...
    ${resp}=  Create Vacation   ${desc1}  ${u_id1}  ${recurringtype[1]}  ${list}  ${CUR_day1}  ${CUR_day}  ${EMPTY}  ${start_time1}  ${end_time1}  
    Log  ${resp.json()}
    Should Be Equal As Strings   ${resp.status_code}     200
    Set Suite Variable  ${v_id01}  ${resp.json()['holidayId']}
    ...
    ${resp}=  Create Vacation   ${desc2}  ${u_id1}  ${recurringtype[1]}  ${list}  ${DAY2}  ${DAY2}  ${EMPTY}  ${start_time2}  ${end_time2} 
    Log  ${resp.json()}
    Should Be Equal As Strings   ${resp.status_code}     200
