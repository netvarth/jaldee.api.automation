...
JD-TC-Reschedule Waitlist-1
    ...
    ${hrs}  ${mins}=   Convert_hour_mins   ${waitingtime}
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg}  [username]   ${uname} 
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg1}  [wait/service]  wait
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg1}  [service]   ${SERVICE1}
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg1}  [date]   ${date}
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg1}  [estTime]   ${hrs}${SPACE}hrs${SPACE}${mins}${SPACE}mins
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg1}  [qTime]  ${sTime1}-${eTime1}
    # ${defcheckInML_msg1}=  Replace String  ${defcheckInML_msg1}  [provider name]   ${bsname}

    # ${defreshedule_checkin_msg}=  Replace String  ${defreshedule_checkin_msg}  [username]   ${uname}
    # ${defreshedule_checkin_msg}=  Replace String  ${defreshedule_checkin_msg}  [consumername]  ${uname}
    # ${defreshedule_checkin_msg}=  Replace String  ${defreshedule_checkin_msg}  [service]   ${SERVICE1}
    # ${defreshedule_checkin_msg}=  Replace String  ${defreshedule_checkin_msg}  [date]   ${date}
    # ${defreshedule_checkin_msg}=  Replace String  ${defreshedule_checkin_msg}  [time]   ${sTime1}
    # ${defreshedule_checkin_msg}=  Replace String  ${defreshedule_checkin_msg}  [provider name]   ${bsname}
...
JD-TC-Reschedule Waitlist-2
    ...
    Verify Response  ${resp}  date=${DAY3}  waitlistStatus=${wl_status[0]}  
    ...   waitlistedBy=${waitlistedby[1]}   personsAhead=0   #checkInTime=${sTime1}  
    ...   consLastVisitedDate=${date2}${SPACE}${sTime1}  appxWaitingTime=0  serviceTime=${sTime1}
    Should Be Equal As Strings  ${resp.json()['service']['id']}                   ${s_id}
    ...
    ...   consLastVisitedDate=${date2}${SPACE}${sTime1}  appxWaitingTime=${wait_time}  serviceTime=${sTime2}
    Should Be Equal As Strings  ${resp.json()['service']['name']}                 ${SERVICE1}
    Should Be Equal As Strings  ${resp.json()['service']['id']}                   ${s_id}
    Should Be Equal As Strings  ${resp.json()['consumer']['id']}                  ${cid}
    Should Be Equal As Strings  ${resp.json()['waitlistingFor'][0]['id']}         ${cid}
...
JD-TC-Reschedule Waitlist-3
    ...
    Verify Response  ${resp}  date=${DAY1}  waitlistStatus=${wl_status[1]}  
    ...   waitlistedBy=${waitlistedby[1]}   personsAhead=0   #checkInTime=${sTime1}  
    ...   consLastVisitedDate=${date1}${SPACE}${sTime2}  serviceTime=${sTime2}
    Should Be Equal As Strings  ${resp.json()['service']['name']}                 ${SERVICE1}
    Should Be Equal As Strings  ${resp.json()['service']['id']}                   ${s_id}
    Should Be Equal As Strings  ${resp.json()['consumer']['id']}                  ${cid}
    Should Be Equal As Strings  ${resp.json()['waitlistingFor'][0]['id']}         ${cid}
...
JD-TC-Reschedule Waitlist-5
    ...
    ${resp}=  Make payment Consumer Mock  ${pid}  ${servicecharge}  ${purpose[0]}  ${wid}  ${s_id}  ${bool[0]}   ${bool[1]}  ${jdconID}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Set Test Variable   ${payref}   ${resp.json()['paymentRefId']}
...
JD-TC-Reschedule Waitlist-7
    ...
    ${resp}=  AddCustomer  ${CUSERNAME13}  firstname=${fname1}  lastname=${lname1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${cid1}   ${resp.json()}
...
JD-TC-Reschedule Waitlist-9
    ...
    ${resp}=  Provider Login  ${multilocPro[2]}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    clear_provider_msgs  ${multilocPro[2]}
    clear_consumer_msgs  ${CUSERNAME12}
...
JD-TC-Reschedule Waitlist-11
    [Documentation]  Provider takes checkin for a consumer and reschedules it it to a future date when future checkin is disabled
...
JD-TC-Reschedule Waitlist-UH6
    [Documentation]  Provider takes checkin for a consumer and reschedules it with the same details twice
...
JD-TC-Reschedule Waitlist-UH22
    [Documentation]  Provider takes future checkin for a consumer and reschedules it to today when today checkin is disabled
