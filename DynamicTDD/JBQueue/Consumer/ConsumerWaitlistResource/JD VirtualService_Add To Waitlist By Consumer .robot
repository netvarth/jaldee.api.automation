...
JD-TC-VirtualService_Add To WaitlistByConsumer-UH18  
    ...
    ${sTime1}=  subtract_timezone_time  ${p1_tz1}  0  30
    ${eTime1}=  subtract_timezone_time  ${p1_tz1}   0  10
...
JD-TC-VirtualService_Add To WaitlistByConsumer-UH22
    ...
    Set Suite Variable  ${virtualService1}
    ${FUTURE_DATE}=  db.add_timezone_date  ${tz}  10  
    ${consumerNote1}=   FakerLibrary.word
...
JD-TC-VirtualService_Add To WaitlistByProvider-21
    ...
    ${eTime1}=  add_timezone_time  ${p1_tz1}  6  30  
    ${p1queue1}=    FakerLibrary.word
    ${capacity}=  FakerLibrary.Numerify  %%
    ${list}=  Create List  1  2  3  4  5  6  7
    ...
    ${consumerNote1}=   FakerLibrary.word
    ${resp}=  Consumer Add To WL With Virtual Service  ${acc_id}  ${QId}  ${DAY}  ${p1_s1}  ${consumerNote1}  ${bool[0]}  ${virtualService1}   0
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    ...
    ${resp}=  Get consumer Waitlist By Id   ${wid1}  ${acc_id}   
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}  date=${DAY}  waitlistStatus=${wl_status[0]}  partySize=1  waitlistedBy=CONSUMER
    Should Be Equal As Strings  ${resp.json()['service']['name']}  ${P1SERVICE1}
    ...
    ${consumerNote2}=   FakerLibrary.word
    ${resp}=  Consumer Add To WL With Virtual Service  ${acc_id}  ${QId}  ${DAY}  ${p1_s1}  ${consumerNote2}  ${bool[0]}  ${virtualService1}   ${cidfor}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    
    ${wid}=  Get Dictionary Values  ${resp.json()}
    Set Test Variable  ${wid2}  ${wid[0]}
    ${resp}=  Get consumer Waitlist By Id   ${wid2}  ${acc_id}   
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}  date=${DAY}  waitlistStatus=${wl_status[0]}  partySize=1    waitlistedBy=CONSUMER 
    Should Be Equal As Strings  ${resp.json()['service']['name']}  ${P1SERVICE1}
    ...
    ${resp}=  Cancel Waitlist  ${wid1}  ${acc_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    ${resp}=  Cancel Waitlist  ${wid2}  ${acc_id}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
