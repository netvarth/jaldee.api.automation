...
JD-TC-GetItemTaxByFilter-1

    [Documentation]  SA Get Item Tax By Filter SA - tax name
...
JD-TC-GetItemTaxByFilter-2
    ...
    ${resp}=    Get Item Tax By Filter SA  ${account_id}   taxCode-eq=${itemtax_id}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()[0]['taxCode']}            ${itemtax_id}
    Should Be Equal As Strings    ${resp.json()[0]['taxName']}            ${taxName}
...
JD-TC-GetItemTaxByFilter-3
    ...
    ${resp}=    Get Item Tax By Filter SA  ${account_id}   taxName-eq=${taxName}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings    ${resp.json()[0]['taxCode']}            ${itemtax_id}
    Should Be Equal As Strings    ${resp.json()[0]['taxName']}            ${taxName}
...
JD-TC-GetItemTaxByFilter-4
    ...
    ${resp}=    Get Item Tax By Filter SA  ${account_id}   status-eq=${toggle[1]}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    

JD-TC-GetItemTaxByFilter-UH1
