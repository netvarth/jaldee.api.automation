...
JD-TC-GetQuestionsByID-1
    ...
    ${qns}   getColumnValuesByName  ${sheet1}  ${colnames[6]}
    Log  ${qns}
    # ${qns_len}=  Get Length  ${qns}
    ${qnr_ids}   getColumnValuesByName  ${sheet1}  ${colnames[0]}
    Log  ${qnr_ids}
    Remove Values From List  ${qnr_ids}   ${NONE}
    ${unique_qnrids}=    Remove Duplicates    ${qnr_ids}
    Log  ${unique_qnrids}
    ...
    FOR  ${i}  IN RANGE   1  ${j}
        ...
        Should Be Equal As Strings  ${qns.json()['transactionType']}   ${transactionTypeVal[3]}
        Should Be Equal As Strings  ${qns.json()['channel']}   ${ChannelVal[3]}
...
JD-TC-GetQuestionsByID-2
    ...
    ${servicenames1}   getColumnValuesByName  ${sheet2}  ${colnames1[7]}
    Log   ${servicenames1}
    ${resp}=  Provider Login  ${PUSERNAME3}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    Log  ${unique_snames1}
    ${snames_len}=  Get Length  ${unique_snames1}
    FOR  ${i}  IN RANGE   ${snames_len}
        ${kwstatus} 	${value} = 	Run Keyword And Ignore Error  List Should Contain Value  ${snames}  ${unique_snames[${i}]}
        Log Many  ${kwstatus} 	${value}
        Continue For Loop If  '${kwstatus}' == 'PASS'
    ...
    ${qns1}   getColumnValuesByName  ${sheet2}  ${colnames1[1]}
    Log  ${qns1}
    # ${qns1_len}=  Get Length  ${qns1}
    ${qnr_ids1}   getColumnValuesByName  ${sheet2}  ${colnames1[0]}
    Log  ${qnr_ids1}
    Remove Values From List  ${qnr_ids1}   ${NONE}
    ${unique_qnrids1}=    Remove Duplicates    ${qnr_ids1}
    Log  ${unique_qnrids1}
    ...
    FOR  ${i}  IN RANGE   1  ${j}
        ${qns1}   Get Questionnaire By Id  ${account_id}  ${i}  
        Log  ${qns1.json()}
        ...
        ${transactionTypeVal1}=  Run Keyword If   '${status}' == 'PASS'   getColumnValueByAnotherVal  ${sheet2}  ${colnames1[5]}  ${colnames1[0]}  ${qnr_id1}
        ...   ELSE  getColumnValueByAnotherVal  ${sheet1}  ${colnames[5]}  ${colnames[0]}  ${qnr_id1}
        Log  ${transactionTypeVal1}
        ${ChannelVal1}=  Run Keyword If   '${status}' == 'PASS'   getColumnValueByAnotherVal  ${sheet2}  ${colnames1[6]}  ${colnames1[0]}  ${qnr_id1}
        ...   ELSE  getColumnValueByAnotherVal  ${sheet1}  ${colnames[6]}  ${colnames[0]}  ${qnr_id1}
        Log  ${ChannelVal1}
