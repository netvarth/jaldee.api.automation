...
${SERVICE1}  Note Book112312
${SERVICE2}  boots137343
${SERVICE3}  pen10376321
${SERVICE4}  ABCD19234
${queue1}  morning12
${LsTime}   08:00 AM
${LeTime}   09:00 AM
...
JD-RemoveRecreatedReimburseReport1


    [Documentation]  Remove Recreated Reimburs Report with payment
    ...
    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+458812
    ...
    ${resp}=  Recreate reimburse report   ${eg_id}
    Should Be Equal As Strings  ${resp.status_code}  200
    #${recreate_date}=   db.get_tz_time_secs  ${tz}  

    ${resp}=  Remove reimburse report  ${eg_id}
    Should Be Equal As Strings  ${resp.status_code}  200
    sleep  2s
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()[0]['reportStatus']}   CANCELED
    #Should Be Equal As Strings  ${resp.json()[0]['reCreateDate']}   ${recreate_date}


JD-RemoveRecreatedReimburseReport2

    [Documentation]   Remove Recreated Reimburs Report without payment 
     ${billable_domains}=  get_billable_domain
    Set Test Variable  ${domains}  ${billable_domains[0]}
    Set Test Variable  ${sub_domains}   ${billable_domains[1]}
    ${firstname}=  FakerLibrary.first_name
    ...
    ${dob}=  FakerLibrary.Date
    ${gender}    Random Element    ${Genderlist}
    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+45698
    Append To File  ${EXECDIR}/TDD/numbers.txt  ${PUSERPH0}${\n}
    ${licid}=  get_highest_license_pkg
    ${resp}=  Account SignUp  ${firstname}  ${lastname}  ${None}  ${domains[2]}  ${sub_domains[2]}  ${PUSERPH0}  ${licid[0]}
    Log   ${resp.json()}
    ...
    Set Suite Variable  ${list}  ${list}
    @{Views}=  Create List  self  all  customersOnly
    ${ph1}=  Evaluate  ${PUSERPH0}+7100000000
    ${ph2}=  Evaluate  ${PUSERPH0}+8100000000
    ${views}=  Evaluate  random.choice($Views)  random
    ${name1}=  FakerLibrary.name
    ${name2}=  FakerLibrary.name
    ${name3}=  FakerLibrary.name
    ...
    ${start}=  db.add_tz_time24  ${tz}   0  -5
    # ${end}=  db.add_tz_time_sec  ${tz}   0  0  0
    # ${start}=  db.add_tz_time_sec  ${tz}   0  -10  0
    ${resp}=  Create Reimburse Reports By Provider  ${start}  ${end}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    sleep  02s
