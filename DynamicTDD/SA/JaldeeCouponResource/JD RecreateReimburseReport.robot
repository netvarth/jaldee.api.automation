...
JD-RecreateReimburseReport1

    [Documentation]  Recreate Reimburs Report
    ...
    ${ph1}=  Evaluate  ${PUSERPH0}+3000000000
    ${ph2}=  Evaluate  ${PUSERPH0}+4000000000
...
JD-RecreateReimburseReport2

  [Documentation]     Recreate Reimburs Report without Payment
    ...
    Set Suite Variable  ${list}  ${list}
    @{Views}=  Create List  self  all  customersOnly
    ${ph1}=  Evaluate  ${PUSERPH0}+7000000000
    ${ph2}=  Evaluate  ${PUSERPH0}+8000000000
    ${views}=  Evaluate  random.choice($Views)  random
    ${name1}=  FakerLibrary.name
    ${name2}=  FakerLibrary.name
    ${name3}=  FakerLibrary.name
