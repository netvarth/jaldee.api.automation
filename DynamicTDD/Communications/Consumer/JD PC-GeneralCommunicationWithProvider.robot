...
JD-TC-General Communication with Provider-1
    ...
    ${resp}=    Send Otp For Login    ${CUSERNAME3}    ${acc_id}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME3}    ${acc_id}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    clear_Consumermsg  ${CUSERNAME3}
    
    
    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME3}    ${acc_id}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-General Communication with Provider-2
    ...
    Should Be Equal As Strings  ${resp.json()[1]['owner']['id']}    ${con_id}
    ...
    Should Contain  ${resp.json()[1]['attachements'][0]['s3path']}   .png
...
JD-TC-General Communication with Provider-3
    ...
    IF   '${resp.content}' == '${emptylist}'
        ...
        Log  ${resp1.content}
        Should Be Equal As Strings  ${resp1.status_code}  200
        Set Suite Variable  ${cid4}   ${resp1.json()}
    ELSE
    ...
    ${resp}=    Send Otp For Login    ${CUSERNAME4}    ${acc_id}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
  
    ${resp}=    Verify Otp For Login   ${CUSERNAME4}   12  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable   ${token}  ${resp.json()['token']}
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME4}    ${acc_id}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    clear_Consumermsg  ${CUSERNAME4}

    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME4}    ${acc_id}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${con_id}   ${resp.json()['id']} 
...
JD-TC-General Communication with Provider-8
    ...
    ${resp}=  GetCustomer   account-eq=${con_id} 
    Should Be Equal As Strings  ${resp.status_code}  200  
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.json()[0]['phoneNo']}        ${CUSERNAME4} 
