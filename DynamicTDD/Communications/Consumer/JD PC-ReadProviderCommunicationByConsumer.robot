...
JD-TC-Read Provider Communications-INDEPENDENT_SP-1
    ...
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=    Send Otp For Login    ${CUSERNAME16}    ${account_id1}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
  
    ${resp}=    Verify Otp For Login   ${CUSERNAME16}   12  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME16}    ${account_id1}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200

    
    
    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME16}    ${account_id1}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-Read Provider Communications-INDEPENDENT_SP-2
    ...
    ${resp}=    Send Otp For Login    ${CUSERNAME16}    ${account_id2}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME16}    ${account_id2}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    # clear_Consumermsg  ${CUSERNAME16}
    

    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME16}    ${account_id2}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${cR_id1}   ${resp.json()['id']} 
...
JD-TC-Read Provider Communications-INDEPENDENT_SP-3
    ...
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${cr16}   ${resp.json()['id']} 
   ...
   ${resp}=    Send Otp For Login    ${CUSERNAME18}    ${account_id1}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME18}    ${account_id1}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    # clear_Consumermsg  ${CUSERNAME18}
    

    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME18}    ${account_id1}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${cr18}   ${resp.json()['id']} 
...
JD-TC-Read Provider Communications-BRANCH-4
    ...
    ${PUSERNAME_E1}=  Evaluate  ${PUSERNAME}+60390907
    ...
    Set Test Variable  ${email_id}  ${P_Email}${PUSERNAME_E1}.${test_mail}
    ...
    # Set Suite Variable  ${pR_id1}  ${resp.json()['id']}
    ${account_id11}=  get_acc_id  ${PUSERNAME_E1}
    Set Suite Variable  ${account_id11}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=    Send Otp For Login    ${CUSERNAME19}    ${account_id11}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME19}    ${account_id11}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    # clear_Consumermsg  ${CUSERNAME19}
    

    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME19}    ${account_id11}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${con_id}   ${resp.json()['id']} 
    ...
    Verify Response List  ${resp}  0  accountId=${account_id11}  msg=${msg1}
    Should Be Equal As Strings  ${resp.json()[0]['owner']['id']}    ${p1_id}
    Should Be Equal As Strings  ${resp.json()[0]['receiver']['id']}    ${cR_id19}
    Set Test Variable  ${msgId1}  ${resp.json()[0]['messageId']}
    
    Verify Response List  ${resp}  1  accountId=${account_id11}  msg=${msg2}
    ...
    ${resp}=  Reading Provider Communications  ${p1_id}  ${account_id11}  ${msgId1}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=  Get Consumer Communications Unread Count
    Log  ${resp.content}
    ...
    ${resp}=  Reading Provider Communications  ${p2_id}  ${account_id11}  ${msgId2}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=  Get Consumer Communications Unread Count
    Log  ${resp.content}
...
JD-TC-Read Provider Communications-BRANCH-5
    ...
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=    Send Otp For Login    ${CUSERNAME17}    ${account_id11}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
  
    ${resp}=    Verify Otp For Login   ${CUSERNAME17}   12  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable   ${token}  ${resp.json()['token']}
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME17}    ${account_id11}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    # clear_Consumermsg  ${CUSERNAME17}
    

    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME17}    ${account_id11}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${con_id}   ${resp.json()['id']} 
    ...
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=    Send Otp For Login    ${CUSERNAME15}    ${account_id11}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
  
    ${resp}=    Verify Otp For Login   ${CUSERNAME15}   12  
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable   ${token}  ${resp.json()['token']}
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME15}    ${account_id11}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    # clear_Consumermsg  ${CUSERNAME15}
    

    ${cookie}  ${resp}=    Imageupload.ProconLogin    ${CUSERNAME15}    ${account_id11}    ${token}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Test Variable    ${cid}    ${resp.json()['providerConsumer']}
    Set Test Variable    ${cR_id5}   ${resp.json()['id']} 

    ${resp}=  Get Consumer Communications
    Log  ${resp.content}
    Verify Response List  ${resp}  0  accountId=${account_id11}  msg=${msg2}
    Should Be Equal As Strings  ${resp.json()[0]['owner']['id']}   ${p1_id}
    ...
    Should Be Equal As Strings  ${resp.json()}  1
    Should Be Equal As Strings    ${resp.status_code}    200
    ${resp}=  Reading Provider Communications  ${p1_id}   ${account_id11}  ${msgId2}  
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
