...
JD-TC-Partner Generate Email Otp-1
                                  
    [Documentation]              Partner Generate Emil Otp for Loan Application
...
JD-TC-Partner Generate Email Otp-UH1
                                  
    [Documentation]              Partner Generate Emil Otp for Loan Application with email as empty
    
    ${resp}=    Login Partner with Password    ${account_id}  ${P_phone}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 

    ${resp}=    Partner Otp For Email    ${empty}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}   ${EMAIL_ID_REQUIRED}


JD-TC-Partner Generate Email Otp-UH2
                                  
    [Documentation]              Partner Generate Emil Otp for Loan Application with email as number

    ${resp}=    Login Partner with Password    ${account_id}  ${P_phone}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 

    ${emailfake}    Random Number 	digits=5

    ${resp}=    Partner Otp For Email    ${emailfake}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}   ${INVALID_EMAIL}


JD-TC-Partner Generate Email Otp-UH3
                                  
    [Documentation]              Partner Generate Emil Otp for Loan Application with invalid email format

    ${resp}=    Login Partner with Password    ${account_id}  ${P_phone}  ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 

    ${emailformat}    FakerLibrary.firstname
    ${resp}=    Partner Otp For Email    ${emailformat}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}      ${INVALID_EMAIL}