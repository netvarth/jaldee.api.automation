...
JD-TC-AddWaitlistLabel-1
    ...
    # ${billable_providers}=    Billable Domain Providers   min=20   max=30
...
JD-TC-AddWaitlistLabel-2
    ...
    ${Future_Day}=  add_date  5
      ...
      Log  ${resp.json()}
      Should Be Equal As Strings  ${resp.status_code}  200
      Verify Response  ${resp}  date=${Future_Day}  waitlistStatus=${wl_status[0]} 
      Should Be Equal As Strings  ${resp.json()['service']['name']}                 ${SERVICE1}
      Should Be Equal As Strings  ${resp.json()['service']['id']}                   ${s_id1}
      Should Be Equal As Strings  ${resp.json()['consumer']['id']}                  ${cid}
      Should Be Equal As Strings  ${resp.json()['waitlistingFor'][0]['id']}         ${cid}
...
#     ${resp}=  Add Label for Waitlist   ${wid2}  ${labelname[0]}  ${label_value2}
#     Log   ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200

#     ${lblname}=  Convert To String  ${labelname[0]}
#     ${label2}=    Create Dictionary  ${lblname}=${label_value2}


#       ${resp}=  Get Waitlist By Id  ${wid2} 
#       Log  ${resp.json()}
#       Should Be Equal As Strings  ${resp.status_code}  200
#       Verify Response  ${resp}  ynwUuid=${wid2}  waitlistStatus=${wl_status[1]}  label=${label2}
...
JD-TC-AddWaitlistLabel-5
    [Documentation]  Remove Waitlist label from Today's checkin  and add the same label again in future.
    ${resp}=  Consumer Login  ${CUSERNAME12}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    Set Suite Variable  ${jdconID}   ${resp.json()['id']}
...
JD-TC-AddWaitlistLabel-UH4
    ...
    ${resp}=  Provider Login  ${PUSERNAME227}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
