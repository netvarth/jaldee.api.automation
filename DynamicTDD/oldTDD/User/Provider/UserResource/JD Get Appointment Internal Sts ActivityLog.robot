...
JD-TC-AppointmentGetInternalStsActivityLog-1
    [Documentation]  Apply Internal sts on appointment and getting activity log
...
#      ${date_time}=  get_date_time_format
    ...
    ${resp}=  Get Appointment Internal Sts ActivityLog   ${apptid1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AppointmentGetInternalStsActivityLog-2
    ...
    ${internal_sts}=  MultiUser_InternalStatus  ${combined_json}  ${pid1}
    ...
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
#     Verify Response List  ${resp}  1  User=${firstname1}  InternalStatus=${internal_sts_dis_name1}
...
#     ${date_time}=  get_date_time_format
    ...
    ${resp}=  Get Appointment Internal Sts ActivityLog   ${apptid3}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response List  ${resp}  0  User=${fullname}  InternalStatus=${internal_sts_dis_name2}

JD-TC-AppointmentGetInternalStsActivityLog -UH1
     [Documentation]   Provider get appointment internal sts activity log without login      
...
JD-TC-AppointmentGetInternalStsActivityLog -UH2
    [Documentation]   Consumer get appointment internal sts activity log
