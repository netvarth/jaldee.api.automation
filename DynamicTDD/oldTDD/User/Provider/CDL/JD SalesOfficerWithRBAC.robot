...
JD-TC-SalesOfficerWithRBAC-1
    ...
    [Documentation]               Create Partner Using Sales officer Role with RBAC
...
JD-TC-SalesOfficerWithRBAC-9
    ...
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200 
    Set Suite Variable  ${partner_id1}  ${resp.json()['id']}
    ...
    ${resp}=   Requst For Partner Aadhar Validation    ${custid}    ${loanuid1}    ${consnum}    ${aadhaar}    ${aadhaarAttachments}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Requst For Partner Pan Validation    ${custid}    ${loanuid1}    ${consnum}    ${pan}    ${panAttachments}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Add Partner loan Bank Details    4    ${loanuid1}    ${loanuid1}    ${bankName}    ${bankAccountNo}    ${bankIfsc}    ${bankAddress1}    ${bankAddress2}    ${bankCity}    ${bankState}   ${bankPin}    ${bankStatementAttachments}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Update Partner loan Bank Details    4    ${loanuid1}    ${loanuid1}    ${bankName}    ${bankAccountNo2}    ${bankIfsc}    ${bankAddress1}    ${bankAddress2}    ${bankCity}    ${bankState}   ${bankPin2}    ${bankStatementAttachments2}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    
    ${resp}=    Get Partner loan Bank Details    ${loanuid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    Set Suite Variable    ${kyid}    ${resp.json()['id']}

    ${resp}=    Verify Partner loan Bank   ${loanuid1}    ${bankName}    ${bankAccountNo2}    ${bankIfsc}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Verify Partner loan Bank Details     ${loanuid1}  ${loanProducts}  ${category}  ${type}   ${loanproductcatid}   ${loanproductSubcatid}  ${partner}  ${invoiceAmount}  ${downpaymentAmount}  ${requestedAmount}  ${emiPaidAmountMonthly}  ${bool[1]}  ${employeeCode}  ${loanid}  ${LoanApplicationKycList}      
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
...
JD-TC-SalesOfficerWithRBAC-10
    ...
    [Documentation]              invoice updation Using Sales officer Role with RBAC
    ...
    ${resp}=    Loan Application Manual Approval        ${loanuid1}    ${loanScheme}   ${invoiceAmount}    ${downpaymentAmount}    ${requestedAmount}    ${sanctionedAmount}    loanProduct=${loanProduct}    note=${note}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=  salesofficer Approval    ${loanuid1}    ${sch1}    ${tenu1}    ${noOfAdvanceEmi}    ${dayofmonth}     partner=${partner}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=    Otp for Consumer Loan Acceptance Phone    ${consumernumber}    ${OtpPurpose['Authentication']}    ${loanuid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=  Partner Accepted    ${loanuid1}    ${so_id1}    ${pdffile}    ${fileSize}   ${caption}  ${fileType}    ${LoanAction[0]}  ${EMPTY}  invoice  ${order}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200    
    

JD_TC_SalesOfficerWithRBAC-UH1
...
JD_TC_SalesOfficerWithRBAC-UH2
...
JD-TC_SalesOfficerWithRBAC-UH3
...
JD-TC_SalesOfficerWithRBAC-UH4

    [Documentation]               View loan by other sales officer who is in the different branch but in same location

    ${resp}=  SendProviderResetMail   ${SOUSERNAME3}
    Should Be Equal As Strings  ${resp.status_code}  200

    @{resp}=  ResetProviderPassword  ${SOUSERNAME3}  ${PASSWORD}  ${OtpPurpose['ProviderResetPassword']}
    Should Be Equal As Strings  ${resp[0].status_code}  200
    Should Be Equal As Strings  ${resp[1].status_code}  200
...
JD-TC_SalesOfficerWithRBAC-UH5

    [Documentation]               View loan by other sales officer who is in the different branch and different location

    ${resp}=  SendProviderResetMail   ${SOUSERNAME4}
    Should Be Equal As Strings  ${resp.status_code}  200

    @{resp}=  ResetProviderPassword  ${SOUSERNAME4}  ${PASSWORD}  ${OtpPurpose['ProviderResetPassword']}
    Should Be Equal As Strings  ${resp[0].status_code}  200
    Should Be Equal As Strings  ${resp[1].status_code}  200
...
JD-TC_SalesOfficerWithRBAC-UH6
...
JD-TC_SalesOfficerWithRBAC-UH7
...
JD-TC_SalesOfficerWithRBAC-UH8
    ...
    ${resp}=  Generate Phone Partner Creation    ${partnern1}    ${countryCodes[0]}    partnerName=${dealername}   partnerUserFirstName=${dealerfname}  partnerUserLastName=${dealerlname}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=  Verify Phone Partner Creation    ${partnern1}   ${OtpPurpose['ProviderVerifyPhone']}    ${dealerfname}   ${dealerlname}   branch=${branch}    partnerUserFirstName=${dealerfname}  partnerUserLastName=${dealerlname}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    Set Suite Variable  ${partid1}  ${resp.json()['id']}
...
JD-TC_SalesOfficerWithRBAC-UH9
...
JD-TC_SalesOfficerWithRBAC-UH10
...
JD-TC_SalesOfficerWithRBAC-UH11
