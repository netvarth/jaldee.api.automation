...
#    lead generation   
    ...
    ${resp}=    Get KYC Details   ${user_token}   ${le_uid1}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    Should Be Equal As Strings      ${resp.json()[0]['customerName']}                         ${customerName}
    Should Be Equal As Strings      ${resp.json()[0]['telephone'][0]['telephoneNumber']}      ${phoneNo}
    Should Be Equal As Strings      ${resp.json()[0]['relationName']}                         ${relationName}
    Should Be Equal As Strings      ${resp.json()[0]['validationIds'][0]['idTypes']}          ${idTypes}
    Should Be Equal As Strings      ${resp.json()[0]['validationIds'][0]['idValue']}          ${idValue}
    Should Be Equal As Strings      ${resp.json()[0]['validationIds'][1]['idTypes']}          UID
    Should Be Equal As Strings      ${resp.json()[0]['validationIds'][1]['idValue']}          ${idValue}
