...
JD-TC-GetAppointmentNote-1

    [Documentation]  Provider gets note for appointment
    ...
    clear_service   ${PUSERNAME255}
    clear_location  ${PUSERNAME255}
    clear_appt_schedule   ${PUSERNAME255}
    clear_customer   ${PUSERNAME255}
    ...
    ${cookie}  ${resp}=   Imageupload.spLogin  ${PUSERNAME255}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-GetAppointmentNote-2

    [Documentation]  Provider gets note without creating appointment note
    ...
    clear_service   ${PUSERNAME254}
    clear_location  ${PUSERNAME254}
    clear_appt_schedule   ${PUSERNAME254}
...
JD-TC-GetAppointmentNote-UH2
    ...
    ${note1}=   Fakerlibrary.sentence
    ${resp}=  Get Appointment Note   abcdef
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  404
    Should Be Equal As Strings  "${resp.json()}"  "${INVALID_APPOINTMENT_UID}"

JD-TC-GetAppointmentNote-UH3

    [Documentation]  Provider gets note for an appointment without login
...
JD-TC-GetAppointmentNote-UH4

    [Documentation]  Provider gets note for an appointment using consumer login
