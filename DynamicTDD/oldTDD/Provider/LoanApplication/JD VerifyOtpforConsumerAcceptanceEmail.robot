...
JD-TC-VerifyOtpforConsumerAcceptanceEmail-1
...
JD-TC-VerifyOtpforConsumerAcceptanceEmail-2
    ...
    Should Be Equal As Strings      ${resp.status_code}  200
    IF   '${resp.content}' == '${emptylist}'
        ${resp1}=  AddCustomer with email   ${fname}  ${lname}  ${EMPTY}  ${email}  ${gender}  ${dob}  ${consumernumber}  ${EMPTY}
        Log  ${resp1.content}
        Should Be Equal As Strings  ${resp1.status_code}  200
        Set Test Variable  ${pcid14}   ${resp1.json()}
    ELSE
...
# <----------------------------- KYC Details ------------------------------------------>
    ...
    ${resp}=   Requst For Aadhar Validation    ${kycid}    ${loanuid1}    ${consumernumber}    ${aadhaar}    ${aadhaarAttachments}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Requst For Pan Validation    ${kycid}    ${loanuid1}    ${consumernumber}    ${pan}    ${panAttachments}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
    ...
    ${resp}=    Update loan Application Kyc Details    ${loanid1}     ${loanuid1}   ${consumernumber}       ${aadhaar}   ${pan}    ${aadhaarAttachments}    panAttachments=${panAttachments}    
    ...  permanentAddress1=${permanentAddress1}    permanentCity=${permanentCity}    permanentPin=${pin}    permanentState=${permanentState}
    ...  currentAddress1=${currentAddress1}    currentCity=${currentCity}    currentPin=${pin}    currentState=${currentState}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
...
# <----------------------------- Bank Details ------------------------------------------>
    ...
    ${resp}=    Verify Otp for Consumer Acceptance Email     ${email}   ${OtpPurpose['ConsumerAcceptance']}   ${loanuid1}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}    ${ENTER_VALID_OTP}

JD-TC-VerifyOtpforConsumerAcceptanceEmail-3
                                  
    [Documentation]               Verify Otp for Consumer Acceptance Email    invalid mail
...
# <----------------------------- Loan Details ------------------------------------------>
    ...
    ${LoanApplicationKycList}=    Create Dictionary   id=${kycid}  employmentStatus=${employmentStatus[0]}  monthlyIncome=${monthlyIncome}  nomineeType=${nomineeType[1]}  nomineeName=${nomineeName}
    Log  ${LoanApplicationKycList}
...
# <----------------------------- Bank Details ------------------------------------------>
    ...
    ${loanScheme}=     Create Dictionary    id=${Schemeid}  
    ${loanProduct}=    Create Dictionary    id=${Productid} 
    ${resp}=    Loan Application Manual Approval        ${loanUid}    ${loanScheme}   ${invoiceAmount}    ${downpaymentAmount}    ${requestedAmount}    80000    loanProduct=${loanProduct}
    Log    ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=    Verify Otp for Consumer Acceptance Email     g6t77g   15   ${loanUid}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings     ${resp.json()}    ${ENTER_VALID_OTP}

JD-TC-VerifyOtpforConsumerAcceptanceEmail-4
                                  
    [Documentation]               Verify Otp for Consumer Acceptance Email    invalid loan uid
    ...
    ${resp}=    Verify Otp for Consumer Acceptance Email     ${email2}   15   78yuh
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
JD-TC-VerifyOtpforConsumerAcceptanceEmail-5
    ...
    ${resp}=    Verify Otp for Consumer Acceptance Email     ${email2}   15   ${loanUid}
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  419
    Should Be Equal As Strings  ${resp.json()}   ${SESSION_EXPIRED}
    
JD-TC-VerifyOtpforConsumerAcceptanceEmail-6
                                  
    [Documentation]               Verify Otp for Consumer Acceptance Email   with consumer login
...
JD-TC-VerifyOtpforConsumerAcceptanceEmail-7
