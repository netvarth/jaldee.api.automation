...
JD-TC-GetAppointmentStatusBoard-1
    ...
    ${serv}=  Create Dictionary   id=${s_id2}
    ${ser}=  Create List   ${serv} 
    ${dep}=  Create List   
    ${appt_sh}=   Create Dictionary  id=${sch_id}
    ${appt_shd}=    Create List   ${appt_sh}
    ${app_status}=    Create List   ${apptStatus[2]}
    ...
    ${resp}=   Create Status Board Appointment    ${Data1[0]}  ${Data1[1]}  ${Data1[2]}  ${matric_list1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()[0]['id']}  ${sb_id}
    ...
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['sbId']}  ${sba_id1}  
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['position']}  ${Positions[0]} 

    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['queueSet']['name']}   ${s_name[0]} 
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['queueSet']['displayName']}   ${s_name[1]}  
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['queueSet']['queueSetFor'][0]['type']}   ${statusboard_type[0]}
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['queueSet']['qBoardConditions']['services'][0]['id']}    ${s_id1}
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['queueSet']['qBoardConditions']['apptSchedule'][0]['id']}    ${sch_id}
    Should Be Equal As Strings  ${resp.json()[0]['metric'][0]['queueSet']['queryString']}    service-eq=${s_id1}&schedule-eq=${sch_id}&apptStatus-eq=Arrived&label-eq=::


    Should Be Equal As Strings  ${resp.json()[1]['id']}  ${sb_id1}
    ...
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['sbId']}  ${sba_id2}  
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['position']}  ${Positions1[0]}  

    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['queueSet']['name']}   ${s_name1[0]} 
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['queueSet']['displayName']}   ${s_name1[1]}  
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['queueSet']['queueSetFor'][0]['type']}   ${statusboard_type[0]}
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['queueSet']['qBoardConditions']['services'][0]['id']}    ${s_id2}
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['queueSet']['qBoardConditions']['apptSchedule'][0]['id']}    ${sch_id}
    Should Be Equal As Strings  ${resp.json()[1]['metric'][0]['queueSet']['queryString']}    service-eq=${s_id2}&schedule-eq=${sch_id}&apptStatus-eq=Arrived&label-eq=::



JD-TC-GetAppointmentStatusBoard -UH1
    [Documentation]   Provider Get Labels without login  
    ${resp}=  Get AppointmentStatusBoards
    Should Be Equal As Strings    ${resp.status_code}   419
    Should Be Equal As Strings   "${resp.json()}"   "${SESSION_EXPIRED}"
 
JD-TC-GetAppointmentStatusBoard -UH2
