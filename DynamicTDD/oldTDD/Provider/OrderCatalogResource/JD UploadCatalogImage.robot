...
JD-TC-Upload_Catalog_Image-1
    [Documentation]   Provider check to upload Catalog image when displayImage is true
...
JD-TC-Upload_Catalog_Image-2
    [Documentation]   Provider check to again upload Catalog image when displayImage is false
    ${cookie}  ${resp}=   Imageupload.spLogin  ${PUSERNAME42}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${resp}=   uploadCatalogImages   ${CatalogId1}   ${boolean[0]}   ${cookie}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Not Be Equal As Strings  ${resp.json()['catalogImages']}   ${EMPTY}
    Should Be Equal As Strings    ${resp.json()['catalogImages'][0]['displayImage']}   ${bool[0]} 
    Should Be Equal As Strings    ${resp.json()['catalogImages'][1]['displayImage']}   ${bool[1]}



JD-TC-Upload_Catalog_Image-UH1
    [Documentation]  Provider check to upload catalog image with another provider catalog_id
...
JD-TC-Upload_Catalog_Image-UH2
    [Documentation]  Provider check to upload catalog image with invalid catalog_id
    ${cookie}  ${resp}=   Imageupload.spLogin  ${PUSERNAME42}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=   uploadCatalogImages   ${Invalid_id}   ${boolean[1]}   ${cookie}
    Log  ${resp.json()}
...
JD-TC-Upload_Catalog_Image-UH3
    [Documentation]  Consumer check to upload catalog image  
...
JD-TC-Upload_Catalog_Image-UH4
    [Documentation]  upload catalog image without login 
    ${empty_cookie}=  Create Dictionary
    ${resp}=  uploadCatalogImages    ${CatalogId1}   ${boolean[1]}  ${empty_cookie} 
    Log  ${resp.json()}
