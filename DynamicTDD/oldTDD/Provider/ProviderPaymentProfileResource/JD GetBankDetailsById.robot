...
JD-TC-GetBankInfoById-1
    ...
    ${resp}=   Create Bank Info   ${pid}  ${bank_name}  ${bank_ac}   ${ifsc_code}  ${name}  ${name}  ${branch}  ${businessFilingStatus[1]}  ${accountType[1]}  ${pan_num}  ${PUSERNAME130}         
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    Should Be Equal As Strings  ${resp.json()['payTmLinkedPhoneNumber']}    ${PUSERNAME130}
...
JD-TC-GetBankInfoById-2
    ...
    ${resp}=   Create Bank Info   ${pid}  ${bank_name1}  ${bank_ac1}  ${ifsc_code1}  ${name1}  ${name1}  ${branch1}  ${businessFilingStatus[1]}  ${accountType[1]}  ${pan_num1}  ${PUSERNAME130}   
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
