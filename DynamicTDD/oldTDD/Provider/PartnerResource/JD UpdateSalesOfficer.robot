...
JD-TC-Update Sales Officer-1
    ...
    Set Suite Variable    ${Salesofficer}
...
JD-TC-Update Sales Officer-UH1
    ...
    ${resp}=    Update Sales Officer    ${inv_puid}    ${Salesofficer}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}    ${INVALID_PARTNER_UID}


JD-TC-Update Sales Officer-UH2
    ...
    ${inv_sid}    FakerLibrary.Random Number

    ${inv_sid}=    Convert to String    ${inv_sid}

    ${Salesofficer2}=    Create Dictionary    id=${inv_sid}  isDefault=${bool[1]}

    ${INVALID_SALESOFFICER}=   Replace String  ${INVALID_SALESOFFICER}  {}  ${inv_sid}

    ${resp}=    Update Sales Officer    ${Puid1}    ${Salesofficer2}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings    ${resp.json()}    ${INVALID_SALESOFFICER}



JD-TC-Update Sales Officer-UH3
    ...
    ${Salesofficer3}=    Create Dictionary    id=${so_id1}  isDefault=${bool[0]}

    ${resp}=    Update Sales Officer    ${Puid1}    ${Salesofficer3}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200
