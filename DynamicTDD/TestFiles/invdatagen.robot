...
JD-TC-Appointment-1
    ...
    # END
        IF    '${service['serviceCategory']}' == 'MainService' and ${service['totalAmount']} > 0 and 'provider' not in ${service} and '${service['status']}' == 'ACTIVE' and '${service['maxBookingsAllowed']}' > '200'
            Log    ${service['name']}
            ...
            END
        # Appointment service
        ELSE IF    '${service['serviceCategory']}' == 'MainService' and ${service['totalAmount']} == 0 and 'provider' not in ${service} and '${service['status']}' == 'ACTIVE' and '${service['maxBookingsAllowed']}' > '200'
            Log    ${service['name']}
            ...
            Set Test Variable    ${s_id2}    ${service['id']}
        # SubService
        ELSE IF    '${service['serviceCategory']}' == 'SubService' and '${service['status']}' == 'ACTIVE' and '${service['maxBookingsAllowed']}' > '200'
            Log    ${service['name']}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    IF   ${resp.json()['parallelServing']} < ${parallel}
    ...
    Log  ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
    ${disc_length}=  Get Length  ${resp.json()}
    Set Test Variable   ${discountId}   ${resp.json()[${disc_length-1}]['id']}
    ${discountprice}=  Set Variable   ${resp.json()[${disc_length-1}]['discValue']}


    ${slot}=  Get Random Slot  ${sch_id}  ${DAY1}  ${s_id2}

# *** COMMENTS ***
    comment  ---------------------------- FOR LOOP Starts Here ---------------------------------------
    ...
    FOR   ${i}  IN RANGE   ${loop_count}
        ...
        #-------------------------5th Appointment-Take an appt then apply service level/invoice level discount-------------------------------
        ...
        IF    ${i} % 2 == 0
            ...
            Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.status_code}  200
        ELSE
            ${resp}=   Apply Discount   ${invoice_uid5}   ${discountId}    ${discountprice}   ${privateNote}  ${displayNote}
            Log  ${resp.content}
            Should Be Equal As Strings  ${resp.status_code}  200
    ...
    #-------------------------6th Appointment-Take appointments for future- Nested FOR LOOP -------------------------------
        ${random_days1}=   Evaluate  random.sample(range(int(${max_days})), int(${max_days}/2))  random
        ${random_days2}=   Evaluate  random.sample(range(int(${max_days})), int(${max_days}/2))  random
        FOR  ${j}  IN RANGE   ${max_days}
            ...
            IF  ${j} in @{random_days2}
                ${privateNote}=     FakerLibrary.word
                ${displayNote}=   FakerLibrary.word
                ${resp}=  Apply Discount   ${invoice_uid7}   ${discountId}    ${discountprice}   ${privateNote}  ${displayNote}
                Log  ${resp.content}
                Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.status_code}  200
            END
        ...
        #-------------------------7th Appointment-Take appointments for past dates- Nested FOR LOOP -------------------------------
        ${random_days1}=   Evaluate  random.sample(range(int(${max_days})), int(${max_days}/2))  random
        ${random_days2}=   Evaluate  random.sample(range(int(${max_days})), int(${max_days}/2))  random
        FOR  ${j}  IN RANGE   ${max_days}
            ...
            IF  ${j} in @{random_days2}
                ${privateNote}=     FakerLibrary.word
                ${displayNote}=   FakerLibrary.word
                ${resp}=  Apply Discount   ${invoice_uid6}   ${discountId}    ${discountprice}   ${privateNote}  ${displayNote}
                Log  ${resp.content}
                Run Keyword And Continue On Failure  Should Be Equal As Strings  ${resp.status_code}  200
            END
