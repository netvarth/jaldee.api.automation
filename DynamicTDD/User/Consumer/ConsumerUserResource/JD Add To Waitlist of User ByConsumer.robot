...
JD-TC-Add To Waitlist of User ByConsumer-1
        ...
        ${emails1}=  Emails  ${name3}  Email  ${P_Email}${MUSERNAME29}.ynwtest@netvarth.com  ${views}
        ${bs}=  FakerLibrary.bs
        ${city}=   get_place
        ${latti}=  get_latitude
        ${longi}=  get_longitude
       ...
       ${tlgnum}=  Evaluate  ${MUSERNAME29}+336345
        ...
        # Log   ${resp.content}
        # Should Be Equal As Strings  ${resp.status_code}  200
        ${resp}=  Create User  ${firstname}  ${lastname}  ${dob}  ${Genderlist[0]}  ${P_Email}${PUSERNAME_U1}.ynwtest@netvarth.com   ${userType[0]}  ${pin}  ${countryCodes[0]}  ${PUSERNAME_U1}  ${dep_id}  ${sub_domain_id}  ${bool[0]}  ${countryCodes[1]}  ${whpnum}  ${countryCodes[1]}  ${tlgnum} 
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Add To Waitlist of User ByConsumer-4
	[Documentation]  A Consumer Added To Waitlist for same service in diffrent queues of Different USERS
        ...
        # ${resp}=  Create User  ${firstname2}  ${lastname2}  ${address}  ${PUSERNAME_U2}  ${dob}    ${Genderlist[0]}  ${userType[0]}  ${P_Email}${PUSERNAME_U2}.ynwtest@netvarth.com  ${location}  ${state}  ${dep_id}  ${sub_domain_id}
        # Log   ${resp.content}
        # Should Be Equal As Strings  ${resp.status_code}  200
        ${resp}=  Create User  ${firstname2}  ${lastname2}  ${dob}  ${Genderlist[0]}  ${P_Email}${PUSERNAME_U2}.ynwtest@netvarth.com   ${userType[0]}  ${pin}  ${countryCodes[0]}  ${PUSERNAME_U2}  ${dep_id}  ${sub_domain_id}  ${bool[0]}  ${countryCodes[1]}  ${PUSERNAME_U2}  ${countryCodes[1]}  ${PUSERNAME_U2} 
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Add To Waitlist of User ByConsumer-14
        ...
        ${resp}=  Delete Waitlist Consumer  ${cwid1}  ${p_id}
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Add To Waitlist of User ByConsumer-UH13   
    [Documentation]   Add to waitlist After Business time
        ...
        ${psTime}=  subtract_time  0  45
        ${peTime}=  subtract_time   0  10
        ${list}=  Create List  1  2  3  4  5  6  7
        ${CUR_DAY}=  get_date
...
JD-TC-Add To Waitlist of User ByConsumer-UH14
    [Documentation]  Add consumer to waitlist when service time exceeds queue time.
        ...
        ${psTime}=  add_time  0  15
        ${peTime}=  add_time  0  20
        ${list}=  Create List  1  2  3  4  5  6  7
        ${CUR_DAY}=  get_date
...
JD-TC-Add To Waitlist of User ByConsumer-UH15
	[Documentation]  Add consumer to waitlist for a service with prepayment and try to change prepayment status from prepaymentPending to arrived.   
        ...
        # Log   ${resp.content}
        # Should Be Equal As Strings  ${resp.status_code}  200
        ${resp}=  Create User  ${firstname}  ${lastname}  ${dob}  ${Genderlist[0]}  ${P_Email}${PUSERNAME_U1}.ynwtest@netvarth.com   ${userType[0]}  ${pin}  ${countryCodes[0]}  ${PUSERNAME_U1}  ${dep_id}  ${sub_domain_id}  ${bool[0]}  ${countryCodes[1]}  ${PUSERNAME_U1}  ${countryCodes[1]}  ${PUSERNAME_U1} 
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        Set Suite Variable  ${u_id}  ${resp.json()}
        ...
        ${resp}=  ProviderLogin  ${MUSERNAME10}  ${PASSWORD}
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Add To Waitlist of User ByConsumer-UH16
	[Documentation]  the consumer add to waitlist for a service with prepayment  , try to change prepaymentPending to STARTED 
        ...
        ${resp}=  ProviderLogin  ${MUSERNAME${a}}  ${PASSWORD}
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-Add To Waitlist of User ByConsumer-19
    ...
    [Documentation]  Add to waitlist a consumer and completes prepayment
        ...
        # Log   ${resp.content}
        # Should Be Equal As Strings  ${resp.status_code}  200
        ${resp}=  Create User  ${firstname}  ${lastname}  ${dob}  ${Genderlist[0]}  ${P_Email}${PUSERNAME_U1}.ynwtest@netvarth.com   ${userType[0]}  ${pin}  ${countryCodes[0]}  ${PUSERNAME_U1}  ${dep_id}  ${sub_domain_id}  ${bool[0]}  ${countryCodes[1]}  ${PUSERNAME_U2}  ${countryCodes[1]}  ${PUSERNAME_U2} 
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        Set Suite Variable  ${u_id}  ${resp.json()}
        ${resp}=  Get User
...
JD-TC-Add To Waitlist of User ByConsumer-20
        ...
        # Log   ${resp.content}
        # Should Be Equal As Strings  ${resp.status_code}  200
        ${resp}=  Create User  ${firstname}  ${lastname}  ${dob}  ${Genderlist[0]}  ${P_Email}${PUSERNAME_U32}.ynwtest@netvarth.com   ${userType[0]}  ${pin}  ${countryCodes[0]}  ${PUSERNAME_U32}  ${dep_id}  ${sub_domain_id}  ${bool[0]}  ${countryCodes[1]}  ${PUSERNAME_U32}  ${countryCodes[1]}  ${PUSERNAME_U32} 
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
        ...
        ${VirtualcallingMode2}=   Create Dictionary   callingMode=${CallingModes[1]}   value=${MUSERNAME5}   status=ACTIVE    instructions=${instructions2} 
...
JD-TC-Add To Waitlist of User ByConsumer-UH17
    [Documentation]  add to waitlist, then cancel the waitlist and try to change cancelled waitlit to CHECKIN
        ...
        ${resp}=  Delete Waitlist Consumer  ${cwid1}  ${p_id30}
        Log   ${resp.content}
        Should Be Equal As Strings  ${resp.status_code}  200
