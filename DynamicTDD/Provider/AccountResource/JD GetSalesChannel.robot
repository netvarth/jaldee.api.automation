...
JD-TC-Get SalesChannel1
    ...
    clear_ScRepTable  ABCDE
    clear_ScTable     ${pri_ph}

   

    # ${resp}=  Create Sales Channel    ABCDE    ${pro_disc_jd}   ${pro_disc_dur}   ${scname}   ${contact_fname}    ${contact_lname}    ${address}    ${city}    ${metro}    ${state}   ${latti}   ${longi}   ${radius}   ${postcode}   ${Sctype[2]}  ${pri_ph}   ${alt_ph1}   ${alt_ph2}    ${comm_dur}   ${comm_pct}    ${pri_email}   ${alt_email1}   ${alt_email2}   ${bonusPeriod[0]}    ${id}   ${targetCount}   ${rate}   ${private_note}
    # Log   ${resp.json()}

    ${resp}=  Create SA SalesChannel    ABCDE    ${pro_disc_jd}   ${pro_disc_dur}   ${scname}   ${contact_fname}    ${contact_lname}    ${address}    ${city}    ${metro}    ${state}   ${latti}   ${longi}   ${radius}   ${postcode}   ${Sctype[2]}  ${pri_ph}   ${alt_ph1}   ${alt_ph2}    ${comm_dur}   ${comm_pct}    ${pri_email}   ${alt_email1}   ${alt_email2}   ${bonusPeriod[0]}    ${id}   ${targetCount}   ${rate}   ${private_note}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Set Suite Variable   ${sales_channel_id}    ${resp.json()} 


    ${resp}=  Create Sales Channel Rep   ABCDE   ${rep_fname}  ${rep_lname}     ${alt_ph1}    ${alt_email1}   ${kyc[0]}   ${kycDoneBy}    ${area_rep}    ${repcode}
    Log   ${resp}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Set Suite Variable   ${sales_channel_repid}    ${resp.json()} 
    Set Suite Variable  ${sc_code}     ABCDE-${scDiscCodes}-${repcode}-0
    Log  ${sc_code}
...
JD-TC-Get SalesChannel-UH1

    [Documentation]  GET Sales channel without Login
...
JD-TC-Get SalesChannel-UH2

    [Documentation]  consumer try to get sales channel
