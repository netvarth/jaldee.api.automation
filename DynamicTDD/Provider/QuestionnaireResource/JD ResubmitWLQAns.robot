...
JD-TC-ResubmitQuestionnaireForWaitlist-1
    ...
    ${cookie}  ${resp}=  Imageupload.spLogin  ${PUSERNAME26}   ${PASSWORD}
    Log  ${resp.content}
    Should Be Equal As Strings   ${resp.status_code}    200
...
JD-TC-ResubmitQuestionnaireForWaitlist-3
    ...
    # ${fudata}=  db.fileUploadDT   ${qnr_resp.json()}  ${FileAction[0]}  ${mp4file}  
    # Log  ${fudata}
    ...
    # ${resp}=  Imageupload.CWlResubmitQns   ${cookie}  ${account_id}   ${wid1}   ${data}  ${pdffile}
    # Log  ${resp.content}
    # Should Be Equal As Strings  ${resp.status_code}  200
