...
JD-TC-GetServiceOptionForOrder-1
    ...
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME18}.ynwtest@netvarth.com
    ${landMark}=  FakerLibrary.street name 
    ${address}=  Create Dictionary   phoneNumber=${CUSERNAME18}    firstName=${fname}   lastName=${lname}   email=${C_email}    address=${district}   city=${city}   postalCode=${pin}    landMark=${landMark}   countryCode=${countryCodes[0]}
    ...
    ${resp}=   Create Order By Provider For HomeDelivery    ${cookie}  ${proconid}   ${proconid}   ${CatalogId1}   ${boolean[1]}   ${address}  ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME18}    ${C_email}  ${orderNote}  ${countryCodes[1]}  ${item_id1}   ${item_quantity1}  
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=    Get Service Options By Catalogueid and Channel  ${CatalogId1}  ${QnrChannel[0]}
    Log     ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}     200
