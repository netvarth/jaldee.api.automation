...
JD-TC-ServiceOptionPaymentForOrder-2
    ...
    ${pin}  ${city}  ${district}  ${state}=  get_pin_loc
    ${first}= 	Split String 	${fname}
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME38}.ynwtest@netvarth.com
    ${landMark}=  FakerLibrary.street name 
...
JD-TC-ServiceOptionPaymentForOrder-4
    ...
    #     ${u_ttype}=    Remove Duplicates    ${ttype}
    #     Log  ${u_ttype}
    #     ${CatalogId11}=  Run Keyword If   '${kwstatus}' == 'FAIL' and '${QnrTransactionType[8]}' in @{u_ttype}  Create Sample Catalog  ${unique_cnames[${i}]}  ${item_id1}  
    # END
    FOR  ${i}  IN RANGE   ${c_len}
...
JD-TC-ServiceOptionPaymentForOrder-5
    ...
    ${pin}  ${city}  ${district}  ${state}=  get_pin_loc
    ${first}= 	Split String 	${fname}
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME29}.ynwtest@netvarth.com
    ${landMark}=  FakerLibrary.street name 
