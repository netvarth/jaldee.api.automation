...
*** Test Cases ***
JD-TC-ReleaseOrderQnrForConsumer-1
    ...
    Set Test Variable  ${C_email}  ${first[0]}${CUSERNAME10}.ynwtest@netvarth.com
    ${landMark}=  FakerLibrary.street name 
    ${address}=  Create Dictionary   phoneNumber=${CUSERNAME10}    firstName=${fname}   lastName=${lname}   email=${C_email}    address=${district}   city=${city}   postalCode=${pin}    landMark=${landMark}   countryCode=${countryCodes[0]}
    ...
    ${resp}=   Create Order For HomeDelivery   ${cookie}   ${account_id}    ${self}    ${catalogid}   ${bool[1]}    ${address}    ${sTime}    ${eTime}   ${DAY1}    ${CUSERNAME10}    ${C_email}  ${countryCodes[1]}  ${EMPTY_List}   ${item_id1}    ${item_quantity1} 
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-ReleaseOrderQnrForConsumer-UH2
    ...
    ${resp}=    Release Order Qnr For Consumer    ${empty}   ${orderid1}   ${QnrReleaseStatus[2]}
    Log   ${resp.content}
...
JD-TC-ReleaseOrderQnrForConsumer-UH3
    ...
    ${resp}=    Release Order Qnr For Consumer    ${questId}   ${empty}   ${QnrReleaseStatus[2]}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
...
JD-TC-ReleaseOrderQnrForConsumer-UH4
    ...
    ${resp}=    Release Order Qnr For Consumer    ${questId}   ${orderid1}   ${empty}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
...
JD-TC-ReleaseOrderQnrForConsumer-UH5
    ...
    ${resp}=    Release Order Qnr For Consumer    ${inv_questId}   ${orderid1}   ${QnrReleaseStatus[1]}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-ReleaseOrderQnrForConsumer-UH6
    ...
    ${inv_order}=  Generate Random String  3  [NUMBERS]

    ${resp}=    Release Order Qnr For Consumer    ${questId}   ${inv_order}   ${QnrReleaseStatus[1]}
    Log   ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}  404
...
JD-TC-ReleaseOrderQnrForConsumer-4
    ...
    # ${CUSERNAME10}    Generate random string    10    123456789
    # ${CUSERNAME10}    Convert To Integer  ${CUSERNAME10}

    # ${resp}=  AddCustomer  ${CUSERNAME10}
        
    # Log   ${resp.json()}
    # Should Be Equal As Strings  ${resp.status_code}  200
    # Set Suite Variable    ${CUSERNAME10}

    ${resp}=    Send Otp For Login    ${CUSERNAME10}    ${accountId}
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200

    ${resp}=    Verify Otp For Login   ${CUSERNAME10}   12
    Log   ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}   200
    ...
    ${resp}=    ProviderConsumer Login with token    ${CUSERNAME10}    ${accountId}  ${token} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
    Set Suite VAriable   ${pcid}    ${resp.json()['providerConsumer']}


    ${resp}=   Get Order By Id   ${pcid}   ${orderid1}  
    Log  ${resp.content}
    Should Be Equal As Strings    ${resp.status_code}    200
