...
JD-TC-Create Bill -2
        ...
        ${resp}=  Get Bill By UUId  ${wid}
        Should Be Equal As Strings  ${resp.status_code}  200
        Verify Response  ${resp}  uuid=${wid}  netTotal=${netTotal}  billStatus=${billStatus[0]}  billViewStatus=${billViewStatus[1]}  netRate=${netrate}  billPaymentStatus=${paymentStatus[0]}  totalAmountPaid=0.0  amountDue=${netrate}  taxableTotal=${ser_amount2}  totalTaxAmount=${taxamnt}
        Should Be Equal As Strings  ${resp.json()['service'][0]['serviceId']}  ${sid1}
        Should Be Equal As Strings  ${resp.json()['service'][0]['serviceName']}  ${SERVICE1} 
        Should Be Equal As Strings  ${resp.json()['service'][0]['quantity']}  1.0
        Should Be Equal As Strings  ${resp.json()['service'][0]['GSTpercentage']}    0.0
...
JD-TC- Create Bill -3
        ...
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200
        Verify Response  ${resp}  uuid=${wid}  netTotal=${ser_amount2}  billStatus=${billStatus[0]}  billViewStatus=${billViewStatus[1]}  netRate=${netrate}  billPaymentStatus=${paymentStatus[0]}  totalAmountPaid=0.0  amountDue=${netrate}  taxableTotal=${ser_amount2}  totalTaxAmount=${taxamnt}
...
JD-TC- Create Bill -21
        ...
        ${items}=   Create List   ${itemId}
        ${resp}=  Create Provider Coupon   ${coupon}  ${desc}  ${cou_amount}  ${calctype[1]}  ${cupn_code1}  ${recurringtype[1]}  ${list}  ${sTime}  ${eTime}  ${ST_DAY}  ${EN_DAY}  ${EMPTY}  ${bool[0]}  ${min_bill_amount}  ${max_disc_val1}  ${bool[1]}  ${max_prov_use}  ${book_channel}  ${coupn_based}  ${tc}  services=${services}  items=${items} 
        Log  ${resp.json()}
        Should Be Equal As Strings  ${resp.status_code}  200
        Set Suite Variable  ${couponId1}  ${resp.json()}
