...
JD-TC-VerifyOtpForEmail-1
                                  
    [Documentation]               varify otp for mail
...
JD-TC-VerifyOtpForEmail-2
                                  
    [Documentation]               varify otp for mail  with same mail different loan uuid
    ...
    # Should Be Equal As Strings      ${resp.status_code}  200
    # IF   '${resp.content}' == '${emptylist}'
    #     ${resp1}=  AddCustomer with email   ${fname}  ${lname}  ${EMPTY}  ${emai22}  ${gender}  ${dob}  ${${phone12}}  ${EMPTY}
    #     Log  ${resp1.content}
    #     Should Be Equal As Strings  ${resp1.status_code}  200
    #     Set Test Variable  ${pcid14}   ${resp1.json()}
    # ELSE
    ...
    ${resp}=   Verify Email Otp and Create Loan Application    ${email2}   ${OtpPurpose['ConsumerVerifyEmail']}    ${loanuid12}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    200


JD-TC-VerifyOtpForEmail-UH
                                  
    [Documentation]               varify otp for mail   same loan id and different mail
    ...
    ${resp}=   Verify Email Otp and Create Loan Application    ${emai22}   ${OtpPurpose['ConsumerVerifyEmail']}    ${loanuid12}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings   ${resp.json()}    ${LOGIN_ENTER_CORRECT_OTP}
  

JD-TC-VerifyOtpForEmail-UH1
                                  
    [Documentation]               varify otp for mail  without  generate opt for mail
...
JD-TC-VerifyOtpForEmail-UH2
    ...
    ${resp}=   Verify Email Otp and Create Loan Application    ${email2}   ${OtpPurpose['ConsumerVerifyEmail']}    dgcfhg78
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings   ${resp.json()}    ${LOGIN_ENTER_CORRECT_OTP}
  
JD-TC-VerifyOtpForEmail-UH3
    ...
    ${resp}=   Verify Email Otp and Create Loan Application    ${email2}    4    ${loanuid}
    Log  ${resp.content}
    Should Be Equal As Strings     ${resp.status_code}    422
    Should Be Equal As Strings   ${resp.json()}    ${LOGIN_ENTER_CORRECT_OTP}
  
JD-TC-VerifyOtpForEmail-UH4
...
JD-TC-VerifyOtpForEmail-UH5
                                  
    [Documentation]               varify otp for mail   with consumer login
