...
*** Test Cases ***
JD-TC-AddOrderLabel-1
    [Documentation]  Add label for an order for homedelivery.

    clear_queue    ${PUSERNAME80}
    clear_service  ${PUSERNAME80}
    clear_customer   ${PUSERNAME80}
    clear_Item   ${PUSERNAME80}
    ${resp}=  ProviderLogin  ${PUSERNAME80}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${pid}  ${resp.json()['id']}
    
    ${accId}=  get_acc_id  ${PUSERNAME80}

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME80}.ynwtest@netvarth.com
...
JD-TC-AddOrderLabel-2
    [Documentation]  Add label for an order for store pickup.
    
    clear_queue    ${PUSERNAME81}
    clear_service  ${PUSERNAME81}
    clear_customer   ${PUSERNAME81}
    ...
    ${accId1}=  get_acc_id  ${PUSERNAME81}

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME81}.ynwtest@netvarth.com
    ...
    ${resp}=   Create Order For Pickup   ${cookie}   ${accId1}    ${self}    ${CatalogId1}   ${bool[1]}   ${sTime1}    ${eTime1}   ${DAY1}    ${CUSERNAME20}    ${email}  ${countryCodes[1]}  ${EMPTY_List}  ${item_id1}    ${item_quantity1} 
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=  Add Label for Order   ${orderid2}  ${lblname_1}  ${lbl_value_1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
...
JD-TC-AddOrderLabel-3
    [Documentation]  Add label for an order with integer label value.

    clear_queue    ${PUSERNAME83}
    clear_service  ${PUSERNAME83}
    clear_customer   ${PUSERNAME83}
    clear_Item   ${PUSERNAME83}
    ${resp}=  ProviderLogin  ${PUSERNAME83}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${pid}  ${resp.json()['id']}
    
    ${accId}=  get_acc_id  ${PUSERNAME83}

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME83}.ynwtest@netvarth.com
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}   orderStatus=${orderStatuses[0]}   label=${label}
...
JD-TC-AddOrderLabel-4
    [Documentation]  Remove Order label and add the same label again.

    clear_queue    ${PUSERNAME85}
    clear_service  ${PUSERNAME85}
    clear_customer   ${PUSERNAME85}
    clear_Item   ${PUSERNAME85}
    ${resp}=  ProviderLogin  ${PUSERNAME85}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${pid}  ${resp.json()['id']}
    
    ${accId}=  get_acc_id  ${PUSERNAME85}

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME85}.ynwtest@netvarth.com
...
JD-TC-AddOrderLabel-5
    [Documentation]  Add same label for multile orders.

    clear_queue    ${PUSERNAME86}
    clear_service  ${PUSERNAME86}
    clear_customer   ${PUSERNAME86}
    clear_Item   ${PUSERNAME86}
    ${resp}=  ProviderLogin  ${PUSERNAME86}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${pid}  ${resp.json()['id']}
    
    ${accId}=  get_acc_id  ${PUSERNAME86}

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME86}.ynwtest@netvarth.com
...
JD-TC-AddOrderLabel-6
    [Documentation]  Add multiple labels to an order.
    
    clear_queue    ${PUSERNAME87}
    clear_service  ${PUSERNAME87}
    clear_customer   ${PUSERNAME87}
    clear_Item   ${PUSERNAME87}
    ${resp}=  ProviderLogin  ${PUSERNAME87}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Set Test Variable  ${pid}  ${resp.json()['id']}
    
    ${accId}=  get_acc_id  ${PUSERNAME87}

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME87}.ynwtest@netvarth.com
    ...
    ${resp}=  Add Label for Order   ${orderid1}  ${labelname1[0]}  ${label_value1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    ${resp}=  Add Label for Order   ${orderid1}  ${labelname2[0]}  ${label_value2}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response  ${resp}   orderStatus=${orderStatuses[0]}  
...
JD-TC-AddOrderLabel-UH1

    [Documentation]  Add label for order without login
...
JD-TC-AddOrderLabel-UH2

    [Documentation]  Add label for oder by consumer login
...
JD-TC-AddOrderLabel-UH3
    ...
    ${resp}=  Add Label for Order   ${orderid11}  ${labelname}  ${label_value}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"    "${LABEL_NOT_EXIST}"

JD-TC-AddOrderLabel-UH4
    ...
    ${labelname}=  FakerLibrary.Word
    ${resp}=  Add Label for Order   ${orderid11}  ${labelname}  ${lbl_value_1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${LABEL_NOT_EXIST}"

JD-TC-AddOrderLabel-UH5
    ...
    ${resp}=  Add Label for Order   ${orderid11}  ${lblname_1}  ${labelvalue}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${VALUE_NOT_VALID}"

JD-TC-AddOrderLabel-UH6

    [Documentation]  add label with non existant label value and label name.
    ...
    ${labelvalue}=  FakerLibrary.Word
    ${labelname}=  FakerLibrary.Word
    ${resp}=  Add Label for Order   ${orderid11}  ${labelname}  ${labelvalue}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${LABEL_NOT_EXIST}"

JD-TC-AddOrderLabel-UH7

    [Documentation]  add label with another provider's label name.  

    ${resp}=  ProviderLogin  ${PUSERNAME90}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    clear_Label  ${PUSERNAME90}
...
JD-TC-AddOrderLabel-UH8

    [Documentation]  add label with anoher provider's order id.  
    
    ${resp}=  ProviderLogin  ${PUSERNAME92}  ${PASSWORD}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200

    clear_Label  ${PUSERNAME92}
...
JD-TC-AddOrderLabel-UH9
    ...
    ${labelname}=  FakerLibrary.Word
    ${resp}=  Add Label for Order   ${orderid11}  ${EMPTY}  ${lbl_value_1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${LABEL_NOT_EXIST}"

JD-TC-AddOrderLabel-UH10
    ...
    ${labelvalue}=  FakerLibrary.Word
    ${resp}=  Add Label for Order   ${orderid11}  ${lblname_1}  ${EMPTY}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"   "${VALUE_NOT_VALID}"

JD-TC-AddOrderLabel-UH11

    [Documentation]  add label with invalid order id.

    ${resp}=  Provider Login  ${PUSERNAME80}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200

    ${resp}=  Add Label for Order   000000abcffaf  ${lblname_1}  ${lbl_value_1}
    Log   ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
