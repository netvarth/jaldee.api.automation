...
JD-TC-GetFutureOrderByCriteria-1
    [Documentation]   Get Future order By ordernumber
    ...
    ${resp}=  Get Future Order By Criterias   orderNumber-eq=${ordernumber1} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-GetFutureOrderByCriteria-2
    [Documentation]    Get Future order By homedelivery
    ...
    ${resp}=  Get Future Order By Criterias   homeDelivery-eq=${bool[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200
    Verify Response List    ${resp}  0    uid=${orderid1}   
    Verify Response List    ${resp}  1    uid=${orderid2}
    Verify Response List    ${resp}  2    uid=${orderid3}   
    Verify Response List    ${resp}  3    uid=${orderid4}   
    Verify Response List    ${resp}  4    uid=${orderid}   
 
JD-TC-GetFutureOrderByCriteria-3

    [Documentation]   Get Future order By firstname
    ...
    ${accId1}=  get_acc_id  ${PUSERNAME118}
    Set Suite Variable  ${accId1} 

    ${firstname}=  FakerLibrary.first_name
    ${lastname}=  FakerLibrary.last_name
    Set Test Variable  ${email_id}  ${firstname}${PUSERNAME118}.ynwtest@netvarth.com
    ...
    ${item_quantity1}=  FakerLibrary.Random Int  min=${minQuantity}   max=${maxQuantity}
    ${firstname}=  FakerLibrary.first_name
    Set Test Variable  ${email}  ${firstname}${CUSERNAME2}.ynwtest@netvarth.com
    ...
    ${resp}=  Get Future Order By Criterias   firstName-eq=${fname2}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid0}  
    
JD-TC-GetFutureOrderByCriteria-4
    [Documentation]    Get an Future order details for store pickup.
    ...
    ${resp}=  Get Future Order By Criterias   storePickup-eq=${bool[1]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
...
JD-TC-GetFutureOrderByCriteria-5
    ...
    ${resp}=  Get Future Order By Criterias   lastName-eq=${lname1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-GetFutureOrderByCriteria-6
    ...
    ${resp}=  Get Future Order By Criterias   uid-eq=${orderid2}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid2}  
...
JD-TC-GetFutureOrderByCriteria-7
    ...
    ${resp}=  Get Future Order By Criterias   orderStatus-eq=${orderStatuses[0]}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1} 
    Verify Response List    ${resp}  1    uid=${orderid2} 
...
JD-TC-GetFutureOrderByCriteria-8
    ...
    ${resp}=  Get Future Order By Criterias   orderMode-eq=${order_mode[1]}   
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1} 
    Verify Response List    ${resp}  1    uid=${orderid2} 
...
JD-TC-GetFutureOrderByCriteria-9
    ...
    ${resp}=  Get Future Order By Criterias   email-eq=${email0}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid1} 
    Verify Response List    ${resp}  1    uid=${orderid2}   
...
JD-TC-GetFutureOrderByCriteria-10
    ...
    ${resp}=  Get Future Order By Criterias   phoneNumber-eq=${CUSERNAME2}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1} 
    Verify Response List    ${resp}  1    uid=${orderid2}   
...
JD-TC-GetFutureOrderByCriteria-12
    ...
    ${resp}=  Get Future Order By Criterias   jaldeeId-eq=${jcons_id1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
    Verify Response List    ${resp}  0    uid=${orderid3}  
    Verify Response List    ${resp}  1    uid=${orderid4}
    Verify Response List    ${resp}  2    uid=${orderid}      
...
JD-TC-GetFutureOrderByCriteria-13
    ...
    ${resp}=  Get Future Order By Criterias   jaldeeConsumer-eq=${jdconID}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid1}  
    Verify Response List    ${resp}  1    uid=${orderid2}
...
JD-TC-GetFutureOrderByCriteria-14
    ...
    ${firstname}=  FakerLibrary.first_name
    Set Test Variable  ${email}  ${firstname}${CUSERNAME7}.ynwtest@netvarth.com
    ...
    ${resp}=   Create Order By Provider For HomeDelivery    ${cookie}  ${cid20}   ${cid20}   ${CatalogId1}   ${boolean[1]}   ${address}  ${sTime3}    ${eTime3}   ${DAY1}    ${CUSERNAME7}    ${email}  ${orderNote}  ${countryCodes[1]}  ${item_id3}   ${item_quantity1}  ${item_id4}   ${item_quantity1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=   Create Order By Provider For HomeDelivery    ${cookie}  ${cid20}   ${cid20}   ${CatalogId2}   ${boolean[1]}   ${address}  ${sTime3}    ${eTime3}   ${DAY1}    ${CUSERNAME7}    ${email}  ${orderNote}  ${countryCodes[1]}  ${item_id3}   ${item_quantity1}  ${item_id4}   ${item_quantity1}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
    ...
    ${resp}=  Get Future Order By Criterias   orderNumber-eq=${ordernumber6} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
    Verify Response List    ${resp}  0    uid=${orderid6}   

    ${resp}=  Get Future Order By Criterias   uid-eq=${orderid5} 
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200 
...
JD-TC-GetFutureOrderByCriteria-UH1
    [Documentation]    Get Future order By account without login

    ${resp}=  Get Future Order By Criterias   account-eq=${accId1}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  419  
    Should Be Equal As Strings  "${resp.json()}"   "${SESSION_EXPIRED}"  

JD-TC-GetFutureOrderByCriteria-UH2
    [Documentation]  Get Future order By account with consumer login
    ${resp}=  Consumer Login  ${CUSERNAME2}  ${PASSWORD}
    Log   ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}    200
...
JD-TC-GetFutureOrderByCriteria-UH3
    ...
    ${resp}=  Get Future Order By Criterias   uid-eq=${orderid0}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  200  
