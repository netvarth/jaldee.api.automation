...
JD-TC-UpdateVirtualCallingMode-(Billable Subdomain)-1

    [Documentation]   Update Virtual Calling Mode before creating a Virtual_Service (Billable Subdomain)

    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+441122
...
# JD-TC-UpdateVirtualCallingMode-(Billable Subdomain)-3

#     [Documentation]   Update Virtual Calling Mode After creating a Virtual_Service (Billable Subdomain)
#     Comment  step 1:Enable virtual service and update virtual calling mode (Use SKYPE and WHATSAPP)
#     Comment  step 2:Create virtual service (Use SKYPE as virtual calling mode to create virtual service)
#     Comment  step 3:Again update virtual calling mode (Use IMO and WHATSAPP)
...
#     ${resp}=  Update Virtual Calling Mode   ${CallingModes[0]}  ${PUSERPH0}   ACTIVE  ${instructions1}   ${CallingModes[1]}  ${PUSERPH0}   ACTIVE   ${instructions2}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200
...
#     ${resp}=  Update Virtual Calling Mode   ${CallingModes[1]}  ${PUSERPH0}   ACTIVE  ${instructions1}   ${CallingModes[4]}  ${PUSERPH0}   ACTIVE   ${instructions2}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  422
#     Should Be Equal As Strings  "${resp.json()}"  "${VIRTUAL_MODES_CANT_BE_REMOVED}"
...
JD-TC-UpdateVirtualCallingMode-(Non-Billable Subdomain)-2
    [Documentation]   Update Virtual Calling Mode before creating a Virtual_Service (Non-Billable Subdomain)

    ${PUSERPH2}=  Evaluate  ${PUSERNAME}+11342
    ...
    ${PUSERPH_id2}=  Evaluate  ${PUSERNAME}+10501
...
# JD-TC-UpdateVirtualCallingMode-(Non-Billable Subdomain)-4
#     [Documentation]   Update Virtual Calling Mode After creating a Virtual_Service (Non-Billable Subdomain)
#     Comment  step 1:Enable virtual service and update virtual calling mode (Use SKYPE and WHATSAPP)
#     Comment  step 2:Create virtual service (Use SKYPE as virtual calling mode to create virtual service)
#     Comment  step 3:Again update virtual calling mode (Use IMO and WHATSAPP)
...
#     ${resp}=  Update Virtual Calling Mode   ${CallingModes[0]}  ${PUSERPH2}   ACTIVE  ${instructions1}   ${CallingModes[1]}  ${PUSERPH2}   ACTIVE   ${instructions2}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  200
...
#     Should Be Equal As Strings  ${resp.status_code}  200
#     Should Be Equal As Strings  ${resp.json()['virtualCallingModes'][0]['callingMode']}     ${CallingModes[0]}
#     Should Be Equal As Strings  ${resp.json()['virtualCallingModes'][0]['value']}           ${PUSERPH2}
#     Should Be Equal As Strings  ${resp.json()['virtualCallingModes'][0]['status']}          ACTIVE
#     Should Be Equal As Strings  ${resp.json()['virtualCallingModes'][0]['instructions']}    ${instructions1}
...
#     ${resp}=  Update Virtual Calling Mode   ${CallingModes[1]}  ${PUSERPH2}   ACTIVE  ${instructions1}   ${CallingModes[4]}  ${PUSERPH2}   ACTIVE   ${instructions2}
#     Log  ${resp.json()}
#     Should Be Equal As Strings  ${resp.status_code}  422
#     Should Be Equal As Strings  "${resp.json()}"  "${VIRTUAL_MODES_CANT_BE_REMOVED}"
...
JD-TC-UpdateVirtualCallingMode-UH1
    [Documentation]   Update Virtual Calling Mode without Provider Login
...
JD-TC-UpdateVirtualCallingMode-UH2
    [Documentation]   Update Virtual Calling Mode without Enabling Virtual Service
...
JD-TC-UpdateVirtualCallingMode-(Billable Subdomain)-UH3
    [Documentation]   Update Virtual Calling Mode without Enabling Virtual Service (Billable Subdomain)
    ${resp}=   Encrypted Provider Login  ${PUSERPH0}  ${PASSWORD} 
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-UpdateVirtualCallingMode-(Non-Billable Subdomain)-UH4
    [Documentation]   Update Virtual Calling Mode without Enabling Virtual Service (Non-Billable Subdomain)
    ${resp}=   Encrypted Provider Login  ${PUSERPH2}  ${PASSWORD} 
    Log  ${resp.json()}
    Should Be Equal As Strings    ${resp.status_code}   200
...
JD-TC-UpdateVirtualCallingMode-UH6

    [Documentation]   Update virtual calling mode using invalid Whatsapp number
    ...
    ${VirtualcallingMode1}=   Create Dictionary   callingMode=${CallingModes[1]}   value=${EMPTY}   countryCode=${countryCodes[0]}  status=ACTIVE    instructions=${instructions1} 
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"  "${INVALID_WHATSAPP_NUM}"
    


JD-TC-UpdateVirtualCallingMode-UH7

    [Documentation]   Update virtual calling mode using invalid Phone number
    ...
    ${VirtualcallingMode1}=   Create Dictionary   callingMode=${CallingModes[2]}   value=${EMPTY}   status=ACTIVE    instructions=${instructions1} 
    ${VirtualcallingMode2}=   Create Dictionary   callingMode=${CallingModes[0]}   value=${ZOOM_id2}   status=ACTIVE    instructions=${instructions2} 
    ${vcm1}=  Create List  ${VirtualcallingMode1}   ${VirtualcallingMode2}
...
JD-TC-UpdateVirtualCallingMode-UH8

    [Documentation]   Update virtual calling mode using invalid Google_meet id
    ...
    ${VirtualcallingMode1}=   Create Dictionary   callingMode=${CallingModes[3]}   value=${EMPTY}   status=ACTIVE    instructions=${instructions1} 
    ${VirtualcallingMode2}=   Create Dictionary   callingMode=${CallingModes[0]}   value=${ZOOM_id2}   status=ACTIVE    instructions=${instructions2} 
    ${vcm1}=  Create List  ${VirtualcallingMode1}   ${VirtualcallingMode2}
...
JD-TC-UpdateVirtualCallingMode-UH9

    [Documentation]   Update Virtual Calling Mode with Consumer Login
...
JD-TC-UpdateVirtualCallingMode-UH5

    [Documentation]   Update virtual calling mode using invalid zoom url
    ...
    ${resp}=  Update Virtual Calling Mode   ${CallingModes[0]}  ${PUSERPH2}   ACTIVE  ${instructions1}   ${CallingModes[1]}  ${PUSERPH0}   ACTIVE   ${instructions2}
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
