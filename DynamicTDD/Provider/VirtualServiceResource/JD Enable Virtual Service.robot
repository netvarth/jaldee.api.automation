...
JD-TC-EnableVirtualService-(Billable Subdomain)-1
    ...
    [Documentation]   Enable virtual service before Updating a virtual calling mode 

    ${PUSERPH0}=  Evaluate  ${PUSERNAME}+4334
...
JD-TC-EnableVirtualService-(Non-Billable Subdomain)-2
    [Documentation]   Enable virtual service before Updating a virtual calling mode

    ${PUSERPH2}=  Evaluate  ${PUSERNAME}+44344
...
JD-TC-EnableVirtualService-UH1
    [Documentation]   Enable virtual service without Provider Login
    ${resp}=  Enable Disable Virtual Service  Enable
    Log  ${resp.json()}
...
JD-TC-EnableVirtualService-UH2
    [Documentation]   Enable virtual service with Consumer Login
...
JD-TC-EnableVirtualService-(Billable Subdomain)-UH3
...
JD-TC-EnableVirtualService-(Non-Billable Subdomain)-UH4
    [Documentation]   Update virtual calling mode without Enabling virtual service
...
JD-TC-EnableVirtualService-(Billable Subdomain)-UH5
    [Documentation]   Enable virtual service Again (Billable Subdomain)
    ...
    Log  ${resp.json()}
    Should Be Equal As Strings  ${resp.status_code}  422
    Should Be Equal As Strings  "${resp.json()}"  "${VIRTUAL_SERVICES_ALREDY_ENABLED}"




JD-TC-EnableVirtualService-(Non-Billable Subdomain)-UH6
    [Documentation]   Enable virtual service Again (Non-Billable Subdomain)
